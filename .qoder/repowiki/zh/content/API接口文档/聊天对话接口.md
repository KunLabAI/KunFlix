...
## 更新摘要
**变更内容**
- 新增批量图片附件注入功能，支持从消息元数据中解析和注入多个图片附件
- 增强多模态输入处理能力，支持用户引用多张画布图片进行分析
- 添加 __ATTACHMENTS__ 元数据解析和处理机制
- 优化图片附件数量限制和安全性控制
...
subgraph "前端"
L["SSE 处理钩子<br/>useSSEHandler.ts"]
M["会话管理钩子<br/>useSessionManager.ts"]
N["AI 助手面板<br/>AIAssistantPanel.tsx"]
O["节点附件工具<br/>nodeAttachmentUtils.ts"]
P["AI助手存储<br/>useAIAssistantStore.ts"]
Q["聊天消息解析<br/>ChatMessage.tsx"]
end
A --> B
A --> C
B --> D
...
## 架构总览
后端通过 FastAPI 路由暴露聊天与编排接口，前端通过 fetch + ReadableStream 接收 SSE。生成流程分为单智能体与多智能体两种路径，均通过 LLM 流式调用与工具调度实现。**新增**的图片附件注入功能在消息发送前自动处理，将画布节点的图片转换为多模态输入。
...
## 结论
本聊天对话系统以 SSE 实现实时流式交互，结合单/多智能体生成与上下文压缩，提供高效、可扩展的对话体验。通过剧场上下文与画布联动，支持角色扮演与场景模拟。**新增的批量图片附件注入功能**进一步增强了多模态输入处理能力，支持用户引用多张画布图片进行分析，通过 __ATTACHMENTS__ 元数据系统实现安全、高效的图片处理。前端通过统一的 SSE 解析与会话管理，确保良好的用户体验与稳定性。