...
- [backend/admin/src/components/admin/agents/AgentForm/SystemPrompt.tsx](file://backend/admin/src/components/admin/agents/AgentForm/SystemPrompt.tsx)
...
**更新** 管理后台的提示模板管理系统已进行重大简化，移除了复杂的代理类型分类系统，采用更简单的基于预定义类别的模板管理方式。
...
I["/admin/prompt-templates/page.tsx<br/>提示模板管理"]
end
subgraph "通用层"
J["/context/AuthContext.tsx<br/>认证上下文"]
K["/hooks/*<br/>数据钩子"]
L["/lib/api-utils.ts<br/>API工具"]
M["/types/index.ts<br/>类型定义"]
end
A --> B
A --> C
A --> D
...
- [backend/admin/src/app/admin/prompt-templates/page.tsx:1-252](file://backend/admin/src/app/admin/prompt-templates/page.tsx#L1-L252)
...
- 页面级功能模块：用户管理、智能体管理、LLM供应商、工具与资源、订阅与计费、**提示模板管理**。
- 类型系统：集中定义用户、管理员、订阅计划、工具、视频任务、LLM提供商、Agent、**提示模板**等类型，确保前后端契约一致。

**更新** 新增提示模板管理模块，采用简化的预定义分类系统，支持模板的创建、编辑、删除和状态管理。

**章节来源**
- [backend/admin/src/context/AuthContext.tsx:1-117](file://backend/admin/src/context/AuthContext.tsx#L1-L117)
- [backend/admin/src/lib/api-utils.ts:1-19](file://backend/admin/src/lib/api-utils.ts#L1-L19)
- [backend/admin/src/types/index.ts:1-404](file://backend/admin/src/types/index.ts#L1-L404)
...
### 提示模板管理（新增）
- 功能：管理AI生成使用的提示词模板，支持分类标签、变量定义、状态控制。
- 分类系统：采用简化的预定义分类，包括故事基础设定、角色设定、场景描述、分镜脚本。
- 表单与校验：使用 react-hook-form + zod，支持模板名称、描述、分类标签、系统提示词、用户提示词、变量定义等。
- 对话框与确认：模板创建/编辑采用对话框，删除采用确认弹窗。
- 数据流：SWR 拉取模板列表，mutate 刷新；调用 /prompt-templates 进行 CRUD 操作。

**更新** 提示模板管理系统已简化，移除了复杂的代理类型分类系统，改为基于预定义类别的简单分类方式。

```mermaid
sequenceDiagram
participant U as "管理员"
participant P as "提示模板管理页"
participant D as "模板对话框"
participant API as "后端API"
participant SWR as "SWR缓存"
U->>P : 打开创建/编辑模板对话框
P->>D : 初始化表单
D->>API : POST/PUT /prompt-templates
API-->>D : 返回模板数据
D->>SWR : mutate 刷新列表
SWR-->>P : 模板列表更新
P-->>U : 显示成功/失败提示
...
**图表来源**
- [backend/admin/src/app/admin/prompt-templates/page.tsx:1-252](file://backend/admin/src/app/admin/prompt-templates/page.tsx#L1-L252)
- [backend/admin/src/app/admin/prompt-templates/PromptTemplateDialog.tsx:1-384](file://backend/admin/src/app/admin/prompt-templates/PromptTemplateDialog.tsx#L1-L384)
- [backend/admin/src/hooks/usePromptTemplates.ts:1-53](file://backend/admin/src/hooks/usePromptTemplates.ts#L1-L53)
...
- **提示模板性能**：简化后的模板分类系统减少了前端复杂度，提高了模板管理页面的响应速度。
...
- **提示模板管理问题**
  - 现象：模板分类显示异常或模板无法保存。
  - 排查：检查模板分类是否在预定义列表中；确认模板表单必填字段完整；查看后端返回的错误信息。
  - 处理：使用预定义分类选项；确保模板名称唯一且符合长度要求；检查变量定义格式。
...
## 结论
管理后台以 Next.js App Router 为基础，结合 SWR、Zod、React Hook Form 与 Recharts，构建了清晰的页面层、钩子层与类型层。认证与路由守卫保障了受保护资源的安全访问；工具化的钩子与统一的 API 工具提升了开发效率与一致性。

**更新** 提示模板管理系统经过简化重构，移除了复杂的代理类型分类系统，采用更直观的预定义分类方式，降低了管理复杂度，提高了用户体验。建议在生产环境中进一步完善网络拦截器、错误边界与审计日志，持续优化大数据量下的渲染与缓存策略。
...
- 最佳实践
  ...
  - **提示模板管理**：使用预定义分类系统，确保模板分类的一致性和易用性。
- 常用路径参考
  - 仪表盘：/admin
  - 登录：/admin/login
  - 用户管理：/admin/users
  ...
  - **提示模板管理**：/admin/prompt-templates

**更新** 新增提示模板管理模块的路径参考，便于管理员快速定位和使用模板管理功能。