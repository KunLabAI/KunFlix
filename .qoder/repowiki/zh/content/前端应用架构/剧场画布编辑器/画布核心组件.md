...
## 更新摘要
**变更内容**
- 新增外部文件拖放到画布功能，支持文本、图片、视频文件拖拽
- 新增文件类型检测和上传处理逻辑
- 更新节点类型支持text/image/video/storyboard
- 增强文件拖放状态管理和UI反馈
...
- **新增**：外部文件拖放到画布功能，支持文本、图片、视频文件的拖拽处理
- **新增**：文件类型检测和上传处理逻辑
- 组件间通信机制与数据传递方式
- 可定制性、样式系统与响应式设计实现

## 项目结构
前端采用 Next.js 客户端组件与 Zustand 状态管理，React Flow 提供画布渲染与交互，AI 助手通过 SSE 流式接收后端事件。**新增**外部文件拖放功能集成到 ReactFlow 画布中。
...
MediaUpload["媒体上传<br/>/api/media/upload"]
end
Page --> ReactFlow
Page --> Sidebar
Page --> Zoom
...
- **新增**：文件拖放功能：支持从操作系统文件管理器拖拽文本、图片、视频文件到画布
...
## 架构总览
画布采用"页面容器 + React Flow + 节点/边 + 状态管理 + AI 助手 + 文件拖放"的分层架构。页面容器负责装配与生命周期，React Flow 负责渲染与交互，Zustand 管理画布与 AI 助手状态，AI 助手通过 SSE 与后端流式通信，**新增**文件拖放功能提供外部文件到画布的直接处理能力。
...
participant FD as "文件拖放<br/>外部文件处理"
...
### 文件拖放功能：外部文件到画布处理
**新增功能**：支持从操作系统文件管理器拖拽文件到画布，自动识别文件类型并创建相应节点。

- **拖放状态管理**：跟踪拖放状态、文件类型、拖放位置
- **文件类型检测**：支持文本文件（txt、md、pdf）、图片文件（png、jpg、webp）、视频文件（mp4、webm、ogg）
- **文件大小限制**：文本文件10MB、图片文件5MB、视频文件500MB
- **节点创建**：根据文件类型创建对应节点，支持多文件拖拽偏移
- **上传处理**：异步上传文件到后端，更新节点数据
- **UI反馈**：拖放过程中的视觉反馈和状态指示

```mermaid
flowchart TD
DragStart["外部文件拖拽开始"] --> DetectType["检测文件类型"]
DetectType --> CheckSize{"检查文件大小限制"}
CheckSize --> |超限| ShowError["显示错误提示"]
CheckSize --> |符合| CreatePreview["创建本地预览URL"]
CreatePreview --> CreateNode["创建节点并添加到画布"]
CreateNode --> UploadFile["上传文件到后端"]
UploadFile --> UpdateNode["更新节点数据"]
ShowError --> End["结束"]
UpdateNode --> End
```

**图表来源**
- [page.tsx:277-522](file://frontend/src/app/theater/[id]/page.tsx#L277-L522)

**章节来源**
- [page.tsx:78-822](file://frontend/src/app/theater/[id]/page.tsx#L78-L822)
...
- **新增**：文件拖放功能集成到 ReactFlow 画布中，提供外部文件处理能力
...
AIStore --> API
FileDrop --> MediaUpload["/api/media/upload"]
```
...
- **新增**：文件拖放使用本地URL预览，避免阻塞主线程
- **新增**：文件上传使用XMLHttpRequest，支持进度条和错误处理
- 自动保存去抖（2 秒）降低后端压力
...
- **新增**：文件拖放失败：检查浏览器是否支持拖放API，确认文件类型和大小限制
- **新增**：上传失败：检查网络连接、后端API可用性和认证令牌
...
## 结论
本画布核心组件体系以 React Flow 为基础，结合 Zustand 状态管理与 SSE 实时通信，实现了高交互性的可视化编辑体验。**新增的文件拖放功能**进一步增强了用户体验，支持从操作系统直接拖拽文件到画布进行处理。组件职责清晰、状态持久化完善、扩展性强，适合在复杂叙事与多媒体创作场景中进一步演进。

## 附录
- 可定制性：节点/边类型注册、Handle 自定义、样式覆盖
- 样式系统：Tailwind 类名组织、颜色变量、动画与过渡
- 响应式设计：侧边栏固定定位、面板绝对定位、缩放与适配视图
- **新增**：文件拖放UI：拖放覆盖层、文件类型图标、进度指示器