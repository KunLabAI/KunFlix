...
## 简介
本文件面向节点系统的技术文档，聚焦于四种节点类型：角色节点（CharacterNode）、剧本节点（ScriptNode）、故事板节点（StoryboardNode）与视频节点（VideoNode）。文档从数据结构、属性定义、状态管理、渲染逻辑、样式定制、交互行为、组件组合与继承关系，以及序列化、持久化与版本控制机制等方面进行全面阐述，并辅以可视化图表帮助理解。
...
## 核心组件
本节概述四种节点的核心职责与共同特性：
- 角色节点（CharacterNode）：承载角色图片与元数据，支持上传、缩放模式切换、预览、复制与删除。
- 剧本节点（ScriptNode）：承载富文本内容，集成 Tiptap 编辑器，支持标题编辑、字数统计、复制与删除。
- 故事板节点（StoryboardNode）：承载多维透视表配置与展示，支持全屏编辑、复制与删除。
...
- 工具层：拓扑检测防止循环依赖；编辑器工具链（Tiptap）提供富文本能力。
...
### 角色节点（CharacterNode）
- 数据结构与属性
  - 名称、描述、头像、图片地址、上传状态、适配模式（cover/contain）
- 渲染逻辑
...
## 结论
节点系统通过统一的状态管理与组件化设计，实现了角色、剧本、故事板与视频四类节点的一致交互体验与强大的扩展性。结合富文本编辑器、透视表编辑器与边组件，系统在创作流程中提供了高效的内容组织与可视化编排能力。持久化与剧院同步机制保障了数据安全与协作一致性。
...
### 数据结构与类型定义
- 节点数据类型
  - 角色节点：名称、描述、头像、图片地址、上传状态、适配模式
  - 剧本节点：标题、内容（Tiptap JSON）、标签、角色、场景
