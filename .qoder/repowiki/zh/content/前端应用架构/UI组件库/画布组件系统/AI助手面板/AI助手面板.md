...
## 更新摘要
**变更内容**
- 新增上下文压缩功能：集成完整的上下文压缩系统，包括后端压缩服务和前端事件处理
- 增强SSE事件处理：新增context_compacted事件处理，支持上下文压缩完成通知
- 优化WelcomeMessage组件：改进个性化欢迎显示和预设对话入口
- 增强上下文使用统计：在多个SSE事件中都能更新上下文使用统计
- 改进上下文压缩配置：支持智能体级别的上下文压缩配置管理
...
8. [欢迎状态处理系统](#欢迎状态处理系统)
9. [面板布局优化](#面板布局优化)
10. [消息列表渲染改进](#消息列表渲染改进)
11. [消息状态管理增强](#消息状态管理增强)
12. [会话初始化优化](#会话初始化优化)
13. [多附件支持系统](#多附件支持系统)
14. [附件预览组件](#附件预览组件)
15. [附件拖拽功能](#附件拖拽功能)
16. [附件上下文构建](#附件上下文构建)
17. [主题变量使用改进](#主题变量使用改进)
18. [虚拟滚动系统](#虚拟滚动系统)
19. [懒加载优化](#懒加载优化)
20. [性能监控系统](#性能监控系统)
21. [认证与令牌刷新机制](#认证与令牌刷新机制)
22. [依赖关系分析](#依赖关系分析)
23. [性能考量](#性能考量)
24. [故障排查指南](#故障排查指南)
25. [结论](#结论)
...
- AIAssistantPanel 主面板的控制流与状态管理，包括新增的上下文压缩功能、欢迎状态处理、面板布局优化和消息列表渲染改进
- 上下文压缩系统：完整的上下文压缩服务，包括后端压缩算法、智能体配置管理和前端事件处理
- 欢迎状态处理系统：WelcomeMessage组件的完整实现，支持预设对话快捷入口和用户个性化欢迎
- 面板布局优化：针对仅有欢迎消息时的特殊布局处理，采用底部对齐的用户体验设计
- 消息列表渲染改进：新增isWelcome字段区分欢迎消息与其他消息，优化消息过滤逻辑
...
- 消息渲染组件 ChatMessage 的渲染逻辑与样式策略，包括欢迎消息的特殊处理
- 消息输入组件 MessageInput 的输入、发送与历史交互
- 实时通信机制：基于 SSE 的流式传输与状态同步，现已集成增强的认证处理和上下文压缩事件
...
- 主面板：AIAssistantPanel 负责面板生命周期、会话初始化、消息发送与滚动，现已集成上下文压缩功能、欢迎状态处理、面板布局优化和消息列表渲染改进
- 子组件：ChatMessage、MessageInput、PanelHeader、ThinkingIndicator、SkillCallIndicator、ToolCallIndicator、TypewriterText、MultiAgentSteps、LoadingDots、ContextUsageBar、NodePreviewCard、WelcomeMessage
...
- 状态管理：useAIAssistantStore 提供全局状态与持久化，新增欢迎消息支持和虚拟滚动配置
- 会话与SSE：useSessionManager、useSSEHandler 负责会话生命周期与流式事件解析，现已支持上下文压缩事件
- 性能监控：usePerformanceMonitor 提供全面的性能指标监测
- 网络层：api.ts 统一封装请求与鉴权拦截
- 权限与积分：AuthContext 提供用户与积分状态更新，现已集成自动令牌刷新
- 上下文压缩：context_compaction.py 提供后端上下文压缩服务，agent_extensions.py 提供记忆压缩钩子
...
subgraph "上下文压缩"
ContextService["context_compaction.py"]
AgentExt["agent_extensions.py"]
Models["models.py"]
end
AIPanel --> Header
AIPanel --> VirtualList
...
AuthFetch --> API
SSE --> ContextService
ContextService --> AgentExt
ContextService --> Models
```

**图表来源**
- [AIAssistantPanel.tsx:14-613](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L14-L613)
...
- [models.py:178-271](file://backend/models.py#L178-L271)

## 核心组件
- AIAssistantPanel：主面板容器，负责面板显隐、拖拽定位、尺寸调整、消息发送、SSE事件处理与会话初始化，现已集成上下文压缩功能、欢迎状态处理、面板布局优化和消息列表渲染改进
...
- ChatMessage：消息渲染组件，支持 Markdown、打字机动画、思考指示器、工具/技能调用指示器、多智能体协作展示，现已支持欢迎消息的特殊渲染
...
- WelcomeMessage：欢迎消息组件，提供预设对话快捷入口和用户个性化欢迎
- useAIAssistantStore：Zustand 状态存储，包含消息、会话、面板尺寸位置、图像编辑上下文、剧院切换与持久化，新增欢迎消息支持和虚拟滚动配置
- useSessionManager：会话生命周期管理，加载可用智能体、创建/切换/清空会话、加载历史消息，现已支持欢迎消息的会话初始化
- useSSEHandler：SSE 事件解析与状态机，将服务端事件映射为消息流与 UI 状态，包含上下文使用统计更新和新增的context_compacted事件处理
...
- AuthContext：用户与积分状态更新，现已集成自动令牌刷新
- context_compaction.py：上下文压缩服务，提供完整的上下文压缩算法和配置管理
- agent_extensions.py：Agent扩展，提供记忆压缩钩子和工具守卫功能
- models.py：数据库模型，包含ChatSession和Agent的上下文压缩相关字段
...
## 架构总览
AI助手面板采用"组件-状态-网络-权限-性能监控-附件系统-上下文压缩"八层架构，现已增强上下文压缩功能、欢迎状态处理、面板布局优化和消息列表渲染改进：
- 组件层：主面板与子组件负责 UI 呈现与用户交互，新增欢迎消息组件和布局优化
- 状态层：Zustand store 管理消息、会话、面板尺寸与剧院切换，新增欢迎消息支持和虚拟滚动配置
- 网络层：Axios 封装统一鉴权与401刷新；SSE 通过 fetch + ReadableStream 解析，现已支持上下文压缩事件
- 权限层：鉴权上下文与积分状态联动，现已集成自动令牌刷新
- 性能监控层：usePerformanceMonitor钩子提供全面的性能指标监测，支持FPS监控
- 附件系统层：多附件支持、拖拽交互、预览组件和上下文构建的完整链路
- 欢迎状态层：WelcomeMessage组件提供个性化欢迎和预设对话入口
- 上下文压缩层：完整的上下文压缩系统，包括后端压缩服务和前端事件处理
...
participant CS as "context_compaction.py"
...
- **新增** 上下文压缩集成：在发送消息时自动触发上下文压缩检查，支持智能体级别的压缩配置
- **新增** 欢迎状态处理：当仅有欢迎消息时，采用底部对齐布局，提供更好的用户体验
- **新增** 面板布局优化：针对欢迎界面的特殊布局处理，确保欢迎消息在面板底部显示
- **新增** 消息列表渲染改进：新增isWelcome字段区分欢迎消息与其他消息，优化消息过滤逻辑
...
**更新** 新增了上下文压缩功能、欢迎状态处理、面板布局优化和消息列表渲染改进，AI助手面板现在具备完整的上下文压缩能力、欢迎体验、布局优化和消息过滤能力
...
StartPerfMon --> CheckWelcome["检查欢迎状态"]
CheckWelcome --> |仅有欢迎消息| WelcomeLayout["应用欢迎布局"]
CheckWelcome --> |有其他消息| NormalLayout["应用常规布局"]
WelcomeLayout --> SendMsg["用户发送消息"]
NormalLayout --> SendMsg
SendMsg --> BuildAttachmentContext["构建附件上下文"]
BuildAttachmentContext --> CheckContextCompaction["检查上下文压缩"]
CheckContextCompaction --> |需要压缩| TriggerCompaction["触发上下文压缩"]
CheckContextCompaction --> |不需要压缩| AbortPrev["取消前一请求"]
TriggerCompaction --> AbortPrev
...
#### 后端上下文压缩服务
context_compaction.py提供了完整的上下文压缩服务，包括：

- **配置管理**：支持智能体级别的上下文压缩配置，包括启用开关、摘要生成供应商、模型选择、压缩阈值等
- **令牌估算**：基于字符长度估算消息令牌数，支持混合CJK/Latin文本
- **工具结果截断**：对工具调用结果进行智能截断，保留最近的完整结果
- **压缩决策**：根据上下文窗口和压缩阈值决定是否需要压缩
- **消息分割**：智能分割需要压缩和保留的消息，保留系统提示和最近消息
- **摘要生成**：使用LLM生成旧消息摘要，支持自定义摘要模型和供应商
- **数据库持久化**：将压缩摘要和统计信息保存到数据库

**新增** 完整的上下文压缩系统，提供智能的上下文窗口管理能力

```mermaid
flowchart TD
Init(["初始化上下文压缩"]) --> LoadConfig["加载智能体配置"]
LoadConfig --> TruncateTools["截断工具结果"]
TruncateTools --> CheckNeeded{"检查是否需要压缩"}
CheckNeeded --> |否| ReturnNone["返回None"]
CheckNeeded --> |是| SplitMsgs["分割消息"]
SplitMsgs --> GenerateSummary["生成摘要"]
GenerateSummary --> RebuildMsgs["重建消息列表"]
RebuildMsgs --> PersistDB["持久化到数据库"]
PersistDB --> ReturnNew["返回新消息"]
ReturnNone --> End(["结束"])
ReturnNew --> End
```

**图表来源**
- [context_compaction.py:59-62](file://backend/services/context_compaction.py#L59-L62)
- [context_compaction.py:208-305](file://backend/services/context_compaction.py#L208-L305)
...
#### 上下文压缩配置
Agent模型新增了compaction_config字段，支持：

- **enabled**：是否启用上下文压缩
- **provider_id**：摘要生成供应商ID
- **model**：摘要生成模型名称
- **compact_ratio**：压缩触发阈值（0.5-0.95）
- **reserve_ratio**：保留比例（0.05-0.4）
- **tool_old_threshold**：旧工具结果截断字符数
- **tool_recent_n**：最近完整保留工具数量
- **tool_recent_threshold**：最近工具结果截断字符数
- **max_summary_tokens**：摘要最大令牌数
...
+tool_recent_threshold : number
+max_summary_tokens : number
}
Agent --> CompactionConfig : "配置"
```

**图表来源**
- [models.py:264-265](file://backend/models.py#L264-L265)
- [context_compaction.py:45-57](file://backend/services/context_compaction.py#L45-L57)
...
#### context_compacted事件处理
useSSEHandler新增了context_compacted事件处理：

- **静默处理**：上下文压缩对用户透明，无需额外UI反馈
- **事件触发**：后端完成上下文压缩后发送此事件
- **性能优化**：避免用户感知到上下文压缩过程
- **状态保持**：压缩完成后继续正常的聊天流程

**新增** 上下文压缩完成事件处理，提供透明的压缩体验

```mermaid
flowchart TD
Backend["后端完成上下文压缩"] --> SendEvent["发送context_compacted事件"]
SendEvent --> SSEHandler["useSSEHandler接收事件"]
SSEHandler --> SilentProcess["静默处理压缩完成"]
SilentProcess --> ContinueChat["继续正常聊天流程"]
ContinueChat --> End(["结束"])
```

**图表来源**
- [useSSEHandler.ts:351-354](file://frontend/src/components/ai-assistant/hooks/useSSEHandler.ts#L351-L354)
- [context_compaction.py:292-298](file://backend/services/context_compaction.py#L292-L298)
...
#### WelcomeMessage 欢迎消息组件
WelcomeMessage是新增的欢迎消息组件，提供以下功能：

- **个性化欢迎**：显示用户昵称和个性化欢迎语，包含摇动手势动画效果
- **预设对话快捷入口**：提供4个预设对话按钮，支持快速开始创作
- **图标标识**：每个预设对话都有对应的图标标识，提升视觉识别度
- **响应式网格布局**：支持2列网格布局，适配不同屏幕尺寸
- **动画效果**：包含hover和tap动画效果，提升交互体验
- **主题色适配**：使用CSS变量适配当前主题色

**新增** 全新的欢迎消息组件，提供个性化的欢迎体验和预设对话入口

```mermaid
flowchart TD
Init(["初始化欢迎消息"]) --> GetUser["获取用户信息"]
GetUser --> CheckUsername{"检查用户名"}
CheckUsername --> |有用户名| ShowWelcome["显示个性化欢迎"]
CheckUsername --> |无用户名| ShowDefault["显示默认欢迎"]
ShowWelcome --> ShowPresets["显示预设对话按钮"]
ShowDefault --> ShowPresets
ShowPresets --> SetupAnimation["设置动画效果"]
SetupAnimation --> Render["渲染欢迎界面"]
```
...
#### 欢迎界面布局处理
AIAssistantPanel新增了针对欢迎界面的特殊布局处理：

- **欢迎状态检测**：当messages.length === 1且messages[0].isWelcome为true时，进入欢迎布局模式
- **底部对齐布局**：欢迎界面采用flex-col justify-end布局，确保欢迎消息在面板底部显示
- **专用容器**：使用独立的div容器包裹WelcomeMessage，提供专用的布局样式
- **滚动优化**：欢迎界面不显示滚动条，避免遮挡欢迎消息
- **间距优化**：提供适当的底部间距，确保欢迎消息不会贴到底部边缘

**新增** 针对欢迎界面的专用布局处理，提供更好的用户体验

```mermaid
flowchart TD
CheckMsg["检查消息数量"] --> IsWelcome{"仅有一个欢迎消息?"}
IsWelcome --> |是| WelcomeLayout["应用欢迎布局"]
IsWelcome --> |否| NormalLayout["应用常规布局"]
WelcomeLayout --> BottomAlign["底部对齐"]
BottomAlign --> NoScroll["隐藏滚动条"]
NoScroll --> Padding["设置底部间距"]
Padding --> Render["渲染欢迎界面"]
NormalLayout --> Render
```

**图表来源**
- [AIAssistantPanel.tsx:457-462](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L457-L462)
...
#### 欢迎消息过滤逻辑
AIAssistantPanel改进了消息列表渲染逻辑，新增欢迎消息过滤：

- **消息过滤**：在虚拟滚动中使用messages.filter(m => !m.isWelcome)过滤欢迎消息
- **索引计算**：使用realMessages.length计算最后一条消息的索引
- **渲染优化**：欢迎消息不参与虚拟滚动渲染，提升性能
- **状态同步**：欢迎消息的isWelcome字段确保正确的消息分类

**更新** 新增了欢迎消息的过滤逻辑，优化了消息列表的渲染性能

```mermaid
flowchart TD
GetMessages["获取消息列表"] --> FilterWelcome["过滤欢迎消息"]
FilterWelcome --> CalcIndex["计算索引"]
CalcIndex --> RenderVirtual["渲染虚拟滚动"]
RenderVirtual --> UpdateLast["更新最后消息状态"]
UpdateLast --> ScrollBehavior["智能滚动控制"]
```

**图表来源**
- [AIAssistantPanel.tsx:464-478](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L464-L478)
...
#### Message接口扩展
useAIAssistantStore中扩展了Message接口，新增isWelcome字段：

- **isWelcome字段**：布尔类型，标识消息是否为欢迎消息
- **默认消息**：DEFAULT_MESSAGES数组包含isWelcome: true的欢迎消息
- **状态管理**：支持欢迎消息的完整生命周期管理
- **渲染控制**：isWelcome字段控制消息的渲染逻辑和布局处理

**更新** 新增了isWelcome字段，支持欢迎消息的完整状态管理
...
#### 默认欢迎消息
useSessionManager中优化了会话初始化逻辑：

- **默认消息数组**：DEFAULT_MESSAGES包含isWelcome: true的欢迎消息
- **会话恢复**：从后端恢复会话时，如果历史消息为空则使用默认欢迎消息
- **状态保持**：欢迎消息的状态在会话切换时得到保持
- **用户体验**：确保用户始终能看到欢迎消息，提供更好的引导体验

**更新** 优化了会话初始化逻辑，确保默认欢迎消息的正确显示

```mermaid
flowchart TD
InitSession["初始化会话"] --> CheckHistory["检查历史消息"]
CheckHistory --> |有历史消息| UseHistory["使用历史消息"]
CheckHistory --> |无历史消息| UseWelcome["使用默认欢迎消息"]
UseHistory --> SetMessages["设置消息状态"]
UseWelcome --> SetMessages
SetMessages --> Render["渲染面板"]
```

**图表来源**
- [useSessionManager.ts:8-10](file://frontend/src/components/ai-assistant/hooks/useSessionManager.ts#L8-L10)
- [useSessionManager.ts:81-88](file://frontend/src/components/ai-assistant/hooks/useSessionManager.ts#L81-L88)
...
- **新增** 欢迎消息渲染：当message.isWelcome为true时，显示WelcomeMessage组件
- **优化** 等待动画逻辑：移除了不必要的等待动画，仅在特定条件下显示加载指示器

**更新** 新增了欢迎消息的特殊渲染逻辑，优化了消息渲染性能
...
ChatMessage --> WelcomeMessage : "欢迎消息"
```
...
- **新增** 上下文压缩完成：context_compacted 事件静默处理上下文压缩完成通知

**更新** 增强了上下文使用统计的更新机制，在多个 SSE 事件中都能更新上下文使用情况，并新增了上下文压缩完成事件处理
...
- **新增** 默认欢迎消息：当历史消息为空时，使用包含isWelcome的默认欢迎消息

**更新** 新增了默认欢迎消息的支持，优化了会话初始化体验
...
UseExisting --> CheckHistory{"历史消息是否为空"}
CheckHistory --> |是| UseWelcome["使用默认欢迎消息"]
CheckHistory --> |否| UseHistory["使用历史消息"]
UseWelcome --> Done["完成"]
UseHistory --> Done
CreateNew --> Done
```
...
### 数据模型与状态
- Message：角色、内容、状态（流式/完成）、扩展字段（技能/工具/多智能体）、欢迎状态标记
...
- **新增** Message接口扩展：新增isWelcome字段，支持欢迎消息的状态管理
- **新增** DEFAULT_MESSAGES：包含isWelcome: true的默认欢迎消息数组
...
- **新增** CompactionConfig：上下文压缩配置数据结构，支持智能体级别的压缩配置
- **新增** ChatSession：新增compressed_summary和compressed_before_id字段，支持上下文压缩状态持久化
- **新增** Agent：新增compaction_config字段，支持智能体级别的上下文压缩配置

**更新** 新增了Message接口扩展、DEFAULT_MESSAGES和WelcomeMessage相关数据模型，以及上下文压缩相关的数据模型，用于支持完整的上下文压缩生命周期管理
...
string provider_id
string model
number compact_ratio
number reserve_ratio
number tool_old_threshold
number tool_recent_n
number tool_recent_threshold
number max_summary_tokens
}
CHAT_SESSION {
...
COMPACTION_CONFIG ||--|| AGENT : "配置"
CHAT_SESSION ||--|| AGENT : "关联"
```
...
- [context_compaction.py:45-57](file://backend/services/context_compaction.py#L45-L57)
- [models.py:190-192](file://backend/models.py#L190-L192)
- [models.py:264-265](file://backend/models.py#L264-L265)
...
- **新增** 上下文压缩依赖：context_compaction.py 依赖 SQLAlchemy 和 OpenAI API
- **新增** 欢迎状态依赖：WelcomeMessage组件依赖AuthContext获取用户信息
- **新增** 消息过滤依赖：AIAssistantPanel依赖Message接口的isWelcome字段进行消息过滤
- **新增** 会话初始化依赖：useSessionManager依赖DEFAULT_MESSAGES提供默认欢迎消息
...
- **新增** Agent扩展依赖：agent_extensions.py 提供MemoryCompactionHook功能

**更新** 新增了上下文压缩系统、欢迎状态处理、消息过滤和会话初始化相关的依赖关系
...
AIP --> WELCOME["WelcomeMessage"]
AIP --> COMPACT["context_compaction.py"]
...
COMPACT --> MODELS["models.py"]
COMPACT --> AGENTEXT["agent_extensions.py"]
STORE --> API
INPUT --> CONTEXT
HEADERCTX --> CONTEXT
WELCOME --> AUTH
...
- **欢迎消息优化**：欢迎消息不参与虚拟滚动渲染，提升性能
- **消息过滤优化**：isWelcome字段提供快速的消息分类，减少渲染开销
- **布局优化**：欢迎界面底部对齐布局，避免滚动条干扰
- **上下文压缩优化**：智能压缩算法减少令牌使用，提升聊天效率
- **压缩配置优化**：智能体级别的压缩配置，支持灵活的压缩策略
- **事件处理优化**：context_compacted事件静默处理，避免UI闪烁

**更新** 新增了上下文压缩系统、欢迎状态处理、消息过滤和布局优化相关的性能考量
...
- **欢迎消息显示异常**：检查isWelcome字段是否正确设置；确认消息过滤逻辑
- **面板布局错乱**：检查欢迎界面的布局处理逻辑；确认消息数量判断
- **消息渲染错误**：检查Message接口的扩展字段；确认渲染条件判断
- **上下文压缩失败**：检查compaction_config配置；确认摘要生成模型可用
- **压缩事件处理异常**：检查context_compacted事件处理逻辑；确认SSE事件正确传递
- **压缩配置不生效**：检查Agent.compaction_config字段；确认数据库配置正确
- **压缩摘要丢失**：检查ChatSession.compressed_summary字段；确认数据库持久化成功

**更新** 新增了上下文压缩系统、SSE事件处理和压缩配置相关的故障排查指南
...
## 结论
AI助手面板通过清晰的分层架构与完善的Hook体系，实现了从会话管理、实时流式传输到消息渲染与多智能体协作的完整闭环。新增的上下文压缩系统通过context_compaction.py提供了完整的上下文窗口管理能力，包括智能的令牌估算、工具结果截断、消息分割和摘要生成，显著提升了长对话场景下的性能和效率。上下文压缩对用户完全透明，通过context_compacted事件静默处理压缩完成通知，避免了UI闪烁和用户体验中断。欢迎状态处理系统通过WelcomeMessage组件提供了个性化的欢迎体验，包含用户昵称显示、摇手势动画和预设对话快捷入口。面板布局优化针对仅有欢迎消息的特殊情况，采用了底部对齐的布局处理，确保欢迎消息在面板底部显示，提供更好的用户体验。消息列表渲染改进通过新增isWelcome字段区分欢迎消息与其他消息，优化了消息过滤逻辑和渲染性能。改进的主题变量使用通过CSS变量和主题色的统一管理，提供了更好的主题一致性和可维护性。新增的虚拟滚动系统通过VirtualMessageList组件基于react-window实现高性能消息列表渲染，支持动态行高、智能滚动和overscan优化，显著提升了大量消息场景下的性能表现。滚动到底部按钮组件提供了用户友好的消息导航功能，支持新消息提醒和动画效果。懒加载组件LazyCodeBlock和LazyImage通过IntersectionObserver实现按需加载，显著减少了初始包体积和首屏加载时间。全面的性能监控系统通过usePerformanceMonitor钩子提供了包括Long Task检测、LCP、FID、CLS和FPS在内的全方位性能指标监测，帮助开发者识别和解决性能瓶颈。增强的认证处理机制通过createAuthFetch函数提供自动令牌刷新功能，确保用户获得无缝的认证体验。改进的viewport约束算法和拖拽交互进一步提升了面板的易用性和用户体验。其可扩展的状态模型与事件驱动的SSE处理，为后续自定义消息类型与交互行为提供了良好基础。

## 附录

### 配置选项与扩展开发指南
- **上下文压缩配置**
  - 在 Agent 模型中扩展compaction_config字段，支持智能体级别的压缩配置
  - 在 context_compaction.py 中配置压缩阈值、保留比例和摘要生成参数
  - 在 useSSEHandler 中处理context_compacted事件，提供透明的压缩体验
  - **新增** 支持智能体级别的上下文压缩配置，提供灵活的压缩策略
  - 参考路径：[models.py:264-265](file://backend/models.py#L264-L265)，[context_compaction.py:45-57](file://backend/services/context_compaction.py#L45-L57)，[useSSEHandler.ts:351-354](file://frontend/src/components/ai-assistant/hooks/useSSEHandler.ts#L351-L354)
- **欢迎状态配置**
  - 在 Message 接口中扩展isWelcome字段，支持欢迎消息的状态管理
  - 在 DEFAULT_MESSAGES 中配置默认欢迎消息，包含个性化欢迎语和预设对话
  - 在 AIAssistantPanel 中使用isWelcome字段进行消息过滤和布局处理
  - 参考路径：[useAIAssistantStore.ts:50-61](file://frontend/src/store/useAIAssistantStore.ts#L50-L61)，[useAIAssistantStore.ts:200-202](file://frontend/src/store/useAIAssistantStore.ts#L200-L202)，[AIAssistantPanel.tsx:457-462](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L457-L462)
- **自定义消息类型**
  - 在 Message 接口中扩展字段（如多媒体内容），并在 ChatMessage 中新增渲染分支
  - 在 useSSEHandler 中增加对应事件类型，更新 store 并触发渲染
  - **新增** 支持欢迎消息的isWelcome字段，提供完整的消息类型扩展
...
- **自定义交互行为**
  ...
  - **新增** 欢迎消息的预设对话快捷入口，提供快速交互体验
  - 参考路径：[MessageInput.tsx:32-50](file://frontend/src/components/ai-assistant/MessageInput.tsx#L32-L50)，[PanelHeader.tsx:93-120](file://frontend/src/components/ai-assistant/PanelHeader.tsx#L93-L120)，[WelcomeMessage.tsx:58-75](file://frontend/src/components/ai-assistant/WelcomeMessage.tsx#L58-L75)
- **多智能体协作**
...
- **网络与鉴权**
  ...
  - **新增** AuthContext中新增WelcomeMessage组件，提供个性化欢迎体验
...
- **增强的认证处理**
  ...
  - **新增** WelcomeMessage组件中使用AuthContext获取用户信息
  - 参考路径：[AIAssistantPanel.tsx:29-33](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L29-L33)，[AuthContext.tsx:49](file://frontend/src/context/AuthContext.tsx#L49)，[AuthContext.tsx:171-199](file://frontend/src/context/AuthContext.tsx#L171-L199)，[WelcomeMessage.tsx:28-30](file://frontend/src/components/ai-assistant/WelcomeMessage.tsx#L28-L30)
- **加载状态管理**
...
- **主题变量扩展**
  ...
  - 在组件中使用CSS变量实现主题切换
  - **新增** 欢迎消息组件使用CSS变量适配主题色
  - 参考路径：[_variables.scss:90-162](file://frontend/src/styles/_variables.scss#L90-L162)，[globals.css:216-247](file://frontend/src/app/globals.css#L216-L247)，[WelcomeMessage.tsx:35-55](file://frontend/src/components/ai-assistant/WelcomeMessage.tsx#L35-L55)
- **附件系统扩展**
  ...
  - **新增** 欢迎消息组件的预设对话快捷入口
  - 参考路径：[nodeAttachmentUtils.ts:24-81](file://frontend/src/lib/nodeAttachmentUtils.ts#L24-L81)，[AIAssistantPanel.tsx:27-33](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L27-L33)，[useNodeDragToAI.ts:93-101](file://frontend/src/app/theater/[id]/hooks/useNodeDragToAI.ts#L93-L101)，[CharacterNode.tsx:119-130](file://frontend/src/components/canvas/CharacterNode.tsx#L119-L130)，[WelcomeMessage.tsx:8-14](file://frontend/src/components/ai-assistant/WelcomeMessage.tsx#L8-L14)
- **上下文压缩系统扩展**
  - 在 Agent 模型中扩展compaction_config字段，支持智能体级别的压缩配置
  - 在 context_compaction.py 中调整压缩算法参数和策略
  - 在 useSSEHandler 中处理context_compacted事件，提供透明的压缩体验
  - **新增** 支持自定义摘要生成模型和供应商
  - **新增** 支持工具结果的智能截断和保留策略
  - 参考路径：[models.py:264-265](file://backend/models.py#L264-L265)，[context_compaction.py:208-305](file://backend/services/context_compaction.py#L208-L305)，[useSSEHandler.ts:351-354](file://frontend/src/components/ai-assistant/hooks/useSSEHandler.ts#L351-L354)

### 新增功能详解

#### 上下文压缩系统
- **智能压缩算法**：基于令牌估算和消息分割，智能决定压缩阈值和保留策略
- **摘要生成**：使用LLM生成旧消息摘要，支持自定义摘要模型和供应商
- **配置管理**：支持智能体级别的压缩配置，包括启用开关、阈值设置和模型选择
- **数据库持久化**：将压缩摘要和统计信息保存到数据库，支持会话恢复
- **事件处理**：通过context_compacted事件静默处理压缩完成通知
- **性能优化**：显著减少令牌使用，提升长对话场景下的性能和效率

#### 上下文压缩配置
- **enabled**：是否启用上下文压缩功能
- **provider_id**：摘要生成供应商ID，支持自定义摘要模型
- **model**：摘要生成模型名称，支持不同供应商的模型选择
- **compact_ratio**：压缩触发阈值，控制何时触发压缩（0.5-0.95）
- **reserve_ratio**：保留比例，控制保留的最近消息比例（0.05-0.4）
- **tool_old_threshold**：旧工具结果截断字符数，控制工具结果的截断策略
- **tool_recent_n**：最近完整保留工具数量，控制最近工具结果的保留数量
- **tool_recent_threshold**：最近工具结果截断字符数，控制最近工具结果的截断策略
- **max_summary_tokens**：摘要最大令牌数，控制摘要生成的长度

#### 上下文压缩事件处理
- **context_compacted事件**：后端完成上下文压缩后发送此事件
- **静默处理**：事件处理函数为空，提供透明的压缩体验
- **性能优化**：避免用户感知到压缩过程，提升用户体验
- **状态保持**：压缩完成后继续正常的聊天流程

#### 欢迎状态处理系统
- **WelcomeMessage组件**：提供个性化欢迎体验，包含用户昵称显示和摇手势动画
- **预设对话快捷入口**：提供4个预设对话按钮，支持快速开始创作
- **消息状态管理**：通过isWelcome字段区分欢迎消息与其他消息
- **布局优化**：针对欢迎界面的底部对齐布局处理
- **会话初始化**：默认消息数组包含欢迎消息，提供更好的用户体验

#### 面板布局优化
- **欢迎界面检测**：当仅有欢迎消息时，自动进入欢迎布局模式
- **底部对齐**：欢迎界面采用flex-col justify-end布局
- **滚动优化**：欢迎界面不显示滚动条，避免遮挡欢迎消息
- **间距优化**：提供适当的底部间距，确保欢迎消息美观显示

#### 消息列表渲染改进
- **消息过滤**：使用filter方法过滤欢迎消息，提升渲染性能
- **索引计算**：使用realMessages.length计算最后一条消息索引
- **状态同步**：欢迎消息的isWelcome字段确保正确的消息分类
- **渲染优化**：欢迎消息不参与虚拟滚动渲染，减少DOM节点数量

#### 消息状态管理增强
- **Message接口扩展**：新增isWelcome字段，支持欢迎消息的完整生命周期
- **默认消息配置**：DEFAULT_MESSAGES包含isWelcome: true的欢迎消息
- **状态保持**：欢迎消息状态在会话切换时得到保持
- **渲染控制**：isWelcome字段控制消息的渲染逻辑和布局处理

#### 会话初始化优化
- **默认欢迎消息**：DEFAULT_MESSAGES数组包含欢迎消息
- **历史消息检查**：会话恢复时检查历史消息是否为空
- **状态保持**：欢迎消息的状态在会话切换时得到保持
- **用户体验**：确保用户始终能看到欢迎消息，提供更好的引导体验
...
#### 附件上下文构建
- **元数据隐藏**：使用HTML注释隐藏附件元数据，用于前端恢复渲染附件UI
- **可读上下文**：为每种节点类型生成对应的可读上下文描述
- **AI感知**：构建的上下文让AI能够感知和理解附件内容
- **消息分隔**：使用特殊标记分隔附件上下文和用户消息
...
- **颜色对比度**：优化了前景色和背景色的对比度设置
- **过渡动画**：统一了过渡时间和缓动函数的变量定义
- **欢迎消息适配**：WelcomeMessage组件使用CSS变量适配主题色
...
- **欢迎消息认证**：WelcomeMessage组件使用AuthContext获取用户信息

#### 优化的消息渲染逻辑
- **等待动画移除**：移除了不必要的等待动画，仅在用户发送消息后、AI还未开始回复时显示
- **性能提升**：减少DOM操作，提升渲染性能
- **用户体验**：提供更流畅的交互体验
- **欢迎消息特殊处理**：当message.isWelcome为true时，显示WelcomeMessage组件
...
#### 上下文压缩数据库模型
- **compressed_summary字段**：存储压缩后的对话摘要
- **compressed_before_id字段**：记录被压缩的消息数量，支持压缩状态追踪
- **会话级压缩状态**：每个会话维护独立的压缩状态，支持压缩历史追踪
- **压缩统计持久化**：压缩摘要和统计信息保存到数据库，支持会话恢复
