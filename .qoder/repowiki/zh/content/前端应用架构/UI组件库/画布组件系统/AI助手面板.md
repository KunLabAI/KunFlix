...
## 更新摘要
**变更内容**
- 新增上下文使用可视化系统（ContextUsageBar）
- 改进拖拽定位功能，使用Framer Motion的dragControls
- 增强尺寸调整功能，支持八向调整（四边+四角）
- 新增图像编辑上下文横幅显示
- 移除ESC关闭面板功能
- 优化SSE事件处理中的上下文使用统计更新
...
- 上下文使用可视化系统：ContextUsageBar 提供的上下文使用统计
...
- MessageInput：输入组件，支持 Enter 发送、Shift+Enter 换行、发送状态禁用、自动聚焦，集成了上下文使用可视化
...
**更新** 改进了拖拽定位功能，使用 Framer Motion 的 dragControls 提供更流畅的拖拽体验，并移除了 ESC 关闭功能
...
- 上下文使用可视化：集成 ContextUsageBar，提供上下文使用统计的电池图标显示
- 附加：预留附件按钮（当前为占位）

**更新** 集成了上下文使用可视化系统，通过 ContextBatteryIcon 组件提供上下文使用情况的实时显示
...
**新增** 专门的上下文使用可视化组件，提供直观的上下文使用统计展示
...
class ContextBatteryIcon {
+contextUsage : ContextUsage
+isLoading : boolean
+render() : JSX.Element
}
ContextUsageBar --> ContextUsage : "显示统计"
ContextBatteryIcon --> ContextUsage : "显示统计"
```

**图表来源**
- [ContextUsageBar.tsx:23-141](file://frontend/src/components/ai-assistant/ContextUsageBar.tsx#L23-L141)
- [MessageInput.tsx:220-334](file://frontend/src/components/ai-assistant/MessageInput.tsx#L220-L334)
...
**更新** 拖拽功能现在使用 Framer Motion 的 dragControls，提供更流畅的拖拽体验
...
**更新** 新增了 ContextUsage 数据模型，用于存储上下文使用统计信息
...
**更新** 新增了 ContextUsageBar 组件的依赖关系，以及 Framer Motion 的 dragControls 功能
...
## 结论
AI助手面板通过清晰的分层架构与完善的Hook体系，实现了从会话管理、实时流式传输到消息渲染与多智能体协作的完整闭环。新增的上下文使用可视化系统提供了直观的上下文使用统计展示，增强了用户体验。改进的拖拽定位功能和八向尺寸调整功能进一步提升了面板的易用性和灵活性。其可扩展的状态模型与事件驱动的SSE处理，为后续自定义消息类型与交互行为提供了良好基础。
...
- 上下文使用可视化系统
  ...
  - 参考路径：[useSSEHandler.ts:260-264](file://frontend/src/components/ai-assistant/hooks/useSSEHandler.ts#L260-L264)，[AIAssistantPanel.tsx:325-327](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L325-L327)，[MessageInput.tsx:185-191](file://frontend/src/components/ai-assistant/MessageInput.tsx#L185-L191)
- 网络与鉴权
...
#### 上下文使用可视化系统
- **ContextUsageBar 组件**：提供四格电池图标的上下文使用统计，支持鼠标悬停查看详情
- **ContextBatteryIcon 组件**：在 MessageInput 中提供的电池图标，带有消耗动画效果
- **实时更新机制**：在 SSE 事件中自动更新上下文使用统计，提供实时的视觉反馈

#### 改进的拖拽与尺寸调整功能
- **Framer Motion 集成**：使用 dragControls 提供更流畅的拖拽体验
- **八向调整**：支持左、右、上、下四边以及四个角落的尺寸调整
- **约束容器**：使用 constraintsRef 确保面板不会拖出可视区域

#### 图像编辑上下文横幅
- **视觉反馈**：当从画布节点触发 AI 编辑时，显示当前编辑的节点信息
- **快速关闭**：提供 X 按钮快速清除图像编辑上下文
- **上下文感知**：在发送消息时自动包含图像编辑上下文信息

**章节来源**
- [ContextUsageBar.tsx:23-141](file://frontend/src/components/ai-assistant/ContextUsageBar.tsx#L23-L141)
- [AIAssistantPanel.tsx:177-218](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L177-L218)
- [AIAssistantPanel.tsx:300-316](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L300-L316)
