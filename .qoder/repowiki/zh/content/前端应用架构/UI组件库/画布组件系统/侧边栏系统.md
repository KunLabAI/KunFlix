...
- [ResourcesPage.tsx](file://frontend/src/app/resources/page.tsx)
...
## 项目结构
侧边栏系统位于前端 canvas 子系统内，围绕 ReactFlow 画布构建，通过 Zustand 状态管理与自定义 Hook 协同工作。现已集成完整的音乐资源标签系统，支持音频文件的上传、管理和拖拽添加。
...
subgraph "资源管理系统"
ResourcesPage["ResourcesPage.tsx<br/>资源管理页面"]
AssetCard["AssetCard.tsx<br/>资源卡片组件"]
AssetPreview["AssetPreviewDialog.tsx<br/>资源预览对话框"]
UploadZone["UploadZone.tsx<br/>上传区域"]
ResourceStore["useResourceStore.ts<br/>资源状态管理"]
ResourceApi["resourceApi.ts<br/>资源API接口"]
end
subgraph "状态层"
...
Page --> AI
Sidebar --> Store
Sidebar --> ResourceStore
QuickAdd --> Store
TopBar --> Store
Zoom --> Store
ReactFlow --> Store
ResourcesPage --> ResourceStore
ResourceStore --> ResourceApi
AssetCard --> ResourceStore
AssetPreview --> ResourceStore
UploadZone --> ResourceStore
```
...
- [ResourcesPage.tsx:1-189](file://frontend/src/app/resources/page.tsx#L1-L189)
- [useResourceStore.ts:1-169](file://frontend/src/store/useResourceStore.ts#L1-L169)
- [resourceApi.ts:1-109](file://frontend/src/lib/resourceApi.ts#L1-L109)

## 核心组件
- Sidebar：左侧固定悬浮工具条，包含"节点库"和"资源库"两个子面板，支持拖拽添加节点与资源。现已集成音乐资源标签，支持音频文件的直接拖拽和播放。
...
- ResourcesPage：独立的资源管理页面，提供完整的资源库浏览、上传、编辑和删除功能。
- AssetCard：资源卡片组件，支持图片、视频、音频三种类型的预览和操作。
- UploadZone：上传区域组件，支持拖拽上传和批量文件处理。

**章节来源**
- [Sidebar.tsx:52-341](file://frontend/src/components/canvas/Sidebar.tsx#L52-L341)
- [ResourcesPage.tsx:22-27](file://frontend/src/app/resources/page.tsx#L22-L27)
- [AssetCard.tsx:12-69](file://frontend/src/components/resources/AssetCard.tsx#L12-L69)
- [UploadZone.tsx:15-22](file://frontend/src/components/resources/UploadZone.tsx#L15-L22)

## 架构总览
侧边栏系统采用"组件-状态-事件"的分层架构，现已扩展支持完整的音乐资源生态系统：
- 组件层：Sidebar、QuickAddMenu、TopBar、ZoomControls、AIAssistantPanel 各司其职，通过 ReactFlow 的 onConnect/onDrop 等事件驱动状态变更。
- 资源管理层：ResourcesPage、AssetCard、UploadZone 等组件提供完整的资源管理功能。
- 状态层：useCanvasStore 提供节点、边、历史、同步、吸附等全局状态；useResourceStore 管理资源列表、上传队列和文件类型过滤。
...
QM->>PA : addNode(新节点)+onConnect(自动连接)
PA-->>PA : 更新 nodes/edges 并 takeSnapshot
```

**图表来源**
- [page.tsx:467-505](file://frontend/src/app/theater/[id]/page.tsx#L467-L505)
...
- [useResourceStore.ts:49-99](file://frontend/src/store/useResourceStore.ts#L49-L99)
...
**图表来源**
- [Sidebar.tsx:95-118](file://frontend/src/components/canvas/Sidebar.tsx#L95-L118)
...
### 音乐资源标签系统
- **新增功能**：Sidebar 中新增音乐标签页，支持音频文件的分类管理和拖拽添加。
- 资源分类：通过 `file_type === 'audio'` 过滤音频资源，支持多种音频格式（MP3、WAV、OGG、FLAC、AAC、M4A）。
- 拖拽行为：音频资源拖拽时创建文本节点，内容为音频文件URL，同时添加 'audio' 标签。
- 预览控制：每个音频资源卡片包含内置的HTML5音频播放器，支持播放控制和音量调节。
- 文件类型检测：在画布页面中实现完整的音频文件类型识别和处理逻辑。

**章节来源**
- [Sidebar.tsx:53-57](file://frontend/src/components/canvas/Sidebar.tsx#L53-L57)
- [Sidebar.tsx:296-321](file://frontend/src/components/canvas/Sidebar.tsx#L296-L321)
- [page.tsx:277-283](file://frontend/src/app/theater/[id]/page.tsx#L277-L283)
- [page.tsx:467-505](file://frontend/src/app/theater/[id]/page.tsx#L467-L505)

### ResourcesPage 资源管理页面
- **新增页面**：独立的资源管理界面，提供完整的资源库浏览和管理功能。
- 标签导航：包含"全部"、"图片"、"视频"、"音频"四个标签页，支持按类型筛选。
- 视图模式：支持网格和列表两种视图模式，满足不同使用场景。
- 上传功能：集成 UploadZone 组件，支持拖拽上传和批量文件处理。
- 资源操作：提供预览、重命名、替换文件、删除等完整操作。

**章节来源**
- [ResourcesPage.tsx:22-27](file://frontend/src/app/resources/page.tsx#L22-L27)
- [ResourcesPage.tsx:87-103](file://frontend/src/app/resources/page.tsx#L87-L103)
- [ResourcesPage.tsx:135-152](file://frontend/src/app/resources/page.tsx#L135-L152)

### AssetCard 资源卡片组件
- **新增组件**：支持三种文件类型的预览渲染器映射表。
- 预览渲染：图片使用 ImagePreview，视频使用 VideoPreview，音频使用 AudioPreview。
- 操作菜单：提供重命名、替换文件、删除等操作选项。
- 文件信息：显示文件名、大小等基本信息，支持悬停显示完整信息。

**章节来源**
- [AssetCard.tsx:12-69](file://frontend/src/components/resources/AssetCard.tsx#L12-L69)
- [AssetCard.tsx:48-55](file://frontend/src/components/resources/AssetCard.tsx#L48-L55)

### UploadZone 上传区域组件
- **增强功能**：支持音频文件上传，设置100MB大小限制。
- 类型映射：通过 MIME_TYPE_MAP 实现文件类型推导，支持 image/video/audio 三种类型。
- 上传队列：显示上传进度，支持取消上传操作。
- 错误处理：提供文件大小超限的错误提示和处理。

**章节来源**
- [UploadZone.tsx:15-22](file://frontend/src/components/resources/UploadZone.tsx#L15-L22)
- [UploadZone.tsx:24-31](file://frontend/src/components/resources/UploadZone.tsx#L24-L31)
- [UploadZone.tsx:43-56](file://frontend/src/components/resources/UploadZone.tsx#L43-L56)
...
## 依赖关系分析
- 组件依赖：
  - Sidebar 依赖 useCanvasStore 的 nodes 与拖拽事件处理，现依赖 useResourceStore 管理资源。
...
- [ResourcesPage.tsx:10](file://frontend/src/app/resources/page.tsx#L10)
- [useResourceStore.ts:1-4](file://frontend/src/store/useResourceStore.ts#L1-L4)
...
- **新增** 音频资源优化：音频文件采用预加载none策略，避免自动播放影响用户体验。
...
- **新增** 音频文件上传：检查文件大小限制（100MB）、文件格式支持和网络连接状态。
...
## 结论
侧边栏系统通过清晰的职责划分与状态集中管理，实现了高效、直观的画布操作体验。Sidebar 提供节点与资源的即取即用入口，现已集成完整的音乐资源标签系统，支持音频文件的上传、管理和拖拽添加。QuickAddMenu 优化了连接后的快速建模路径，TopBar/ZoomControls 提升了视图与操作效率，AIAssistantPanel 则拓展了创作辅助能力。新增的资源管理页面提供了完整的资源库功能，包括独立的资源浏览、上传和编辑操作。整体架构具备良好的可扩展性，便于后续集成更多工具面板与功能模块。
...
### 音频文件支持集成指南
- **新增文件类型**：在 FILE_TYPE_MATCHERS 中添加音频文件类型映射，支持多种音频格式。
- **文件大小限制**：设置100MB的音频文件上传限制，避免过大文件影响性能。
- **拖拽行为**：在 DRAG_DATA_BUILDERS 中为音频文件配置拖拽行为，创建文本节点并添加音频标签。
- **预览组件**：在 AssetCard 和 AssetPreviewDialog 中添加音频预览渲染器。
- **上传队列**：在 UploadZone 中为音频文件设置合适的大小限制和错误提示。
...
- **新增** 音频播放：为音频资源提供内置播放器，支持播放控制和音量调节，提升用户体验。