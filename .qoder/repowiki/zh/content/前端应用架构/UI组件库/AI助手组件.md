...
## 更新摘要
**变更内容**
- MessageInput组件移除了键盘快捷键指示器，简化了UI设计，提升了组件性能
- ChatMessage组件的图片过滤机制得到改进，移除了浮动跳跃的三点加载动画，优化了加载状态显示逻辑
- TypewriterText组件移除了光标动画效果，减少了不必要的DOM操作和重绘
- PanelHeader组件新增HeaderContextBattery组件，提供更丰富的上下文使用统计功能
- ContextUsageBar组件仍保留Framer Motion动画，但MessageInput不再使用动画效果
- 整体提升了组件的性能和用户体验
...
- 输入组件：支持回车发送、Shift+Enter 换行、禁用态与加载态反馈，**已移除键盘快捷键指示器，简化UI设计**
