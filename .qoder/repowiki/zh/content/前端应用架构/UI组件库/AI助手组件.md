...
- Store：集中管理消息、会话、Agent、面板尺寸位置、画布关联等状态，并持久化
...
## 依赖关系分析
- 组件依赖：面板依赖消息、输入、Hook 与 Store；消息组件依赖指示器与打字机文本
