...
- [canvas_tools SKILL.md（内置）](file://backend/skills/builtin_skills/canvas_tools/SKILL.md)
- [image_tools SKILL.md（内置）](file://backend/skills/builtin_skills/image_tools/SKILL.md)
- [video_tools SKILL.md（内置）](file://backend/skills/builtin_skills/video_tools/SKILL.md)
- [file_reader SKILL.md（内置）](file://backend/skills/builtin_skills/file_reader/SKILL.md)
- [file_reader 脚本 read.py](file://backend/skills/builtin_skills/file_reader/scripts/read.py)
- [canvas_tools SKILL.md（激活）](file://backend/skills/active_skills/canvas_tools/SKILL.md)
...
- 可选 scripts 目录存放可被技能调用的脚本（如本地文件读取）
...
- 大文件读取：内置 file_reader 支持分段读取，避免一次性读取超大文件
...
- [file_reader SKILL.md（内置）:24-32](file://backend/skills/builtin_skills/file_reader/SKILL.md#L24-L32)
...
## 结论
本技能系统通过“配置即能力”的方式，将复杂任务封装为可复用的教程式技能，结合按需加载与工具门控机制，在保证安全性的同时最大化灵活性与性能。遵循本文档的规范与流程，即可高效地创建、维护与扩展各类技能。

## 附录：开发流程与示例

### 一、技能架构设计原则
- 单一职责：每个技能聚焦一类任务域（如图像生成、视频编辑、文件读取）
...
章节来源
- [canvas_tools SKILL.md（内置）:1-6](file://backend/skills/builtin_skills/canvas_tools/SKILL.md#L1-L6)
- [image_tools SKILL.md（内置）:1-6](file://backend/skills/builtin_skills/image_tools/SKILL.md#L1-L6)
- [video_tools SKILL.md（内置）:1-6](file://backend/skills/builtin_skills/video_tools/SKILL.md#L1-L6)
- [file_reader SKILL.md（内置）:1-6](file://backend/skills/builtin_skills/file_reader/SKILL.md#L1-L6)
...
章节来源
- [skills_manager.py:370-408](file://backend/skills_manager.py#L370-L408)
- [file_reader 脚本 read.py:1-21](file://backend/skills/builtin_skills/file_reader/scripts/read.py#L1-L21)
...
- 参考文件：[image_tools SKILL.md（内置）:1-81](file://backend/skills/builtin_skills/image_tools/SKILL.md#L1-L81)

章节来源
- [image_tools SKILL.md（内置）:15-81](file://backend/skills/builtin_skills/image_tools/SKILL.md#L15-L81)

#### 示例二：视频编辑技能
- 目标：提供 generate_video 与 edit_video 工具的使用说明与异步流程
- 关键点：异步任务、模式区分（编辑/扩展）、参数枚举与质量选项
- 参考文件：[video_tools SKILL.md（内置）:1-104](file://backend/skills/builtin_skills/video_tools/SKILL.md#L1-L104)

章节来源
- [video_tools SKILL.md（内置）:17-104](file://backend/skills/builtin_skills/video_tools/SKILL.md#L17-L104)

#### 示例三：文本处理技能（文件读取）
- 目标：提供 list_directory 与 read_file 的使用说明与大文件分段读取
- 关键点：支持多种文本格式、大文件分段、安全读取
- 参考文件：[file_reader SKILL.md（内置）:1-48](file://backend/skills/builtin_skills/file_reader/SKILL.md#L1-L48)

章节来源
- [file_reader SKILL.md（内置）:16-48](file://backend/skills/builtin_skills/file_reader/SKILL.md#L16-L48)
