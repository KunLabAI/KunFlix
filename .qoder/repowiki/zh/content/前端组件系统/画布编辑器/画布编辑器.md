...
## 更新摘要
**所做更改**
- 新增音频节点类型支持，扩展了 nodeTypes 配置
- 新增 AudioNode 组件，支持音频文件上传、播放和管理
- 扩展 Sidebar 资源库，支持音频资源的拖拽和预览
- 更新节点类型定义，新增 AudioNodeData 类型
- 新增音频节点的默认尺寸和默认数据配置
- 新增音频文件格式支持（mp3、wav、ogg、flac、aac、m4a）
...
- 节点体系：CharacterNode、ScriptNode、StoryboardNode、VideoNode、AudioNode的实现细节与交互行为
...
- 节点组件：CharacterNode、ScriptNode、StoryboardNode、VideoNode、AudioNode分别实现不同媒体类型的节点编辑与交互。
- 自定义边 CustomEdge：贝塞尔曲线路径、悬停高亮、删除按钮与宽轨道增强交互。
- 工具栏 NodeToolbar：统一的节点工具条，支持分组与危险操作分离。
- 侧边栏 Sidebar：提供节点库与资源库拖拽，支持音频资源的拖拽和预览。
...
**图表来源**
- [Sidebar.tsx:114-144](file://frontend/src/components/canvas/Sidebar.tsx#L114-L144)
...
- AudioNodeData：名称、描述、音频URL、上传状态。
- CanvasNode：统一节点类型，联合上述五种数据类型。

**章节来源**
- [useCanvasStore.ts:26-68](file://frontend/src/store/useCanvasStore.ts#L26-L68)
...
### AudioNode 音频节点
**新增** 音频节点支持音频文件的上传、播放和管理

- 交互特性：
  - 支持多种音频格式：mp3、wav、ogg、flac、aac、m4a
  - 100MB大小限制，带进度条显示上传状态
  - 音频播放控件，支持播放、暂停、音量调节
  - 工具栏支持复制节点、删除节点
  - 标题双击编辑，支持键盘快捷键（Enter、Escape）
- 上传流程：
  - 文件类型验证：仅支持指定音频格式
  - 文件大小验证：不超过100MB
  - 本地预览：上传前显示临时URL
  - 后端上传：通过XMLHttpRequest发送FormData
  - 进度跟踪：实时显示上传百分比
  - 错误处理：显示错误信息并允许重试
- 事件与状态：
  - uploading状态控制上传UI显示
  - uploadProgress实时更新上传进度
  - uploadError处理上传失败情况
  - audioUrl存储最终音频URL
...
ValidateFormat --> |不支持| ShowFormatError["显示格式错误"]
ValidateFormat --> |支持| ValidateSize["验证文件大小"]
ValidateSize --> |超限| ShowSizeError["显示大小错误"]
ValidateSize --> |通过| SetLocalPreview["设置本地预览URL"]
SetLocalPreview --> SendXHR["发送XMLHttpRequest"]
SendXHR --> Progress["更新上传进度"]
Progress --> Response{"响应状态"}
Response --> |成功| UpdateNode["更新节点数据"]
Response --> |失败| ShowUploadError["显示上传错误"]
UpdateNode --> RevokeURL["撤销Object URL"]
...
**图表来源**
- [useCanvasStore.ts:76-124](file://frontend/src/store/useCanvasStore.ts#L76-L124)
- [useCanvasStore.ts:188-554](file://frontend/src/store/useCanvasStore.ts#L188-L554)
...
- 组件耦合：
  ...
  - AudioNode新增对useResourceStore的依赖，用于同步上传的音频资源。
- 外部依赖：
...
- 音频上传问题：
  - 确认文件格式在支持列表中（mp3、wav、ogg、flac、aac、m4a）。
  - 检查文件大小是否超过100MB限制。
  - 验证XMLHttpRequest配置和响应处理。
  - 确认useResourceStore的syncAssetFromUpload方法正常工作。
...
## 结论
本画布编辑器通过React Flow与Zustand实现了高扩展性的可视化编辑平台。节点类型丰富、交互完善、状态管理清晰。新增的音频节点功能进一步丰富了多媒体创作能力，支持多种音频格式的上传和播放。Sidebar与ZoomControls提供了良好的创作入口与视图控制体验。建议后续在节点预览卡与拖拽到AI能力上进一步完善，以提升创作效率与智能化水平。
