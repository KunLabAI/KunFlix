...
**更新** 本次更新反映了应用变更：AI助手面板实现完整的停止生动生成功能，包括handleStop方法、AbortController集成、UI状态重置等。

## 项目结构
AI助手面板位于前端工程的画布模块中，采用"容器组件 + 子组件 + Hooks + Store"的分层组织方式：
- 容器组件：AIAssistantPanel.tsx 负责面板生命周期、拖拽/尺寸调整、SSE流式处理、会话管理、状态同步、**新增**生成中断控制等
...
**图表来源**
- [AIAssistantPanel.tsx:1-660](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L1-L660)
...
- [MessageInput.tsx:1-750](file://frontend/src/components/ai-assistant/MessageInput.tsx#L1-L750)
...
- [useSSEHandler.ts:1-404](file://frontend/src/components/ai-assistant/hooks/useSSEHandler.ts#L1-L404)
...
## 核心组件
- 面板容器：负责面板显隐、拖拽、尺寸调整、SSE流式接收、会话初始化、错误处理与登录过期弹窗、**新增**AI生成中断控制
- 头部组件：提供清空会话、关闭面板、拖拽抓手、上下文使用率指示器（含悬浮详情）
- 消息列表：虚拟滚动实现，支持自动滚动、手动滚动检测、等待指示
- 回到最新按钮：平滑滚动到底部，支持"新消息"高亮
- 输入区：支持多行自适应高度、Enter发送、Agent选择器、发送禁用态、**新增**停止生成按钮
...
**章节来源**
- [AIAssistantPanel.tsx:51-660](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L51-L660)
...
- [MessageInput.tsx:29-750](file://frontend/src/components/ai-assistant/MessageInput.tsx#L29-L750)
...
- [useSSEHandler.ts:25-404](file://frontend/src/components/ai-assistant/hooks/useSSEHandler.ts#L25-L404)
...
- 容器组件负责顶层控制流与副作用（拖拽、尺寸、键盘、SSE、会话、**新增**生成中断）
...
P->>P : handleStop中断生成
P->>Z : 更新状态并标记消息完成
P->>SSE : resetStreamingState重置状态
P->>Z : setIsLoading(false)
U->>P : 发送消息
P->>API : POST /api/chats/{sessionId}/messages
API-->>P : SSE事件流
P->>SSE : parseSSELine/handleSSEEvent
...
- [AIAssistantPanel.tsx:228-338](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L228-L338)
- [useSessionManager.ts:52-123](file://frontend/src/components/ai-assistant/hooks/useSessionManager.ts#L52-L123)
- [useSSEHandler.ts:56-404](file://frontend/src/components/ai-assistant/hooks/useSSEHandler.ts#L56-L404)
...
- **新增** 生成中断控制：使用AbortController管理SSE流，支持用户主动中断AI生成
...
SSE --> Update["更新状态与消息"]
Update --> Stop{"点击停止生成？"}
Stop --> |是| Interrupt["handleStop中断生成"]
Interrupt --> Reset["重置流式状态"]
Reset --> MarkComplete["标记最后一条消息为完成"]
MarkComplete --> End(["完成"])
Stop --> |否| End
Minimize --> End
```
...
- **新增** 停止生成按钮：AI生成期间显示红色停止按钮，点击触发中断
...
- [MessageInput.tsx:705-715](file://frontend/src/components/ai-assistant/MessageInput.tsx#L705-L715)
...
- **新增** 生成中断：AbortController提供即时中断能力，避免长时间等待
...
- **新增** 生成中断问题
  - 现象：点击停止生成按钮无效
  - 处理：检查AbortController实例是否正确创建和销毁，确认handleStop方法被正确传入MessageInput
  - 相关：AIAssistantPanel的handleStop实现与MessageInput的onStop回调
- 性能问题
  - 现象：滚动卡顿、动画掉帧
...
- [AIAssistantPanel.tsx:213-226](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L213-L226)
- [useSSEHandler.ts:374-404](file://frontend/src/components/ai-assistant/hooks/useSSEHandler.ts#L374-L404)
- [usePerformanceMonitor.ts:75-200](file://frontend/src/components/ai-assistant/hooks/usePerformanceMonitor.ts#L75-L200)
...
- **新增** 停止生成功能提供更好的用户体验，支持用户主动中断长时间生成
- 全面的性能监控与错误处理机制
...
- **新增** 生成中断：AI生成期间显示停止按钮，用户可随时中断生成
...
- **新增** 生成状态：AbortController管理SSE流，支持中断与清理

**章节来源**
- [useAIAssistantStore.ts:104-381](file://frontend/src/store/useAIAssistantStore.ts#L104-L381)
- [useSessionManager.ts:235-277](file://frontend/src/components/ai-assistant/hooks/useSessionManager.ts#L235-L277)
- [AIAssistantPanel.tsx:132-132](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L132-L132)
...
- **新增** 生成中断：确保MessageInput正确接收onStop回调
...
- **新增** 停止按钮样式：红色destructive样式突出中断功能
...
- **新增** 生成中断优化：使用AbortController避免长时间等待，提升用户体验
...
### 生成中断实现细节
- **新增** AbortController管理：在handleSend中创建新的AbortController实例，用于中断正在进行的SSE流
- **新增** handleStop方法：调用abortControllerRef.current.abort()中断请求，resetStreamingState重置状态，标记最后一条流式消息为完成
- **新增** UI集成：MessageInput组件在isLoading为true时显示红色停止按钮，点击时调用onStop回调
- **新增** 状态同步：中断后立即更新面板状态，确保用户界面与实际状态一致

**章节来源**
- [AIAssistantPanel.tsx:213-226](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L213-L226)
- [AIAssistantPanel.tsx:250-252](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L250-L252)
- [MessageInput.tsx:705-715](file://frontend/src/components/ai-assistant/MessageInput.tsx#L705-L715)
- [useSSEHandler.ts:45-57](file://frontend/src/components/ai-assistant/hooks/useSSEHandler.ts#L45-L57)