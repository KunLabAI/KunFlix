...
- 子组件：PanelHeader、VirtualMessageList、ScrollToBottomButton、MessageInput、NodePreviewCard、WelcomeMessage 等
...
subgraph "Hooks"
H["useSessionManager.ts"]
I["useSSEHandler.ts"]
J["usePerformanceMonitor.ts"]
end
subgraph "状态与网络"
K["useAIAssistantStore.ts"]
L["api.ts"]
end
A --> B
A --> C
...
## 依赖关系分析
- 组件耦合
  - AIAssistantPanel 作为根容器，依赖 PanelHeader、VirtualMessageList、ScrollToBottomButton、MessageInput、NodePreviewCard、WelcomeMessage
...
- 外部依赖
  ...
  - lucide-react：图标
- 状态依赖
  - useAIAssistantStore 提供消息、会话、面板尺寸/位置、上下文使用统计、虚拟滚动参数等
...
A --> F["NodePreviewCard"]
A --> G["WelcomeMessage"]
A --> H["useSessionManager"]
A --> I["useSSEHandler"]
A --> J["usePerformanceMonitor"]
A --> K["useAIAssistantStore"]
A --> L["api"]
```
