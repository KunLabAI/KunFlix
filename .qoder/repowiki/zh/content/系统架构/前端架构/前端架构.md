# 前端架构

<cite>
**本文档引用的文件**
- [frontend/package.json](file://frontend/package.json)
- [frontend/next.config.ts](file://frontend/next.config.ts)
- [frontend/tsconfig.json](file://frontend/tsconfig.json)
- [frontend/tailwind.config.ts](file://frontend/tailwind.config.ts)
- [frontend/src/app/layout.tsx](file://frontend/src/app/layout.tsx)
- [frontend/src/context/AuthContext.tsx](file://frontend/src/context/AuthContext.tsx)
- [frontend/src/context/ThemeContext.tsx](file://frontend/src/context/ThemeContext.tsx)
- [frontend/src/store/useCanvasStore.ts](file://frontend/src/store/useCanvasStore.ts)
- [frontend/src/store/useAIAssistantStore.ts](file://frontend/src/store/useAIAssistantStore.ts)
- [frontend/src/lib/api.ts](file://frontend/src/lib/api.ts)
- [frontend/src/lib/theaterApi.ts](file://frontend/src/lib/theaterApi.ts)
- [frontend/src/app/page.tsx](file://frontend/src/app/page.tsx)
- [frontend/src/components/ui/button.tsx](file://frontend/src/components/ui/button.tsx)
- [frontend/src/components/canvas/AIAssistantPanel.tsx](file://frontend/src/components/canvas/AIAssistantPanel.tsx)
- [frontend/src/hooks/use-tiptap-editor.ts](file://frontend/src/hooks/use-tiptap-editor.ts)
- [frontend/src/i18n/I18nProvider.tsx](file://frontend/src/i18n/I18nProvider.tsx)
- [frontend/src/i18n/index.ts](file://frontend/src/i18n/index.ts)
- [frontend/src/i18n/locales/en-US.json](file://frontend/src/i18n/locales/en-US.json)
- [frontend/src/i18n/locales/zh-CN.json](file://frontend/src/i18n/locales/zh-CN.json)
- [frontend/src/components/LanguageSwitcher.tsx](file://frontend/src/components/LanguageSwitcher.tsx)
- [frontend/src/components/home/TopBar.tsx](file://frontend/src/components/home/TopBar.tsx)
- [frontend/src/components/home/TheaterCard.tsx](file://frontend/src/components/home/TheaterCard.tsx)
</cite>

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 引言
本文件面向 Infinite Game 前端团队与技术读者，系统性梳理基于 Next.js 16 的应用架构，涵盖 App Router 模式、页面路由体系、组件层次结构、状态管理策略（Zustand 与 React Context）、全局状态设计、UI 组件库与主题系统、**国际化支持**、响应式布局、性能优化与构建配置等。文档旨在帮助开发者快速理解系统设计、高效扩展功能并保障运行质量。

## 项目结构
前端采用 Next.js 16 App Router 结构，根目录为 frontend，核心组织方式如下：
- 应用入口与全局布局：src/app 下的 layout.tsx 提供根布局与全局 Provider 注入（Ant Design Registry、认证与主题 Provider、**国际化 Provider**）。
- 页面与路由：src/app 下按路径组织页面，如 /、/login、/resources、/simple、/theater/[id] 等。
- 组件体系：src/components 下按功能域划分，如 canvas（画布与 AI 助手）、home（首页）、ai-assistant（AI 助手子组件）、ui（通用 UI 组件）、**LanguageSwitcher（语言切换器）**等。
- 状态管理：src/store 下使用 Zustand 管理画布与 AI 助手状态，并持久化至 localStorage。
- 上下文与工具：src/context 提供认证与主题上下文；src/lib 提供 API 封装与业务接口；src/hooks 提供自定义 Hook。
- 样式与主题：Tailwind 配置通过 CSS 变量桥接 Ant Design 主题；全局样式与变量集中管理。
- **国际化系统**：src/i18n 下包含 i18n 配置、语言包和国际化 Provider。

```mermaid
graph TB
subgraph "应用入口"
L["src/app/layout.tsx<br/>根布局与Provider注入"]
end
subgraph "页面路由"
P1["src/app/page.tsx<br/>首页"]
P2["src/app/login/page.tsx<br/>登录页"]
P3["src/app/theater/[id]/page.tsx<br/>剧场画布页"]
P4["src/app/resources/page.tsx<br/>资源页"]
P5["src/app/simple/page.tsx<br/>简单示例页"]
end
subgraph "组件层"
C1["src/components/canvas/AIAssistantPanel.tsx<br/>AI助手面板"]
C2["src/components/home/*<br/>首页组件"]
C3["src/components/ai-assistant/*<br/>AI助手子组件"]
C4["src/components/ui/*<br/>通用UI组件"]
C5["src/components/LanguageSwitcher.tsx<br/>语言切换器"]
end
subgraph "状态管理"
S1["src/store/useCanvasStore.ts<br/>画布状态(Zustand)"]
S2["src/store/useAIAssistantStore.ts<br/>AI助手状态(Zustand)"]
end
subgraph "国际化系统"
I1["src/i18n/index.ts<br/>i18n配置"]
I2["src/i18n/I18nProvider.tsx<br/>国际化Provider"]
I3["src/i18n/locales/zh-CN.json<br/>中文语言包"]
I4["src/i18n/locales/en-US.json<br/>英文语言包"]
end
subgraph "上下文与工具"
X1["src/context/AuthContext.tsx<br/>认证上下文"]
X2["src/context/ThemeContext.tsx<br/>主题上下文"]
U1["src/lib/api.ts<br/>Axios封装"]
U2["src/lib/theaterApi.ts<br/>剧场API"]
H1["src/hooks/use-tiptap-editor.ts<br/>TipTap编辑器Hook"]
end
L --> I2
L --> P1
L --> P2
L --> P3
L --> P4
L --> P5
P3 --> C1
C1 --> S2
C1 --> X1
C1 --> U1
C1 --> U2
P1 --> C2
C2 --> S1
C2 --> X1
C2 --> C5
C5 --> I2
C4 --> X2
S1 --> U2
S2 --> U1
H1 --> C3
I2 --> I1
I1 --> I3
I1 --> I4
```

**图表来源**
- [frontend/src/app/layout.tsx:1-45](file://frontend/src/app/layout.tsx#L1-L45)
- [frontend/src/i18n/I18nProvider.tsx:1-20](file://frontend/src/i18n/I18nProvider.tsx#L1-L20)
- [frontend/src/i18n/index.ts:1-28](file://frontend/src/i18n/index.ts#L1-L28)
- [frontend/src/components/LanguageSwitcher.tsx:1-79](file://frontend/src/components/LanguageSwitcher.tsx#L1-L79)

**章节来源**
- [frontend/src/app/layout.tsx:1-45](file://frontend/src/app/layout.tsx#L1-L45)
- [frontend/src/i18n/I18nProvider.tsx:1-20](file://frontend/src/i18n/I18nProvider.tsx#L1-L20)
- [frontend/src/i18n/index.ts:1-28](file://frontend/src/i18n/index.ts#L1-L28)

## 核心组件
- 根布局与全局 Provider：在根布局中注入 Ant Design Registry、认证 Provider、主题 Provider、**国际化 Provider**，确保全局样式、主题与国际化一致。
- 认证上下文：提供用户状态、登录/登出、令牌刷新与自动重试机制，统一拦截 401 并进行刷新队列处理。
- 主题上下文：支持明暗主题切换、系统偏好检测、Ant Design 主题算法切换与本地存储持久化。
- **国际化 Provider**：基于 react-i18next 和 i18next，提供客户端水合后的语言偏好恢复、语言切换与持久化。
- Zustand 状态管理：
  - 画布状态：管理节点/边/视口、历史快照、脏检查、与后端同步、保存与加载。
  - AI 助手状态：管理消息、会话、Agent 列表、面板尺寸与位置、附件与上下文使用统计。
- UI 组件库：基于 Radix UI 与 class-variance-authority 的 Button 等基础组件，配合 Tailwind 实现主题化样式。
- TipTap 编辑器：提供多页面编辑器状态与命令能力的 Hook，支撑富文本编辑场景。
- **语言切换器**：提供中英文切换功能，支持下拉菜单和动画效果。

**章节来源**
- [frontend/src/context/AuthContext.tsx:1-207](file://frontend/src/context/AuthContext.tsx#L1-L207)
- [frontend/src/context/ThemeContext.tsx:1-75](file://frontend/src/context/ThemeContext.tsx#L1-L75)
- [frontend/src/i18n/I18nProvider.tsx:1-20](file://frontend/src/i18n/I18nProvider.tsx#L1-L20)
- [frontend/src/i18n/index.ts:1-28](file://frontend/src/i18n/index.ts#L1-L28)
- [frontend/src/components/LanguageSwitcher.tsx:1-79](file://frontend/src/components/LanguageSwitcher.tsx#L1-L79)

## 架构总览
应用采用"布局-页面-组件-状态-上下文-工具-国际化"的分层架构：
- 布局层：RootLayout 注入 Provider，统一字体、注册 Ant Design、提供认证、主题与**国际化**环境。
- 页面层：App Router 路由到具体页面，页面组件组合业务组件与状态。
- 组件层：UI 组件库与业务组件（画布、AI 助手、首页、**语言切换器**等），组件间通过状态与上下文通信。
- 状态层：Zustand Store 管理局部状态与持久化；React Context 管理全局认证、主题与**国际化**。
- 工具层：API 封装、TipTap Hook、工具函数等。
- **国际化层**：i18n 配置、语言包与国际化 Provider，支持客户端水合与语言偏好持久化。

```mermaid
graph TB
RL["RootLayout<br/>Provider注入"] --> I18N["I18nProvider<br/>国际化"]
RL --> AP["App Router<br/>页面路由"]
AP --> PAGE_HOME["首页页面"]
AP --> PAGE_THEATER["剧场页面"]
PAGE_THEATER --> CANVAS["AIAssistantPanel<br/>画布与AI助手"]
CANVAS --> STORE_AI["useAIAssistantStore"]
CANVAS --> STORE_CANVAS["useCanvasStore"]
CANVAS --> CTX_AUTH["AuthContext"]
CANVAS --> CTX_THEME["ThemeContext"]
CANVAS --> CTX_I18N["I18nContext"]
CANVAS --> API["API封装(Axios/SSE)"]
CANVAS --> THEATER_API["theaterApi"]
UI["UI组件(Button等)"] --> CTX_THEME
UI --> CTX_I18N
LANG_SWITCH["LanguageSwitcher<br/>语言切换器"] --> CTX_I18N
```

**图表来源**
- [frontend/src/app/layout.tsx:34-39](file://frontend/src/app/layout.tsx#L34-L39)
- [frontend/src/i18n/I18nProvider.tsx:1-20](file://frontend/src/i18n/I18nProvider.tsx#L1-L20)
- [frontend/src/components/LanguageSwitcher.tsx:13-31](file://frontend/src/components/LanguageSwitcher.tsx#L13-L31)

## 详细组件分析

### 国际化系统与语言切换
国际化系统采用 react-i18next + i18next 架构，提供完整的多语言支持：

**I18nProvider 组件**：
- 客户端水合后从 localStorage 恢复用户语言偏好，避免 SSR 与 CSR 之间的水合不匹配问题
- 使用 useEffect 在组件挂载后检查并应用保存的语言设置
- 通过 I18nextProvider 将 i18n 实例传递给整个应用

**i18n 配置**：
- 支持 zh-CN 和 en-US 两种语言
- 默认语言为 zh-CN，回退语言也为 zh-CN
- 语言切换时自动持久化到 localStorage
- 使用 escapeValue: false 支持 HTML 内容

**语言包结构**：
- 包含导航、用户菜单、搜索、主题、资源、剧场、画布、侧边栏、AI 助手等完整翻译
- 支持占位符替换和复数形式处理
- 提供中英文双语支持

**LanguageSwitcher 组件**：
- 提供下拉菜单语言切换界面
- 支持动画效果和点击外部关闭
- 实时显示当前语言状态
- 与 i18n 实例集成，即时切换语言

```mermaid
sequenceDiagram
participant User as "用户"
participant LangSwitcher as "LanguageSwitcher"
participant I18n as "i18n实例"
participant LocalStorage as "localStorage"
participant I18nProvider as "I18nProvider"
participant Components as "应用组件"
User->>LangSwitcher : 点击语言切换按钮
LangSwitcher->>I18n : changeLanguage(code)
I18n->>LocalStorage : 存储语言偏好
I18n->>I18nProvider : 触发languageChanged事件
I18nProvider->>Components : 重新渲染使用t()的组件
Components->>User : 显示新的语言内容
```

**图表来源**
- [frontend/src/components/LanguageSwitcher.tsx:28-31](file://frontend/src/components/LanguageSwitcher.tsx#L28-L31)
- [frontend/src/i18n/index.ts:23-25](file://frontend/src/i18n/index.ts#L23-L25)
- [frontend/src/i18n/I18nProvider.tsx:13-16](file://frontend/src/i18n/I18nProvider.tsx#L13-L16)

**章节来源**
- [frontend/src/i18n/I18nProvider.tsx:1-20](file://frontend/src/i18n/I18nProvider.tsx#L1-L20)
- [frontend/src/i18n/index.ts:1-28](file://frontend/src/i18n/index.ts#L1-L28)
- [frontend/src/i18n/locales/en-US.json:1-207](file://frontend/src/i18n/locales/en-US.json#L1-L207)
- [frontend/src/i18n/locales/zh-CN.json:1-207](file://frontend/src/i18n/locales/zh-CN.json#L1-L207)
- [frontend/src/components/LanguageSwitcher.tsx:1-79](file://frontend/src/components/LanguageSwitcher.tsx#L1-L79)

### 认证上下文与自动刷新流程
认证上下文负责：
- 用户状态与登录/登出：写入/清除本地存储，更新全局状态并跳转路由。
- 自动刷新：拦截 401，使用刷新令牌请求新访问令牌，支持并发请求排队与重试。
- fetch 包装：提供带授权头的请求函数，内部处理刷新逻辑与队列。

```mermaid
sequenceDiagram
participant C as "组件"
participant F as "createAuthFetch"
participant A as "AuthContext"
participant API as "后端API"
participant LS as "localStorage"
C->>F : 发起受保护请求
F->>LS : 读取access_token
F->>API : 带Authorization头请求
API-->>F : 返回401
F->>A : 调用refreshToken()
A->>LS : 读取refresh_token
A->>API : POST /auth/refresh
API-->>A : 返回新access_token
A->>LS : 更新access_token
F->>API : 重试原请求(新token)
API-->>C : 返回成功响应
```

**图表来源**
- [frontend/src/context/AuthContext.tsx:52-114](file://frontend/src/context/AuthContext.tsx#L52-L114)
- [frontend/src/context/AuthContext.tsx:172-199](file://frontend/src/context/AuthContext.tsx#L172-L199)

**章节来源**
- [frontend/src/context/AuthContext.tsx:1-207](file://frontend/src/context/AuthContext.tsx#L1-L207)

### 主题系统与 Ant Design 集成
主题系统：
- 支持明/暗主题切换，优先使用本地存储，其次匹配系统偏好。
- 通过 documentElement 类名与属性设置主题，配合 Ant Design 的算法切换与 token 定制。
- Tailwind 通过 CSS 变量映射主题色值，实现 UI 组件与主题的一致性。

```mermaid
flowchart TD
Start(["初始化"]) --> CheckLocal["检查本地存储主题"]
CheckLocal --> HasLocal{"有本地主题?"}
HasLocal --> |是| ApplyLocal["应用本地主题"]
HasLocal --> |否| MatchSystem["匹配系统深色偏好"]
MatchSystem --> ApplySystem["应用系统主题"]
ApplyLocal --> Persist["写入本地存储"]
ApplySystem --> Persist
Persist --> End(["完成"])
```

**图表来源**
- [frontend/src/context/ThemeContext.tsx:16-41](file://frontend/src/context/ThemeContext.tsx#L16-L41)

**章节来源**
- [frontend/src/context/ThemeContext.tsx:1-75](file://frontend/src/context/ThemeContext.tsx#L1-L75)
- [frontend/tailwind.config.ts:1-64](file://frontend/tailwind.config.ts#L1-L64)

### Zustand 状态管理：画布与 AI 助手
- useCanvasStore：管理画布节点/边/视口、历史快照、脏检查、与后端同步与保存、网格吸附设置等。通过 persist 中间件将关键状态持久化至 localStorage，并在合并时去重节点。
- useAIAssistantStore：管理消息、会话、Agent 列表、面板尺寸与位置、附件与上下文使用统计。同样使用 persist，持久化所有剧场会话与当前状态。

```mermaid
classDiagram
class CanvasState {
+nodes : Node[]
+edges : Edge[]
+viewport : Viewport
+history : HistoryState[]
+historyIndex : number
+isSaving : boolean
+isLoading : boolean
+isDirty : boolean
+onNodesChange(changes)
+onEdgesChange(changes)
+onConnect(connection)
+addNode(node)
+deleteNode(id)
+deleteEdge(id)
+reset()
+updateNodeData(id, data)
+updateNodeDimensions(id, w, h)
+setViewport(viewport)
+takeSnapshot()
+undo()
+redo()
+setTheaterId(id)
+setTheaterTitle(title)
+loadTheater(id)
+syncTheater(id)
+saveToBackend()
+markDirty()
+snapToGrid : boolean
+snapToGuides : boolean
}
class AIAssistantState {
+isOpen : boolean
+currentTheaterId : string
+messages : Message[]
+sessionId : string
+agentId : string
+agentName : string
+availableAgents : AgentInfo[]
+theaterSessions : Map
+panelSize : Size
+panelPosition : Position
+imageEditContext : ImageEditContext
+nodeAttachments : NodeAttachment[]
+nodeAttachment : NodeAttachment
+isDragOverPanel : boolean
+contextUsage : ContextUsage
+scrollBehavior : ScrollBehavior
+overscanCount : number
+setIsOpen(isOpen)
+toggleOpen()
+switchTheater(id)
+setMessages(msgs)
+addMessage(msg)
+updateLastMessage(content, status)
+clearMessages()
+setSessionId(id)
+setAgentId(id)
+setAgentName(name)
+setCurrentAgent(id, name)
+clearSession()
+clearMessagesKeepSession()
+setAvailableAgents(agents)
+setPanelSize(size)
+resetPanelSize()
+setPanelPosition(pos)
+resetPanelPosition()
+setImageEditContext(ctx)
+clearImageEditContext()
+setNodeAttachments(attachments)
+addNodeAttachment(attachment)
+removeNodeAttachment(nodeId)
+clearNodeAttachments()
+setNodeAttachment(attachment)
+clearNodeAttachment()
+setIsDragOverPanel(flag)
+setContextUsage(usage)
+setScrollBehavior(behavior)
+setOverscanCount(count)
}
useCanvasStore --> CanvasState
useAIAssistantStore --> AIAssistantState
```

**图表来源**
- [frontend/src/store/useCanvasStore.ts:67-114](file://frontend/src/store/useCanvasStore.ts#L67-L114)
- [frontend/src/store/useAIAssistantStore.ts:104-200](file://frontend/src/store/useAIAssistantStore.ts#L104-L200)

**章节来源**
- [frontend/src/store/useCanvasStore.ts:1-540](file://frontend/src/store/useCanvasStore.ts#L1-L540)
- [frontend/src/store/useAIAssistantStore.ts:1-381](file://frontend/src/store/useAIAssistantStore.ts#L1-L381)

### UI 组件库与响应式布局
- Button 组件：基于 class-variance-authority 的变体与尺寸系统，结合 Tailwind 类名实现主题化按钮。
- 响应式与主题：通过 Tailwind CSS 变量映射 Ant Design 主题色，配合 darkMode class 控制明暗主题。
- TipTap 集成：use-tiptap-editor Hook 提供多页面编辑器状态与命令能力，便于在画布与富文本场景中复用。
- **国际化集成**：组件通过 useTranslation() 和 t() 函数获取翻译，支持动态语言切换。

```mermaid
graph LR
THEME["ThemeContext<br/>主题算法/Token"] --> TAILWIND["Tailwind CSS变量映射"]
TAILWIND --> UI["UI组件(Button等)"]
UI --> COMPONENTS["业务组件(画布/面板/语言切换器)"]
I18N["I18nProvider<br/>国际化"] --> COMPONENTS
COMPONENTS --> TRANSLATIONS["翻译内容(t函数)"]
```

**图表来源**
- [frontend/src/context/ThemeContext.tsx:48-63](file://frontend/src/context/ThemeContext.tsx#L48-L63)
- [frontend/tailwind.config.ts:10-61](file://frontend/tailwind.config.ts#L10-L61)
- [frontend/src/components/ui/button.tsx:1-57](file://frontend/src/components/ui/button.tsx#L1-L57)
- [frontend/src/i18n/I18nProvider.tsx:18](file://frontend/src/i18n/I18nProvider.tsx#L18)

**章节来源**
- [frontend/src/components/ui/button.tsx:1-57](file://frontend/src/components/ui/button.tsx#L1-L57)
- [frontend/src/hooks/use-tiptap-editor.ts:1-71](file://frontend/src/hooks/use-tiptap-editor.ts#L1-L71)

### 页面路由与 App Router 模式
- 根布局：设置语言、字体、全局样式与 Provider 注入，**包含国际化 Provider**。
- 页面组织：按路径组织页面，如首页、登录、剧场画布、资源与简单示例页。
- 路由守卫：认证上下文在初始化时检查路由与登录状态，非公开路由未登录自动跳转。

```mermaid
flowchart TD
Init(["应用启动"]) --> LoadLayout["加载RootLayout"]
LoadLayout --> CheckRoute["检查当前路由"]
CheckRoute --> IsPublic{"是否公开路由?"}
IsPublic --> |是| Render["渲染目标页面"]
IsPublic --> |否| HasToken{"是否存在token?"}
HasToken --> |是| Render
HasToken --> |否| Redirect["重定向到登录页"]
```

**图表来源**
- [frontend/src/app/layout.tsx:18-41](file://frontend/src/app/layout.tsx#L18-L41)
- [frontend/src/context/AuthContext.tsx:127-140](file://frontend/src/context/AuthContext.tsx#L127-L140)

**章节来源**
- [frontend/src/app/layout.tsx:1-45](file://frontend/src/app/layout.tsx#L1-L45)
- [frontend/src/context/AuthContext.tsx:1-207](file://frontend/src/context/AuthContext.tsx#L1-L207)

### AI 助手面板与交互流程
AI 助手面板整合了消息列表、输入区、Agent 切换、附件拖拽与上下文构建、SSE 流式响应处理、性能监控与虚拟滚动等能力。其核心交互流程如下：

```mermaid
sequenceDiagram
participant U as "用户"
participant Panel as "AIAssistantPanel"
participant Store as "useAIAssistantStore"
participant Canvas as "useCanvasStore"
participant Auth as "AuthContext"
participant SSE as "SSE处理器"
participant API as "后端API"
U->>Panel : 打开面板/输入消息
Panel->>Store : 获取当前会话与Agent
Panel->>Canvas : 读取剧场ID与附件
Panel->>Auth : createAuthFetch(带刷新)
Panel->>API : POST /chats/{sessionId}/messages
API-->>Panel : SSE事件流
Panel->>SSE : 解析事件并更新消息
Panel->>Store : 更新消息/上下文使用
Panel-->>U : 展示流式响应与工具/技能指示
```

**图表来源**
- [frontend/src/components/canvas/AIAssistantPanel.tsx:183-293](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L183-L293)
- [frontend/src/store/useAIAssistantStore.ts:148-200](file://frontend/src/store/useAIAssistantStore.ts#L148-L200)
- [frontend/src/store/useCanvasStore.ts:104-107](file://frontend/src/store/useCanvasStore.ts#L104-L107)
- [frontend/src/context/AuthContext.tsx:52-114](file://frontend/src/context/AuthContext.tsx#L52-L114)

**章节来源**
- [frontend/src/components/canvas/AIAssistantPanel.tsx:1-613](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L1-L613)

### 国际化在组件中的应用
国际化系统已深度集成到各个组件中：

**TopBar 组件国际化**：
- 导航链接使用 t() 函数获取翻译文本
- 用户菜单项支持国际化
- 搜索占位符根据语言动态调整
- 主题切换按钮的 aria-label 使用翻译

**TheaterCard 组件国际化**：
- 剧场状态标签使用翻译映射
- 时间格式化支持不同语言的日期显示
- 对话框标题和描述使用翻译
- 菜单项文本完全国际化

**LanguageSwitcher 组件国际化**：
- 语言切换菜单项使用翻译
- 当前语言显示支持翻译
- 动画和交互完全本地化

**章节来源**
- [frontend/src/components/home/TopBar.tsx:36-374](file://frontend/src/components/home/TopBar.tsx#L36-L374)
- [frontend/src/components/home/TheaterCard.tsx:77-320](file://frontend/src/components/home/TheaterCard.tsx#L77-L320)
- [frontend/src/components/LanguageSwitcher.tsx:13-79](file://frontend/src/components/LanguageSwitcher.tsx#L13-L79)

## 依赖关系分析
- 运行时依赖：Next.js 16、React 19、Ant Design 6、Radix UI、Zustand、Axios、@tiptap、framer-motion、react-window、recharts、socket.io-client、**i18next**、**react-i18next** 等。
- 构建与开发：TypeScript、Tailwind CSS v4、Jest、ESLint、SWC 等。
- 关键耦合点：
  - 组件与状态：AI 助手面板与画布面板分别消费对应 Zustand Store。
  - 组件与上下文：认证、主题与**国际化**上下文贯穿全局，组件通过 Hook 使用。
  - 组件与工具：API 封装与剧场 API 为组件提供数据通道。
  - UI 与主题：UI 组件依赖主题上下文与 Tailwind 变量映射。
  - **组件与国际化**：所有业务组件通过 useTranslation() 和 t() 函数使用翻译。

```mermaid
graph TB
PKG["package.json<br/>依赖声明"] --> NEXT["Next.js 16"]
PKG --> REACT["React 19"]
PKG --> ZUSTAND["Zustand"]
PKG --> AXIOS["Axios"]
PKG --> ANT["Ant Design 6"]
PKG --> TIP["TipTap"]
PKG --> RADIX["Radix UI"]
PKG --> FRAMER["Framer Motion"]
PKG --> XYFLOW["@xyflow/react"]
PKG --> I18NEXT["i18next"]
PKG --> R_I18NEXT["react-i18next"]
UI["UI组件/Button"] --> RADIX
UI --> ANT
PANEL["AIAssistantPanel"] --> ZUSTAND
PANEL --> AXIOS
PANEL --> TIP
PANEL --> NEXT
COMPONENTS["业务组件"] --> I18NEXT
COMPONENTS --> R_I18NEXT
LANG_SWITCH["LanguageSwitcher"] --> I18NEXT
```

**图表来源**
- [frontend/package.json:52-61](file://frontend/package.json#L52-L61)

**章节来源**
- [frontend/package.json:1-96](file://frontend/package.json#L1-L96)

## 性能考量
- 状态持久化：Zustand 的 persist 中间件仅持久化必要字段，减少存储体积与序列化开销。
- 虚拟滚动：AI 助手消息列表采用 react-window 虚拟化，提升长列表渲染性能。
- 流式渲染：SSE 流式响应逐步更新 UI，避免一次性渲染大量内容。
- 主题与样式：Tailwind CSS 变量映射与 Ant Design 算法切换，减少重复计算。
- 请求拦截与刷新：统一的 Axios 拦截器与刷新队列，避免并发刷新风暴。
- **国际化性能**：i18n 实例缓存翻译资源，语言切换时只更新受影响的组件，避免全应用重渲染。
- 构建优化：Next.js 16 与 SWC 加速构建，TypeScript 严格模式与增量编译提升开发体验。

## 故障排查指南
- 登录过期与 401 处理：当后端返回 401 且非认证端点时，自动刷新令牌并重试请求；若刷新失败，清空本地存储并跳转登录页。
- 重新登录弹窗：在 AI 助手面板中，遇到 401 且刷新失败时弹出重新登录对话框。
- 本地存储异常：Zustand 的 merge 逻辑会去重节点，若出现状态异常，可清理对应存储键后重试。
- 网络请求失败：检查 API 封装中的拦截器与刷新逻辑，确认刷新令牌是否有效。
- 主题切换无效：确认本地存储与 documentElement 类名是否正确写入，Tailwind CSS 变量映射是否生效。
- **语言切换问题**：检查 localStorage 中的 i18n-lang 键值，确认语言偏好是否正确保存；验证 i18n 实例的语言设置。
- **SSR 水合不匹配**：确认 I18nProvider 的客户端水合逻辑正常工作，避免服务端和客户端语言不一致。

**章节来源**
- [frontend/src/context/AuthContext.tsx:52-114](file://frontend/src/context/AuthContext.tsx#L52-L114)
- [frontend/src/lib/api.ts:31-81](file://frontend/src/lib/api.ts#L31-L81)
- [frontend/src/components/canvas/AIAssistantPanel.tsx:584-610](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L584-L610)
- [frontend/src/store/useCanvasStore.ts:521-536](file://frontend/src/store/useCanvasStore.ts#L521-L536)
- [frontend/src/i18n/I18nProvider.tsx:13-16](file://frontend/src/i18n/I18nProvider.tsx#L13-L16)

## 结论
Infinite Game 前端以 Next.js 16 App Router 为基础，结合 Ant Design 与 Tailwind 实现一致的主题与样式体系；通过 React Context 管理认证、主题与**国际化**，通过 Zustand 管理画布与 AI 助手状态，并以 API 封装与工具函数提供稳定的网络与编辑能力。**新增的国际化系统**提供了完整的多语言支持，包括客户端水合、语言偏好持久化和实时语言切换功能。整体架构清晰、模块职责明确、状态持久化与性能优化策略完善，适合长期演进与团队协作。

## 附录
- 构建配置：next.config.ts 开启大请求体限制与 API 代理，tsconfig.json 配置路径别名与严格模式，tailwind.config.ts 启用暗色模式与动画插件。
- **国际化配置**：使用 react-i18next + i18next，支持 zh-CN 和 en-US 语言包，语言切换时自动持久化到 localStorage，避免 SSR 水合不匹配问题。

**章节来源**
- [frontend/next.config.ts:1-21](file://frontend/next.config.ts#L1-L21)
- [frontend/tsconfig.json:1-35](file://frontend/tsconfig.json#L1-L35)
- [frontend/tailwind.config.ts:1-64](file://frontend/tailwind.config.ts#L1-L64)
- [frontend/src/context/ThemeContext.tsx:5-6](file://frontend/src/context/ThemeContext.tsx#L5-L6)
- [frontend/src/i18n/index.ts:12-20](file://frontend/src/i18n/index.ts#L12-L20)
- [frontend/src/i18n/I18nProvider.tsx:12-16](file://frontend/src/i18n/I18nProvider.tsx#L12-L16)