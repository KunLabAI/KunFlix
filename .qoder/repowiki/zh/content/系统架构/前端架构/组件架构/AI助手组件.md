...
## 更新摘要
**变更内容**
- 新增文本效果组件TextEffect，提供多种预设动画效果和分段渲染能力
- 增强流式传输功能，优化打字机效果的性能和用户体验
- 更新聊天消息组件以支持新的文本渲染选项
- 完善国际化支持，新增文本效果相关的翻译键
...
- **新增** 文本效果组件和增强的流式传输功能，提供更好的视觉反馈和用户体验
...
- **新增** UI组件：frontend/src/components/ui/text-effect.tsx
...
N["TypewriterText<br/>打字机文本"]
O["TextEffect<br/>文本效果组件"]
end
subgraph "国际化系统"
P["I18nProvider<br/>国际化提供者"]
Q["i18n配置<br/>多语言配置"]
R["英文翻译<br/>en-US.json"]
S["中文翻译<br/>zh-CN.json"]
end
subgraph "状态存储"
T["useAIAssistantStore<br/>Zustand状态"]
end
A --> B
A --> C
A --> D
...
**图表来源**
- [AIAssistantPanel.tsx:1-641](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L1-L641)
...
- **增强** 打字机文本：TypewriterText提供更流畅的流式文本渲染，支持自适应字符添加速度。
- **新增** 文本效果：TextEffect提供多种预设动画效果（模糊、抖动、缩放、淡入、滑动），支持按词、字符或行分割渲染。
...
**章节来源**
- [AIAssistantPanel.tsx:51-641](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L51-L641)
...
Typewriter --> TextEffect["文本效果渲染(TextEffect)"]
TextEffect --> RenderDone(["渲染完成"])
...
I["TextEffect<br/>文本效果"]
J["TypewriterText<br/>打字机文本"]
end
A --> B
B --> C
C --> D
...
- 组件耦合
  ...
  - ChatMessage依赖ThinkPanel、ToolCallIndicator、SkillCallIndicator、LazyImage、LazyCodeBlock、TypewriterText、TextEffect等子组件。
...
- **新增** 文本效果应用
  - 在需要强调的文本上应用TextEffect组件，选择合适的预设动画效果。
  - 配置分段渲染策略，优化长文本的显示性能。
...
- **新增** 文本效果标记
  - 在需要特殊视觉效果的文本上使用TextEffect组件，支持多种预设动画。
...
### **新增** 文本效果组件API
- **TextEffectProps**
  - children: string - 要渲染的文本内容
  - per: 'word' | 'char' | 'line' - 分段渲染单位，默认为'word'
  - as: JSX元素类型 - 包装元素类型，默认为'p'
  - variants: 自定义动画变体
  - className: CSS类名
  - preset: 预设动画类型
  - delay: 动画延迟时间
  ...
  - segmentWrapperClassName: 分段包装器类名
...
**章节来源**
- [TypewriterText.tsx:8-12](file://frontend/src/components/ai-assistant/TypewriterText.tsx#L8-L12)
- [TypewriterText.tsx:45-128](file://frontend/src/components/ai-assistant/TypewriterText.tsx#L45-L128)