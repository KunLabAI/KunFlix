...
## 更新摘要
**所做更改**
- 集成国际化支持系统，包括 i18n 初始化、语言切换器和多语言资源文件
...
## 简介
本文档为 Infinite Game 的"资源管理组件"提供系统化技术文档，聚焦媒体资产管理的前端 UI 组件与后端服务端点协同工作方式。经过重大重构后，系统现已具备现代化界面设计、增强的用户交互体验和完整的资源管理功能。内容涵盖：

- 国际化支持系统，包括 i18n 初始化、语言切换器和多语言资源文件
...
## 项目结构
资源管理功能由现代化前端页面与组件、状态管理、API 封装、国际化系统以及后端路由与模型共同组成。前端采用 Next.js App Router，使用 Zustand 管理资源状态，集成 Framer Motion 提供流畅动画效果，使用 Radix UI 构建对话框与下拉菜单；后端基于 FastAPI 提供媒体上传、列表、更新与删除接口，并持久化到本地文件系统。
...
Lang["LanguageSwitcher<br/>语言切换器"]
I18n["I18nProvider<br/>国际化提供者"]
Store["useResourceStore<br/>资源状态"]
API["resourceApi<br/>资源API封装"]
Utils["utils/cn<br/>工具函数"]
end
subgraph "国际化系统"
I18nIndex["i18n/index.ts<br/>i18n初始化"]
ZH["locales/zh-CN.json<br/>中文资源"]
EN["locales/en-US.json<br/>英文资源"]
end
subgraph "后端服务"
Router["/api/media 路由"]
...
**图表来源**
- [frontend/src/app/resources/page.tsx:63-672](file://frontend/src/app/resources/page.tsx#L63-L672)
...
- [frontend/src/i18n/I18nProvider.tsx:1-19](file://frontend/src/i18n/I18nProvider.tsx#L1-L19)
...
## 核心组件
- **现代化资源页 ResourcesPage**：采用固定定位导航栏设计，集成搜索、主题切换、用户菜单、语言切换和响应式布局
...
- **语言切换器 LanguageSwitcher**：提供中英文语言切换功能，支持下拉菜单和状态管理
- **国际化提供者 I18nProvider**：管理国际化状态，支持客户端挂载后的语言恢复
...
### 国际化系统集成
**更新** 新增完整的国际化支持系统，包括 i18n 初始化、语言切换器和多语言资源文件

- **i18n 初始化**：配置中英文双语支持，自动检测和持久化语言偏好
- **语言切换器**：提供下拉菜单式语言切换，支持中英文标识和状态管理
- **多语言资源**：包含导航、用户菜单、搜索、主题、资源管理等完整翻译
- **客户端挂载**：I18nProvider 在客户端挂载后从 localStorage 恢复语言设置
- **翻译键值**：使用 t() 函数获取翻译内容，支持动态参数替换
...
+viewMode : ViewModeTranslations
}
I18nSystem --> LanguageSwitcher : "管理"
I18nSystem --> I18nProvider : "提供"
I18nSystem --> TranslationKeys : "包含"
```

**图表来源**
- [frontend/src/i18n/index.ts:12-27](file://frontend/src/i18n/index.ts#L12-L27)
- [frontend/src/components/LanguageSwitcher.tsx:13-79](file://frontend/src/components/LanguageSwitcher.tsx#L13-L79)
- [frontend/src/i18n/I18nProvider.tsx:7-19](file://frontend/src/i18n/I18nProvider.tsx#L7-L19)
...
- [frontend/src/components/LanguageSwitcher.tsx:26-31](file://frontend/src/components/LanguageSwitcher.tsx#L26-L31)
...
- **国际化集成**：LanguageSwitcher 与 I18nProvider 协同工作，提供语言切换功能
...
Lang --> I18n["I18nProvider"]
I18n --> I18nIndex["i18n/index.ts"]
```
...
**章节来源**
- [frontend/src/app/resources/page.tsx:1-672](file://frontend/src/app/resources/page.tsx#L1-L672)
- [frontend/src/store/useResourceStore.ts:1-182](file://frontend/src/store/useResourceStore.ts#L1-L182)
- [frontend/src/lib/resourceApi.ts:1-109](file://frontend/src/lib/resourceApi.ts#L1-L109)
- [backend/routers/media.py:1-444](file://backend/routers/media.py#L1-L444)
...
## 结论
经过重大重构的资源管理组件通过现代化的界面设计与稳定的前后端协作，实现了媒体资产的高效管理。系统新增的国际化支持、固定导航栏、搜索功能、主题切换、用户菜单和拖拽上传等功能，显著提升了用户体验。前端采用 Framer Motion 提供流畅动画效果，使用 Zustand 管理复杂状态，集成 i18n 系统支持多语言切换，后端继续提供安全可靠的文件存储与资源管理能力。建议后续引入缩略图生成、CDN 加速与更丰富的搜索过滤能力，以进一步提升用户体验与系统性能。
...
- **国际化切换**：使用 LanguageSwitcher 组件在中英文之间切换语言
...
- **翻译键值**：包含导航、用户菜单、搜索、主题、资源管理等完整翻译
- **语言切换**：支持动态切换，自动保存到 localStorage
- **客户端挂载**：I18nProvider 在客户端挂载后恢复语言设置
- **翻译函数**：使用 t() 函数获取翻译内容，支持参数替换
