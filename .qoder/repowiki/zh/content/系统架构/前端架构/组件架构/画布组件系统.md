...
- [frontend/src/components/canvas/_edge-handle.scss](file://frontend/src/components/canvas/_edge-handle.scss)
- [frontend/src/components/canvas/script-editor.scss](file://frontend/src/components/canvas/script-editor.scss)
</cite>

## 更新摘要
**所做更改**
- 更新了节点样式现代化改进部分，包括浮动标题布局的实现
- 新增了统一NodeResizer颜色方案的说明
- 修正了节点适配模式的样式处理
- 增强了画布节点颜色系统和主题变量的说明
...
**更新** 本次更新重点关注画布节点样式的现代化改进，包括浮动标题布局、统一NodeResizer颜色方案和节点适配模式的样式修正。
...
CSS["样式系统(globals.css/_variables.scss)"]
CN --> NTB
SN --> NTB
SB --> NTB
VN --> NTB
...
**图表来源**
- [frontend/src/components/canvas/CharacterNode.tsx:1-596](file://frontend/src/components/canvas/CharacterNode.tsx#L1-L596)
- [frontend/src/components/canvas/ScriptNode.tsx:1-257](file://frontend/src/components/canvas/ScriptNode.tsx#L1-L257)
- [frontend/src/components/canvas/StoryboardNode.tsx:1-208](file://frontend/src/components/canvas/StoryboardNode.tsx#L1-L208)
...
- [frontend/src/app/globals.css:1-536](file://frontend/src/app/globals.css#L1-L536)
...
- 样式系统：统一的颜色变量、主题支持和现代化布局设计。

**更新** 样式系统现已实现统一的颜色变量和主题支持，所有节点组件都采用一致的NodeResizer颜色方案和浮动标题布局。
...
## 架构总览
画布系统以React Flow为核心，通过Zustand状态管理实现节点/边/视口/历史的统一存储；节点组件负责UI与用户交互，工具条提供统一操作入口；钩子模块封装拖拽、快捷键、吸附等行为；布局与校验工具保障自动布局与连通性约束；样式系统提供统一的主题和颜色支持。
...
Styles["样式系统<br/>globals.css/_variables.scss"]
RF --> ZS
ZS --> Nodes
ZS --> Edge
Nodes --> UI
...
- 功能要点
  ...
  - 工具条：复制、删除。
  - **更新** 采用浮动标题布局，标题悬浮在卡片上方，不占用节点布局空间。
- 交互细节
...
#### 浮动标题布局
所有节点组件现在采用统一的浮动标题布局设计：
- 标题使用绝对定位（absolute）脱离文档流
- 通过bottom-full类实现标题悬浮在卡片上方
- mb-1间距确保标题与内容之间有足够的空间
- nodrag类防止标题区域影响节点拖拽
- 支持响应式设计，标题在不同屏幕尺寸下保持一致性

#### 统一NodeResizer颜色方案
所有节点的NodeResizer现在使用统一的颜色方案：
- color="#6d6d6d" - 统一的灰色调
- isVisible={selected} - 仅在节点选中时显示
- minWidth/minHeight - 各节点设置合适的最小尺寸
- lineStyle={{ display: 'none' }} - 隐藏默认连线
- handleStyle - 统一的8px×8px圆形手柄，带边框和背景

#### 节点适配模式样式修正
- fitMode属性现在正确应用到图片和视频元素
- object-cover和object-contain类确保正确的显示效果
- 工具条中的切换图标根据当前模式动态显示
- 适配模式切换时提供即时视觉反馈

#### 颜色系统与主题支持
- 全局颜色变量定义：--node-blue、--node-green、--node-yellow、--node-purple
- 支持浅色和深色主题的自动切换
- CSS自定义属性与Tailwind CSS变量的统一
- 响应式颜色变量在不同主题下的表现

**章节来源**
- [frontend/src/app/globals.css:1-536](file://frontend/src/app/globals.css#L1-L536)
- [frontend/src/styles/_variables.scss:1-297](file://frontend/src/styles/_variables.scss#L1-L297)
- [frontend/src/components/canvas/_edge-handle.scss:1-118](file://frontend/src/components/canvas/_edge-handle.scss#L1-L118)
- [frontend/src/components/canvas/script-editor.scss:1-311](file://frontend/src/components/canvas/script-editor.scss#L1-L311)
...
- [frontend/src/app/globals.css:27-32](file://frontend/src/app/globals.css#L27-L32)
...
- 组件耦合
  ...
  - 所有节点组件依赖统一的样式系统和颜色变量。
- 外部依赖
  - @xyflow/react：画布核心、节点/边渲染、连接、视口控制。
  - dagre：自动布局。
  - Zustand：状态持久化与历史管理。
  - Tailwind CSS：样式框架和响应式设计。
- 循环依赖
...
- **更新** 样式问题排查
  - 浮动标题布局失效：检查absolute定位和bottom-full类的使用。
  - NodeResizer颜色不一致：确认color属性设置和主题变量。
  - 适配模式切换异常：检查fitMode状态和object-*类的应用。
...
## 结论
Infinite Game的画布组件系统以React Flow为基础，结合Zustand状态管理与丰富的交互钩子，构建了高可扩展的可视化编辑器。节点类型体系清晰，工具条与边组件统一了交互语言；状态管理与布局/校验工具保证了复杂场景下的稳定性与性能。

**更新** 本次现代化改进显著提升了用户体验：统一的浮动标题布局让界面更加整洁，一致的NodeResizer颜色方案增强了视觉统一性，修正的节点适配模式提供了更好的媒体展示效果。配合完善的样式系统和主题支持，系统在美观性和可用性方面都有了质的提升。

通过本文档提供的架构、API与最佳实践，开发者可以高效地扩展节点类型、优化性能并提升用户体验。
...
- 常用操作清单
  ...
  - 保存剧场：saveToBackend
- **更新** 样式系统配置
  - 颜色变量：--node-blue、--node-green、--node-yellow、--node-purple
  - 主题支持：浅色/深色/自定义主题切换
  - 响应式设计：移动端和桌面端的适配
...
- [frontend/src/app/globals.css:27-32](file://frontend/src/app/globals.css#L27-L32)
- [frontend/src/styles/_variables.scss:174-206](file://frontend/src/styles/_variables.scss#L174-L206)