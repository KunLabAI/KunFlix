...
## 更新摘要
**所做更改**
- 新增AI助手面板功能，提供实时聊天和创作辅助
- 集成富文本编辑器（Tiptap）支持高级文本格式化
...
- **富文本编辑器**：基于Tiptap的高级文本编辑功能，支持多种格式化选项
...
T[PIXI.js] --> U[图形渲染]
V[Tiptap] --> W[文本处理]
X[Framer Motion] --> Y[动画效果]
end
subgraph "数据持久化"
Z[localStorage] --> AA[状态同步]
AB[离线缓存] --> AC[数据恢复]
end
A --> M
C --> M
...
**章节来源**
- [ScriptNode.tsx:1-117](file://frontend/src/components/canvas/ScriptNode.tsx#L1-L117)
- [ScriptEditor.tsx:1-438](file://frontend/src/components/canvas/ScriptEditor.tsx#L1-L438)
...
subgraph "内部模块"
S[useCanvasStore] --> T[状态逻辑]
U[graphUtils] --> V[算法工具]
W[节点组件] --> X[UI逻辑]
Y[编辑器组件] --> Z[文本处理]
end
subgraph "应用集成"
AA[layout.tsx] --> AB[全局配置]
AC[page.tsx] --> AD[页面路由]
AE[CreateTheaterCard] --> AF[用户交互]
end
S --> T
U --> V
W --> X
...
### 主要优势
- **智能化创作辅助**：AI助手面板提供实时创意支持
- **专业级文本编辑**：Tiptap富文本编辑器支持高级格式化
