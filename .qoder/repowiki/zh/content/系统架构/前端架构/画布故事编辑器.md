...
## 更新摘要
**所做更改**
- 新增Dagre-based智能自动布局算法，支持连接节点和孤立节点的智能排列
- 新增useAutoLayout模块化钩子，提供一键自动布局功能
- 新增layoutUtils工具函数，实现Dagre图布局算法
- 新增ZoomControls自动布局按钮，集成智能布局功能
- 新增智能布局动画过渡效果，提升用户体验
- 更新画布节点视觉和功能改进，支持智能布局后的节点定位
...
- **智能自动布局算法**：新增Dagre-based自动布局，支持连接节点和孤立节点的智能排列
- **一键自动布局功能**：通过ZoomControls按钮实现快速智能布局
- **智能布局动画过渡**：布局变化时提供平滑的动画效果
...
OO[useAutoLayout] --> PP[自动布局钩子]
QQ[ZoomControls] --> RR[布局按钮]
end
subgraph "模块化钩子"
...
Q --> S[自动布局按钮]
T[useSocket] --> U[WebSocket通信]
V[useAutoLayout] --> W[自动布局钩子]
X[layoutUtils] --> Y[Dagre布局算法]
end
subgraph "状态管理层"
Z[useCanvasStore] --> AA[节点状态]
Z --> BB[边连接状态]
Z --> CC[历史记录]
Z --> DD[视口状态]
Z --> EE[剧院状态]
...
subgraph "模块化钩子层"
FF[useTheaterLoading] --> GG[剧院加载]
HH[useCanvasShortcuts] --> II[快捷键处理]
JJ[useCanvasDragDrop] --> KK[拖拽操作]
LL[useAutoLayout] --> MM[自动布局]
NN[useSocket] --> OO[实时通信]
end
subgraph "工具层"
PP[graphUtils] --> QQ[循环检测]
RR[layoutUtils] --> SS[Dagre算法]
TT[PIXI.js] --> UU[图形渲染]
VV[Tiptap] --> WW[文本处理]
XX[Framer Motion] --> YY[动画效果]
ZZ[ReactFlow] --> AA[画布引擎]
BB[Web Worker] --> CC[高性能计算]
DD[Ant Design] --> EE[UI组件]
FF[虚拟滚动] --> GG[大数据渲染]
HH[API封装] --> II[HTTP请求]
JJ[智能布局] --> KK[Dagre算法]
LL[自动布局] --> MM[动画过渡]
...
subgraph "智能体控制层"
PP[canvas_tools.py] --> QQ[画布工具]
RR[chats.py] --> SS[聊天服务]
TT[models.py] --> UU[数据模型]
VV[NodeTypes.tsx] --> WW[智能体节点类型]
end
```
...
系统新增了基于Dagre的智能自动布局算法，提供连接节点和孤立节点的智能排列功能：
...
1. **连接节点智能排列**：使用Dagre算法对连接的节点进行层次化布局
2. **孤立节点网格排列**：将未连接的节点按类型分组，在主图下方创建网格布局
3. **动态尺寸支持**：自动检测节点测量尺寸，支持动态内容变化
4. **类型分组优化**：按节点类型分组，使布局更加有序美观
5. **边界自适应**：根据连接图的最大尺寸动态调整网格起始位置
...
class NodeGrouping {
+Record<string, Node[]> isolatedByType
}
DagreConfig --> LayoutResult : 生成
NodeGrouping --> LayoutResult : 组合
```

**图表来源**
- [layoutUtils.ts:22-27](file://frontend/src/lib/layoutUtils.ts#L22-L27)
- [layoutUtils.ts:82-87](file://frontend/src/lib/layoutUtils.ts#L82-L87)
...
useAutoLayout钩子提供了一键自动布局功能：

```mermaid
sequenceDiagram
participant UC as ZoomControls
participant UA as useAutoLayout
participant LU as layoutUtils
participant RF as useReactFlow
participant CS as useCanvasStore
UC->>UA : 点击自动布局按钮
UA->>LU : 调用getLayoutedElements
LU->>LU : 分离连接/孤立节点
LU->>LU : 计算Dagre布局
LU->>LU : 计算网格布局
LU-->>UA : 返回布局结果
UA->>CS : onNodesChange更新位置
UA->>RF : fitView适应视图
UC->>UC : 显示动画效果
```
...
- [ZoomControls.tsx:66-77](file://frontend/src/components/canvas/ZoomControls.tsx#L66-L77)
...
智能布局按钮已集成到ZoomControls组件中：

```mermaid
flowchart TD
A[ZoomControls组件] --> B[布局按钮]
B --> C[点击事件处理]
C --> D[调用handleAutoLayout]
D --> E[禁用按钮状态]
E --> F[显示脉冲动画]
F --> G[布局完成后恢复状态]
```
...
participant RF as useReactFlow
participant CS as useCanvasStore
UC->>UA : handleAutoLayout()
UA->>LU : getLayoutedElements(nodes, edges)
LU->>LU : 分离连接/孤立节点
LU->>LU : 计算Dagre布局
LU->>LU : 计算网格布局
LU-->>UA : 返回布局结果
...
GG[dagre] --> HH[自动布局]
II[typescript] --> JJ[类型定义]
end
subgraph "内部模块"
KK[useCanvasStore] --> LL[状态逻辑]
MM[graphUtils] --> NN[算法工具]
OO[layoutUtils] --> PP[Dagre算法]
QQ[节点组件] --> RR[UI逻辑]
SS[编辑器组件] --> TT[文本处理]
UU[工具组件] --> VV[辅助功能]
WW[Pivot系统] --> XX[数据分析]
YY[VideoNode] --> ZZ[媒体处理]
AA[Web Worker] --> BB[高性能计算]
CC[虚拟滚动] --> DD[大数据渲染]
EE[模块化钩子] --> FF[功能分离]
GG[useSocket] --> HH[WebSocket通信]
II[useAutoLayout] --> JJ[自动布局]
KK[API封装] --> LL[HTTP请求处理]
MM[AI助手] --> NN[实时流式响应]
OO[画布工具] --> PP[节点控制]
QQ[聊天服务] --> RR[智能体集成]
SS[数据模型] --> TT[数据库映射]
...
- [package.json:49](file://frontend/package.json#L49)
...
### 主要优势
- **模块化钩子架构**：全新的useTheaterLoading、useCanvasShortcuts、useCanvasDragDrop、useAutoLayout等专用钩子，提供更好的代码组织和可维护性
...
- **智能自动布局算法**：基于Dagre的智能布局系统，支持连接节点和孤立节点的智能排列
- **一键自动布局功能**：通过ZoomControls按钮实现快速智能布局
- **智能布局动画过渡**：布局变化时提供平滑的动画效果
...
- **智能自动布局算法**：基于Dagre的高效图布局，提升复杂场景的布局质量
- **一键自动布局功能**：简化用户的布局操作流程
- **智能布局动画过渡**：提供流畅的视觉反馈和用户体验
