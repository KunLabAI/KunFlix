...
画布故事编辑器是一个基于React和Next.js构建的AI驱动叙事创作工具，允许用户通过可视化画布界面创建和管理故事内容。该系统提供了四种核心节点类型：剧本节点、角色节点、分镜节点和多维表格节点，支持实时协作编辑、撤销重做功能以及循环检测机制。

**更新后的新特性**：
- **Pivot表编辑器系统**：全新的多维数据分析和可视化功能
...
subgraph "应用集成"
SS[page.tsx] --> TT[页面路由]
UU[layout.tsx] --> VV[全局配置]
WW[CreateTheaterCard] --> XX[用户交互]
YY[TheaterCanvas] --> ZZ[画布渲染]
AA[ZoomControls] --> BB[缩放控制]
CC[AIAssistantPanel] --> DD[AI助手]
EE[ScriptEditor] --> FF[富文本编辑]
GG[ScriptNode] --> HH[节点编辑]
II[CharacterNode] --> JJ[角色管理]
KK[StoryboardNode] --> LL[分镜管理]
MM[CharacterEditModal] --> NN[角色编辑]
OO[StoryboardEditModal] --> PP[分镜编辑]
QQ[Sidebar] --> RR[节点库]
TT[PivotEditor] --> SS[数据分析]
UU[VideoNode] --> VV[媒体管理]
XX[usePivotEngine] --> WW[计算引擎]
YY[PivotTable] --> ZZ[数据展示]
AA[PivotDropzone] --> BB[拖拽区域]
```
