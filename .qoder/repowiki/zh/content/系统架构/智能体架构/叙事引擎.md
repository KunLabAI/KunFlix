...
## 简介
本文件面向Infinite Game叙事引擎，聚焦于NarrativeEngine类的整体设计与实现，涵盖引擎初始化、配置加载机制、智能体创建流程；LLM提供商标识系统、模型类型映射与配置参数处理；三智能体协作架构（Director、Narrator、NPC_Manager）；章节生成流程、token统计机制与错误处理策略；以及配置管理最佳实践、性能监控与故障排除建议。文档旨在帮助开发者与运营人员理解并高效维护该系统。
...
## 结论
NarrativeEngine通过清晰的配置加载与智能体创建流程，结合三智能体协作与统一的token统计机制，实现了稳定可控的章节生成能力。配合DynamicOrchestrator的多智能体编排、DialogAgent的安全与工具集成能力，以及完善的日志与计费体系，为Infinite Game叙事系统提供了可扩展、可观测、可维护的技术基础。
