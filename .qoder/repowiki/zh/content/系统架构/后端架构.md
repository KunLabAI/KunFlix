...
## 简介
本文件系统化梳理无限剧情游戏后端架构，围绕 FastAPI 异步应用、依赖注入与生命周期管理、数据库连接池与 SQLAlchemy ORM 映射、业务服务层设计、路由组织与中间件/CORS、数据库模型与索引策略、错误处理与日志、以及性能监控方案展开。目标是帮助开发者快速理解并高效扩展系统。
...
- 业务服务层：GameService 封装玩家创建、世界初始化、选择处理等业务流程。
...
## 结论
本后端以 FastAPI 为核心，结合 SQLAlchemy 异步 ORM、AgentScope 智能体与 Alembic 迁移，构建了可扩展、可观测、易维护的无限剧情游戏后端。通过清晰的分层与依赖注入，实现了从路由到业务再到数据层的高内聚低耦合；通过生命周期管理与连接池配置，保障了运行时稳定性与性能。
