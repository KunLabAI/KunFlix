...
## 简介
本指南面向“无限剧情剧场系统”的后端与管理端，聚焦于安全配置与最佳实践，覆盖认证授权、API密钥管理、访问控制、数据传输安全（HTTPS/WebSocket）、防火墙与网络策略、安全审计与合规、漏洞扫描与修复、隐私保护与数据处理合规、以及应急响应与事件处置流程。文档以仓库现有代码与配置为依据，结合概念性建议，帮助团队建立可操作的安全基线。
...
- 建议：
  ...
  - 对管理端与剧场端分别设置独立的 CORS 规则与子路径访问控制。
