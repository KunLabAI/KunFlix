...
## 简介
本文件详细介绍Infinite Theater项目重构后的身份认证与授权系统。该系统基于JWT标准实现了完整的认证流程，包括用户注册、登录、令牌刷新、权限控制等功能。系统采用bcrypt进行密码哈希，支持管理员角色和用户权限分离，实现了从Player表到User/Admin双表架构的完全重构，为后续的企业级认证集成奠定了基础。
