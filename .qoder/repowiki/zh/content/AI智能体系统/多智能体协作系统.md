...
## 更新摘要
**变更内容**
- 系统已从Pipeline/Plan/Discussion三种协作策略简化为单一UnifiedStrategy
- 移除了复杂的多策略选择机制，采用统一的依赖驱动执行策略
- 更新了智能体配置常量，仅保留'unified'协作模式
- 简化了编排器的策略选择逻辑
...
本项目是一个基于AgentScope框架构建的多智能体协作系统，专注于叙事创作和故事生成。系统实现了三个核心智能体：Director导演智能体、Narrator叙述智能体和NPC_Manager管理智能体，通过统一的协作策略实现高效的协作流程。

系统采用领导者模式（Leader Mode）进行任务分配和协调，现已简化为单一的UnifiedStrategy协作策略。每个智能体都具备工具调用能力，可以访问内置技能和自定义技能，实现丰富的功能扩展。
...
**更新** 系统已从三种协作策略简化为单一UnifiedStrategy策略

#### 统一协作策略（UnifiedStrategy）

UnifiedStrategy是系统当前唯一支持的协作策略，实现了依赖驱动的任务调度：

```mermaid
flowchart TD
Start([开始执行]) --> BuildGraph[构建依赖图]
BuildGraph --> FindReady[查找就绪任务]
FindReady --> ReadyTasks{有就绪任务?}
ReadyTasks --> |是| CheckCount{就绪任务数量}
CheckCount --> |多个| Parallel[并行执行多个任务]
CheckCount --> |单个| Streaming[流式执行单个任务]
ReadyTasks --> |否| CheckPending{仍有待执行任务?}
Parallel --> UpdateOutputs[更新已完成输出]
Streaming --> UpdateOutputs
UpdateOutputs --> RemoveTask[从待执行列表移除]
RemoveTask --> FindReady
CheckPending --> |是| FindReady
CheckPending --> |否| Complete[完成]
Complete --> End([结束])
```
...
UnifiedStrategy的主要特点：
- **依赖驱动调度**：根据子任务间的依赖关系自动调度执行顺序
- **智能并行**：同一依赖层级且无相互依赖的任务并行执行
- **实时流式**：单个任务执行时提供实时进度反馈
- **错误处理**：支持任务级别的错误捕获和重试
...
2. **简化的协作策略**：单一的UnifiedStrategy策略降低了复杂性，提升了执行效率
3. **强大的智能体能力**：具备工具调用和记忆管理功能
4. **完善的计费系统**：精确的计费机制确保成本控制
5. **实时响应能力**：流式响应提供良好的用户体验

系统在实际应用中展现了优秀的可扩展性和稳定性，为复杂的人工智能协作场景提供了可靠的基础设施。统一的协作策略使得系统更加简洁高效，同时保持了强大的功能完整性。