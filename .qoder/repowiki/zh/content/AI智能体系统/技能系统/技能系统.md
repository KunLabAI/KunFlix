...
- [ToolCapabilities.tsx](file://backend/admin/src/components/admin/agents/AgentForm/Tools/ToolCapabilities.tsx)
- [NodeTypes.tsx](file://backend/admin/src/components/admin/agents/AgentForm/NodeTypes.tsx)
- [Skills.tsx](file://backend/admin/src/components/admin/agents/AgentForm/Tools/Skills.tsx)
...
## 更新摘要
**所做更改**
- 更新了 AgentForm schema，新增画布节点类型支持
- 新增技能激活状态管理，支持传统工具、图像生成能力和画布节点类型的独立控制
- 增强了工具能力配置界面，提供更直观的技能、图像生成和画布工具开关
- 扩展了技能系统与工具管理的集成，支持多维度能力配置
...
- **新增**：技能激活状态管理，支持传统工具、图像生成能力和画布节点类型的独立控制
...
- 后端核心
  ...
  - 应用入口与路由注册：main.py
- **新增**：前端 AgentForm 配置界面
  - schema.ts：Agent 配置表单验证规则
  - Tools 组件：技能、图像生成、画布工具配置界面
- **新增**：工具管理模块
  - tool_manager：统一工具管理
  - providers：工具提供者实现（CanvasProvider、ImageGenProvider）
...
TM["tool_manager/__init__.py<br/>工具管理模块"]
TP["providers/canvas.py<br/>画布工具提供者"]
TIMG["providers/image_gen.py<br/>图像生成工具提供者"]
end
subgraph "前端配置"
AF["AgentForm/schema.ts<br/>Agent配置验证"]
TC["Tools/ToolCapabilities.tsx<br/>工具能力配置"]
NT["NodeTypes.tsx<br/>画布节点类型"]
SK["Tools/Skills.tsx<br/>技能选择"]
end
Builtin --> SM
Custom --> SM
SM --> Active
...
- [schema.ts:31-79](file://backend/admin/src/components/admin/agents/AgentForm/schema.ts#L31-L79)
- [ToolCapabilities.tsx:1-148](file://backend/admin/src/components/admin/agents/AgentForm/Tools/ToolCapabilities.tsx#L1-L148)
- [NodeTypes.tsx:1-103](file://backend/admin/src/components/admin/agents/AgentForm/NodeTypes.tsx#L1-L103)
- [Skills.tsx:1-63](file://backend/admin/src/components/admin/agents/AgentForm/Tools/Skills.tsx#L1-L63)
...
- **新增**：工具管理模块
  - ToolManager：统一工具注册、发现与分发
  - CanvasProvider：画布节点 CRUD 工具
  - ImageGenProvider：图像生成工具
  - 工具上下文：统一的工具执行环境
...
- [canvas.py:513-548](file://backend/services/tool_manager/providers/canvas.py#L513-L548)
...
TK->>TM : "build_tool_defs(ToolContext)"
TM-->>Agent : "注册完成"
```
...
- [agents.py:85-113](file://backend/agents.py#L85-L113)
- [tool_manager/manager.py:42-52](file://backend/services/tool_manager/manager.py#L42-L52)
...
### **新增**：AgentForm 配置与技能激活状态管理

#### AgentForm Schema 更新
AgentForm schema 已更新以支持新的技能激活状态管理：

- **技能激活状态**：`tools_enabled` 字段控制整体工具能力开关
- **技能选择**：`tools` 数组包含已启用的技能名称
- **图像生成能力**：`image_config.image_generation_enabled` 控制图像生成工具
- **画布节点类型**：`target_node_types` 数组定义可控制的画布节点类型
- **验证规则**：启用能力时至少需要选择一个技能、启用图像生成或画布工具

```mermaid
flowchart TD
A["AgentForm 提交"] --> B["tools_enabled?"]
B --> |true| C["验证至少一个能力"]
B --> |false| D["保存基础配置"]
C --> E["hasSkills?"]
E --> |true| F["保存技能配置"]
E --> |false| G["hasImageGen?"]
G --> |true| F
G --> |false| H["hasCanvas?"]
H --> |true| F
H --> |false| I["验证失败"]
...
**图表来源**
- [schema.ts:67-76](file://backend/admin/src/components/admin/agents/AgentForm/schema.ts#L67-L76)

#### 工具能力配置界面
前端提供了直观的工具能力配置界面：

- **技能选择**：`Skills.tsx` 提供技能列表选择
- **图像生成**：`ToolCapabilities.tsx` 提供图像生成工具开关
- **画布工具**：`ToolCapabilities.tsx` 和 `NodeTypes.tsx` 提供画布节点类型配置
- **实时验证**：表单提交时进行完整性检查

**章节来源**
- [schema.ts:31-79](file://backend/admin/src/components/admin/agents/AgentForm/schema.ts#L31-L79)
- [AgentForm/index.tsx:52-320](file://backend/admin/src/components/admin/agents/AgentForm/index.tsx#L52-L320)
- [ToolCapabilities.tsx:1-148](file://backend/admin/src/components/admin/agents/AgentForm/Tools/ToolCapabilities.tsx#L1-L148)
- [NodeTypes.tsx:1-103](file://backend/admin/src/components/admin/agents/AgentForm/NodeTypes.tsx#L1-L103)
- [Skills.tsx:1-63](file://backend/admin/src/components/admin/agents/AgentForm/Tools/Skills.tsx#L1-L63)
...
#### ToolManager 架构
工具管理模块提供了统一的工具注册、发现与分发机制：

- **ToolManager**：中央协调器，管理所有工具提供者
- **ToolProvider 协议**：插件化工具提供者的抽象接口
- **工具上下文**：统一的工具执行环境（theater_id、agent、db 等）
...
+display_name "图像生成工具"
+description "统一的图像生成接口"
+condition "需要 image_config"
+tool_names_set
+build_defs(ctx) list
+execute(name, args, ctx) str
}
...
- **节点类型**：text（文本）、image（图像）、video（视频）、storyboard（分镜）
- **工具集**：list_canvas_nodes、get_canvas_node、create_canvas_node、update_canvas_node、delete_canvas_node
- **动态配置**：根据目标节点类型动态生成工具定义

**章节来源**
- [tool_manager/manager.py:23-108](file://backend/services/tool_manager/manager.py#L23-L108)
- [canvas.py:25-86](file://backend/services/tool_manager/providers/canvas.py#L25-L86)
- [canvas.py:126-200](file://backend/services/tool_manager/providers/canvas.py#L126-L200)
- [chat_generation.py:19](file://backend/services/chat_generation.py#L19)
...
- **新增**：工具能力隔离
  - 通过 `tools_enabled` 控制整体工具能力
  - 独立控制技能、图像生成、画布工具等不同能力维度
...
TM["tool_manager/manager.py"] --> TP["providers/canvas.py"]
TM --> TIMG["providers/image_gen.py"]
AF["AgentForm/schema.ts"] --> TC["Tools/ToolCapabilities.tsx"]
TC --> NT["NodeTypes.tsx"]
TC --> SK["Tools/Skills.tsx"]
```
...
- [agent_extensions.py:7-163](file://backend/agent_extensions.py#L7-L163)
- [tool_manager/manager.py:1-108](file://backend/services/tool_manager/manager.py#L1-L108)
- [schema.ts:1-79](file://backend/admin/src/components/admin/agents/AgentForm/schema.ts#L1-L79)
- [ToolCapabilities.tsx:1-148](file://backend/admin/src/components/admin/agents/AgentForm/Tools/ToolCapabilities.tsx#L1-L148)
...
- [tool_manager/manager.py:42-81](file://backend/services/tool_manager/manager.py#L42-L81)
...
- **新增**：工具能力配置问题
  - 检查 AgentForm 验证规则，确保至少启用一种能力
  - 确认工具提供者正确注册到 ToolManager
  - 验证工具上下文（theater_id、agent 配置）是否完整
...
- **新增**：完善的工具能力配置管理，支持传统工具、图像生成和画布工具的独立控制
- **新增**：统一的工具管理模块，提供插件化的工具提供者架构
...
- **新增**：Agent 配置字段说明
  - `tools_enabled`：整体工具能力开关
  - `tools`：已启用技能列表
  - `image_config.image_generation_enabled`：图像生成能力开关
  - `target_node_types`：画布节点类型列表
