...
- [backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx](file://backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx)
...
## 更新摘要
**变更内容**
- 新增ImageGenConfigDialog组件，提供统一的图像生成配置对话框
- 增强工具页面的图像生成配置功能，支持智能体级别的图像生成参数配置
- 更新工具注册表以支持统一图像配置系统，包括智能体级配置参数展示
- 新增useImageCapabilities钩子用于获取图像供应商能力
...
**更新** 系统现已支持统一图像配置管理，通过新增的ImageGenConfigDialog组件和图像供应商能力API为管理员提供动态的图像生成配置支持。系统采用分层架构设计，通过ToolManager中央协调器统一管理各种工具Provider，实现了高度模块化和可扩展的工具管理体系。

管理员可以通过直观的Web界面实时监控工具使用情况，进行故障诊断和性能优化，同时支持跨供应商的统一图像配置管理。新的图像生成配置功能允许管理员为每个智能体单独配置图像生成参数，包括供应商选择、模型设置、宽高比、画质和批量生成数量等。
...
ImageGenConfigDialog[图像生成配置对话框]
end
subgraph "接口层"
Router[AdminTools路由]
Auth[管理员认证]
...
ImageGenConfigDialog --> ImageCapabilitiesAPI
```
...
- **图像生成配置对话框**：新增的ImageGenConfigDialog组件，支持智能体级别的图像生成参数配置
...
**章节来源**
- [backend/admin/src/app/admin/tools/page.tsx:14-215](file://backend/admin/src/app/admin/tools/page.tsx#L14-L215)
- [backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx:1-286](file://backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx#L1-L286)
...
API-->>Admin : 图像能力数据
Admin->>API : POST /admin/tools/agents/{id}
API->>Manager : 更新Agent配置
Manager-->>API : 更新结果
API-->>Admin : 配置更新完成
```
...
+stats : ToolStats
+agents : Agent[]
+imageCapabilities : ImageProviderCapabilities
+isLoading : boolean
+configAgent : Agent | null
+configDialogOpen : boolean
+renderStatsCards()
+renderRegistry()
+renderToolStats()
+renderAgentUsage()
+renderImageGenConfigTable()
}
class ImageGenConfigDialog {
+agent : Agent | null
+open : boolean
...
ToolsPage --> ImageGenConfigDialog
ImageGenConfigDialog --> Agent
ImageGenConfigDialog --> LLMProvider
ImageGenConfigDialog --> ImageProviderCapability
```

**图表来源**
- [backend/admin/src/app/admin/tools/page.tsx:14-215](file://backend/admin/src/app/admin/tools/page.tsx#L14-L215)
- [backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx:42-49](file://backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx#L42-L49)
...
- 启用条件说明
- 支持的工具列表（以标签形式展示）
- **新增** generate_image工具的智能体级可配置参数定义
...
#### **新增** 图像生成工具配置表格
**更新** 新增专门的图像生成工具配置区域，支持智能体级别的配置管理：

```mermaid
flowchart TD
Start([打开配置对话框]) --> LoadAgent[加载智能体数据]
LoadAgent --> InitForm[初始化表单状态]
InitForm --> LoadCapabilities[加载图像能力]
LoadCapabilities --> RenderFields[渲染配置字段]
RenderFields --> UserInput[用户输入配置]
UserInput --> ValidateConfig[验证配置有效性]
ValidateConfig --> SaveConfig[保存配置到后端]
SaveConfig --> ShowSuccess[显示成功消息]
ShowSuccess --> CloseDialog[关闭对话框]
```

**图表来源**
- [backend/admin/src/app/admin/tools/page.tsx:173-238](file://backend/admin/src/app/admin/tools/page.tsx#L173-L238)
- [backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx:72-82](file://backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx#L72-L82)

**章节来源**
- [backend/admin/src/app/admin/tools/page.tsx:44-211](file://backend/admin/src/app/admin/tools/page.tsx#L44-L211)

### ImageGenConfigDialog组件

**新增** ImageGenConfigDialog是本次更新的核心组件，提供完整的图像生成配置功能：
...
**图表来源**
- [backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx:42-49](file://backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx#L42-L49)
- [backend/admin/src/types/index.ts:21-26](file://backend/admin/src/types/index.ts#L21-L26)

#### 组件功能特性
- **智能体级别配置**：支持为每个智能体单独配置图像生成参数
- **动态供应商选择**：根据选择的供应商动态更新可用模型和配置选项
- **实时能力验证**：基于图像供应商能力API验证用户输入的有效性
- **统一配置适配**：自动将统一配置转换为供应商特定的配置格式
- **完整的配置覆盖**：支持宽高比、画质、批量数量、输出格式等完整参数集
...
| 参数名称 | 类型 | 描述 | 供应商支持情况 |
|----------|------|------|----------------|
| image_generation_enabled | 布尔值 | 是否启用图像生成功能 | 全部支持 |
| image_provider_id | 字符串 | 图像生成供应商ID | 全部支持 |
| image_model | 字符串 | 图像生成模型名称 | 全部支持 |
| aspect_ratio | 枚举 | 图像宽高比 | Gemini: 6种, xAI: 15种 |
| quality | 枚举 | 图像质量等级 | Gemini: 3种, xAI: 2种 |
| batch_count | 整数 | 每次批量生成数量 | Gemini: 1-8, xAI: 1-10 |
| output_format | 枚举 | 输出图像格式 | Gemini: 3种, xAI: 无 |
...
SWR --> AdminPage
SWR --> ImageGenConfigDialog
NextJS --> AdminPage
Lucide --> AdminPage
AdminPage --> Hooks
ImageGenConfigDialog --> Hooks
Hooks --> Router
Router --> Manager
Manager --> Providers
Manager --> Context
...
- **配置对话框缓存**：ImageGenConfigDialog组件缓存供应商能力数据
...
#### 5. **新增** 图像生成配置对话框问题
**症状**：ImageGenConfigDialog组件无法正常工作
**可能原因**：
- 供应商能力数据加载失败
- 表单状态同步问题
- 配置保存API调用失败
...
**解决步骤**：
1. 检查useImageCapabilities钩子的数据获取
2. 验证表单状态的初始化和更新逻辑
3. 确认updateAgent API调用成功
4. 检查toast消息的显示和错误处理
...
- [backend/admin/src/components/admin/agents/AgentForm/Parameters.tsx:95-109](file://backend/admin/src/components/admin/agents/AgentForm/Parameters.tsx#L95-L109)
- [backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx:104-132](file://backend/admin/src/components/admin/tools/ImageGenConfigDialog.tsx#L104-L132)
...
8. **智能体级配置**：新增的ImageGenConfigDialog组件支持每个智能体的独立配置

**更新** 新增的ImageGenConfigDialog组件和统一图像配置系统为管理员提供了更强大的工具管理能力，支持动态配置和跨供应商的图像生成管理。该组件提供了完整的配置界面，包括供应商选择、模型配置、参数设置等功能，大大简化了图像生成配置的管理工作。

通过合理的架构设计和性能优化，系统能够满足大规模工具管理的需求，为管理员提供全面的工具监控和管理能力。新的图像生成配置功能不仅提高了系统的灵活性，还为未来的功能扩展奠定了坚实的基础。