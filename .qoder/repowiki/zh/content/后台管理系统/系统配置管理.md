...
本项目是一个基于FastAPI的无限叙事剧场系统，采用现代化的配置管理模式。系统通过Pydantic Settings实现类型安全的配置管理，支持多环境配置、运行时配置热更新和数据库驱动的配置持久化。
...
| 配置项 | 类型 | 默认值 | 描述 |
|--------|------|--------|------|
| PROJECT_NAME | string | "Infinite Narrative Theater" | 项目名称 |
| VERSION | string | "1.0.0" | 版本号 |
| DATABASE_URL | string | sqlite+aiosqlite:///infinite_theater.db | 数据库连接字符串 |
...
flowchart LR
subgraph "开发环境"
DevDB[DATABASE_URL<br/>sqlite+aiosqlite:///infinite_theater.db]
...
该配置管理系统为无限叙事剧场提供了稳定、可扩展的基础架构，支持从开发到生产的全生命周期管理。
...
# 或使用SQLite用于开发
# DATABASE_URL=sqlite:///./infinite_theater.db
...
```bash
# PostgreSQL备份
pg_dump -h localhost -U username infinite_theater_db > backup_$(date +%Y%m%d_%H%M%S).sql

# SQLite备份  
cp infinite_theater.db infinite_theater_backup_$(date +%Y%m%d_%H%M%S).db
```
