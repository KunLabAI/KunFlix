...
- [backend/admin/src/hooks/usePromptTemplates.ts](file://backend/admin/src/hooks/usePromptTemplates.ts)
...
**更新** 新增提示词模板管理功能，支持AI生成使用的提示词模板创建、编辑、删除和搜索，包含智能体类型分类和变量定义功能。
...
G --> J["提示词模板页面<br/>backend/admin/src/app/admin/prompt-templates/page.tsx"]
J --> K["提示词模板对话框<br/>backend/admin/src/app/admin/prompt-templates/PromptTemplateDialog.tsx"]
H --> L["数据获取与可视化<br/>useSWR + Recharts"]
I --> M["表单校验与反馈<br/>react-hook-form + Zod"]
```
...
- [backend/admin/src/components/admin/AdminLayout.tsx](file://backend/admin/src/components/admin/AdminLayout.tsx#L1-L178)
...
- 页面级组件：仪表盘统计卡片与柱状图、登录表单与校验、提示词模板管理页面。
- 数据钩子：基于 SWR 的智能体列表、详情、增删改查封装，新增提示词模板数据钩子。
- 工具函数：Tailwind 合并类名工具，简化样式组合。

**更新** 新增提示词模板管理功能，包含完整的CRUD操作和变量定义功能。
...
- [backend/admin/src/hooks/usePromptTemplates.ts](file://backend/admin/src/hooks/usePromptTemplates.ts#L1-L53)
...
V --> PT["提示词模板<br/>admin/prompt-templates/page.tsx"]
PT --> PD["模板对话框<br/>PromptTemplateDialog.tsx"]
D --> SWR["数据钩子<br/>useAgents.ts"]
PT --> SWR2["数据钩子<br/>usePromptTemplates.ts"]
...
- 导航项：仪表盘、AI 供应商、智能体管理、**提示词模板**、用户管理、故事管理、订阅套餐、管理员。
- 侧边栏交互：支持折叠/展开，移动端可通过顶部按钮触发菜单（预留）。
- 用户菜单：顶部下拉菜单提供"退出登录"。

**更新** 新增提示词模板导航项，图标为 FileCode2，对应 /admin/prompt-templates 路由。
...
- [backend/admin/src/components/admin/AdminLayout.tsx](file://backend/admin/src/components/admin/AdminLayout.tsx#L44-L85)
...
### 提示词模板管理功能
- 模板管理：支持创建、编辑、删除提示词模板，包含搜索和过滤功能。
- 模板分类：支持故事基础设定、角色设定、场景描述、分镜脚本、自定义等分类。
- 智能体类型：支持文本处理、图像处理、多模态三种类型。
- 变量定义：支持字符串、多行文本、数字、布尔值、下拉选择五种变量类型。
- 状态管理：支持启用/禁用和设为默认模板功能。
- 对话框表单：完整的模板创建和编辑对话框，包含基础信息、智能体类型、提示词内容、输入变量定义和状态设置。

**新增** 提示词模板管理功能，提供完整的AI提示词模板生命周期管理。

```mermaid
sequenceDiagram
participant U as "用户"
participant PT as "提示词模板页面"
participant PD as "模板对话框"
participant SWR as "数据钩子"
participant API as "后端 API"
U->>PT : 点击创建模板
PT->>PD : 打开创建对话框
PD->>PD : 填写模板信息
PD->>SWR : 调用 createTemplate
SWR->>API : POST /prompt-templates/
API-->>SWR : 返回创建结果
SWR-->>PD : 更新状态
PD-->>U : 显示成功提示
```
...
**图示来源**
- [backend/admin/src/app/admin/prompt-templates/page.tsx](file://backend/admin/src/app/admin/prompt-templates/page.tsx#L93-L102)
- [backend/admin/src/app/admin/prompt-templates/PromptTemplateDialog.tsx](file://backend/admin/src/app/admin/prompt-templates/PromptTemplateDialog.tsx#L137-L175)
- [backend/admin/src/hooks/usePromptTemplates.ts](file://backend/admin/src/hooks/usePromptTemplates.ts#L33-L37)

**章节来源**
- [backend/admin/src/app/admin/prompt-templates/page.tsx](file://backend/admin/src/app/admin/prompt-templates/page.tsx#L1-L268)
- [backend/admin/src/app/admin/prompt-templates/PromptTemplateDialog.tsx](file://backend/admin/src/app/admin/prompt-templates/PromptTemplateDialog.tsx#L1-L416)
- [backend/admin/src/hooks/usePromptTemplates.ts](file://backend/admin/src/hooks/usePromptTemplates.ts#L1-L53)
- [backend/admin/src/types/index.ts](file://backend/admin/src/types/index.ts#L163-L191)
...
### 数据模型与工具常量
- 类型定义：Agent、LLMProvider、AgentFormValues、**PromptTemplate、PromptTemplateVariable** 等接口，明确字段与可选时间戳。
- 智能体工具：可用工具集合与默认值常量，用于表单初始化与校验。
- **提示词模板类型**：包含模板ID、名称、描述、分类、智能体类型、系统提示词模板、用户提示词模板、输出模式、变量模式、默认智能体ID、启用状态、默认状态等字段。

**更新** 新增提示词模板相关类型定义，支持完整的模板管理功能。
...
**更新** 新增提示词模板相关依赖，包括专门的数据钩子和类型定义。

```mermaid
graph LR
UI["页面组件"] --> HOOK["数据钩子<br/>useAgents.ts / usePromptTemplates.ts"]
...
PT["提示词模板页面"] --> PD["PromptTemplateDialog.tsx"]
```
...
- **模板加载优化**：提示词模板使用分页查询（limit=100），避免大量数据一次性加载。
...
- **模板操作失败**：检查模板表单必填字段；查看 Toast 错误提示；确认模板分类和变量定义正确。
...
## 结论
该管理界面以清晰的分层架构实现认证、导航与数据展示，结合响应式布局与主题化设计，具备良好的可维护性与扩展性。**新增的提示词模板管理功能提供了完整的AI提示词模板生命周期管理能力，包括创建、编辑、删除、搜索和分类等功能，为智能体系统的提示词管理提供了强大支持。** 后续可在权限细化、批量操作与第三方集成方面进一步完善。
...
- 折叠侧边栏：顶部按钮点击切换。
- 退出登录：顶部用户菜单选择"退出登录"。
- **模板操作**：在提示词模板页面支持键盘导航，Tab 键在表单字段间切换。
...
- **模板样式**：提示词模板页面使用 FileCode2 图标，支持彩色标签和状态徽章。
...
- **模板扩展**：可扩展更多模板类型和变量类型，支持复杂的AI提示词需求。
