# 提示词模板管理系统

<cite>
**本文档引用的文件**
- [main.py](file://backend/main.py)
- [models.py](file://backend/models.py)
- [schemas.py](file://backend/schemas.py)
- [prompt_templates.py](file://backend/routers/prompt_templates.py)
- [llm_stream.py](file://backend/services/llm_stream.py)
- [billing.py](file://backend/services/billing.py)
- [database.py](file://backend/database.py)
- [auth.py](file://backend/auth.py)
- [config.py](file://backend/config.py)
- [requirements.txt](file://backend/requirements.txt)
- [page.tsx](file://frontend/src/app/create-theater/page.tsx)
- [TemplateList.tsx](file://frontend/src/components/create-theater/TemplateList.tsx)
- [TemplateDetails.tsx](file://frontend/src/components/create-theater/TemplateDetails.tsx)
- [data.ts](file://frontend/src/components/create-theater/data.ts)
</cite>

## 目录
1. [项目概述](#项目概述)
2. [系统架构](#系统架构)
3. [核心组件](#核心组件)
4. [数据库设计](#数据库设计)
5. [API接口设计](#api接口设计)
6. [前端界面](#前端界面)
7. [服务层实现](#服务层实现)
8. [安全机制](#安全机制)
9. [部署配置](#部署配置)
10. [总结](#总结)

## 项目概述

提示词模板管理系统是一个基于FastAPI开发的AI内容生成平台，专门用于管理和使用预定义的提示词模板来生成各种类型的剧场内容。系统支持多种模板类型，包括故事基础设定、角色设定、场景描述、分镜脚本等，为用户提供了一个灵活且强大的AI内容创作工具。

该系统采用前后端分离架构，后端使用Python FastAPI框架，前端使用Next.js React技术栈，支持多模态AI模型集成，具备完善的权限管理和计费系统。

## 系统架构

```mermaid
graph TB
subgraph "前端层"
FE1[Next.js 应用]
FE2[React 组件]
FE3[用户界面]
end
subgraph "API网关层"
API[FastAPI 应用]
ROUTER[路由管理]
AUTH[认证中间件]
end
subgraph "业务逻辑层"
PT[提示词模板服务]
ORCH[编排服务]
BILL[计费服务]
LLM[LLM流式服务]
end
subgraph "数据持久层"
DB[(数据库)]
MODELS[ORM模型]
SCHEMAS[数据模式]
end
subgraph "外部服务"
LLM_API[LLM提供商]
REDIS[缓存服务]
end
FE1 --> API
FE2 --> API
FE3 --> API
API --> ROUTER
ROUTER --> AUTH
AUTH --> PT
PT --> ORCH
PT --> BILL
PT --> LLM
ORCH --> DB
BILL --> DB
LLM --> LLM_API
DB --> MODELS
MODELS --> SCHEMAS
```

**图表来源**
- [main.py](file://backend/main.py#L84-L104)
- [prompt_templates.py](file://backend/routers/prompt_templates.py#L25-L29)
- [database.py](file://backend/database.py#L25-L30)

## 核心组件

### 1. 提示词模板模型

系统的核心数据结构是`PromptTemplate`模型，它定义了AI内容生成的模板规范：

```mermaid
classDiagram
class PromptTemplate {
+String id
+String name
+String description
+String template_type
+String agent_type
+String system_prompt_template
+String user_prompt_template
+Dict output_schema
+List variables_schema
+String default_agent_id
+Boolean is_active
+Boolean is_default
+DateTime created_at
+DateTime updated_at
}
class Agent {
+String id
+String name
+String provider_id
+String model
+Float temperature
+Integer context_window
+String system_prompt
+List tools
+Boolean thinking_mode
+Float input_credit_per_1m
+Float output_credit_per_1m
+Float image_output_credit_per_1m
+Float search_credit_per_query
}
class LLMProvider {
+String id
+String name
+String provider_type
+String api_key
+String base_url
+List models
+List tags
+Boolean is_active
+Boolean is_default
+Dict model_costs
}
PromptTemplate --> Agent : "关联"
Agent --> LLMProvider : "使用"
```

**图表来源**
- [models.py](file://backend/models.py#L287-L322)
- [models.py](file://backend/models.py#L167-L207)
- [models.py](file://backend/models.py#L118-L142)

### 2. 数据模型关系图

```mermaid
erDiagram
PROMPT_TEMPLATES {
String id PK
String name UK
String template_type
String agent_type
String system_prompt_template
String user_prompt_template
JSON output_schema
JSON variables_schema
String default_agent_id FK
Boolean is_active
Boolean is_default
DateTime created_at
DateTime updated_at
}
AGENTS {
String id PK
String name
String provider_id FK
String model
String agent_type
Float temperature
Integer context_window
String system_prompt
JSON tools
Boolean thinking_mode
Float input_credit_per_1m
Float output_credit_per_1m
Float image_output_credit_per_1m
Float search_credit_per_query
Boolean is_leader
JSON coordination_modes
JSON member_agent_ids
Integer max_subtasks
Boolean enable_auto_review
JSON gemini_config
DateTime created_at
DateTime updated_at
}
LLM_PROVIDERS {
String id PK
String name UK
String provider_type
String api_key
String base_url
JSON models
JSON tags
Boolean is_active
Boolean is_default
JSON config_json
JSON model_costs
DateTime created_at
DateTime updated_at
}
PROMPT_TEMPLATES }o--|| AGENTS : "default_agent_id"
AGENTS }o--|| LLM_PROVIDERS : "provider_id"
```

**图表来源**
- [models.py](file://backend/models.py#L287-L322)
- [models.py](file://backend/models.py#L167-L207)
- [models.py](file://backend/models.py#L118-L142)

**章节来源**
- [models.py](file://backend/models.py#L287-L322)
- [schemas.py](file://backend/schemas.py#L481-L519)

## 数据库设计

### 核心表结构

系统采用异步SQLAlchemy ORM进行数据库操作，主要数据表包括：

1. **PromptTemplate表** - 存储提示词模板定义
2. **Agent表** - 智能体配置信息
3. **LLMProvider表** - LLM提供商配置
4. **User表** - 用户信息和积分余额
5. **CreditTransaction表** - 积分交易记录

### 数据库连接配置

```mermaid
flowchart TD
START([应用启动]) --> CONFIG[加载配置]
CONFIG --> ENGINE[创建异步引擎]
ENGINE --> POOL[配置连接池]
POOL --> MIGRATE[运行数据库迁移]
MIGRATE --> READY[数据库就绪]
READY --> SESSION[创建会话]
SESSION --> QUERY[执行查询]
QUERY --> CLOSE[关闭会话]
CLOSE --> SESSION
```

**图表来源**
- [database.py](file://backend/database.py#L8-L23)
- [main.py](file://backend/main.py#L58-L62)

**章节来源**
- [database.py](file://backend/database.py#L1-L31)
- [main.py](file://backend/main.py#L48-L82)

## API接口设计

### 提示词模板管理API

系统提供完整的提示词模板管理RESTful API：

| 方法 | 路径 | 功能 | 权限 |
|------|------|------|------|
| GET | `/api/prompt-templates/` | 获取模板列表 | 用户/管理员 |
| GET | `/api/prompt-templates/{template_id}` | 获取单个模板 | 用户/管理员 |
| POST | `/api/prompt-templates/` | 创建新模板 | 管理员 |
| PUT | `/api/prompt-templates/{template_id}` | 更新模板 | 管理员 |
| DELETE | `/api/prompt-templates/{template_id}` | 删除模板 | 管理员 |
| POST | `/api/prompt-templates/{template_id}/generate` | 使用模板生成内容 | 用户/管理员 |

### API工作流程

```mermaid
sequenceDiagram
participant Client as 客户端
participant API as API路由器
participant DB as 数据库
participant LLM as LLM服务
participant Billing as 计费服务
Client->>API : POST /api/prompt-templates/{id}/generate
API->>DB : 获取模板和智能体配置
DB-->>API : 返回模板数据
API->>API : 渲染Jinja2模板
API->>LLM : 调用流式LLM接口
LLM-->>API : 返回流式响应
API->>API : 解析JSON响应
API->>Billing : 计算积分费用
Billing-->>API : 返回费用信息
API-->>Client : 返回生成结果
```

**图表来源**
- [prompt_templates.py](file://backend/routers/prompt_templates.py#L157-L275)
- [llm_stream.py](file://backend/services/llm_stream.py#L502-L551)

**章节来源**
- [prompt_templates.py](file://backend/routers/prompt_templates.py#L32-L154)
- [prompt_templates.py](file://backend/routers/prompt_templates.py#L157-L275)

## 前端界面

### 模板选择界面

前端使用Next.js构建了直观的模板选择界面：

```mermaid
flowchart LR
PAGE[创建剧场页面] --> LIST[模板列表组件]
PAGE --> DETAILS[模板详情组件]
LIST --> FILTER[分类筛选]
LIST --> SEARCH[搜索功能]
LIST --> GRID[网格布局]
DETAILS --> CONFIRM[确认按钮]
DETAILS --> BACK[返回按钮]
GRID --> CARD[模板卡片]
CARD --> SELECT[选择模板]
```

**图表来源**
- [page.tsx](file://frontend/src/app/create-theater/page.tsx#L11-L23)
- [TemplateList.tsx](file://frontend/src/components/create-theater/TemplateList.tsx#L18-L29)
- [TemplateDetails.tsx](file://frontend/src/components/create-theater/TemplateDetails.tsx#L15-L143)

### 模板数据结构

前端使用TypeScript定义了模板数据结构：

| 字段 | 类型 | 描述 |
|------|------|------|
| id | string | 模板唯一标识符 |
| category | TemplateCategory | 模板分类 |
| name | string | 模板名称 |
| description | string | 模板描述 |
| features | string[] | 特色元素列表 |
| scenarios | string[] | 适用场景列表 |
| color | string | 颜色主题 |
| iconName | string | 图标名称 |
| backgroundImage | string | 背景图片URL |

**章节来源**
- [page.tsx](file://frontend/src/app/create-theater/page.tsx#L1-L61)
- [TemplateList.tsx](file://frontend/src/components/create-theater/TemplateList.tsx#L1-L92)
- [TemplateDetails.tsx](file://frontend/src/components/create-theater/TemplateDetails.tsx#L1-L143)
- [data.ts](file://frontend/src/components/create-theater/data.ts#L1-L94)

## 服务层实现

### LLM流式调用服务

系统实现了统一的LLM流式调用服务，支持多种AI提供商：

```mermaid
classDiagram
class StreamContext {
+String provider_type
+String api_key
+String base_url
+String model
+messages[]
+Float temperature
+Integer context_window
+Boolean thinking_mode
+Dict~gemini_config~
}
class StreamResult {
+String full_response
+String reasoning_content
+Integer input_tokens
+Integer output_tokens
+Integer text_output_tokens
+Integer image_output_tokens
+Integer search_query_count
}
class LLMStreamService {
+Dict~provider_type, handler~ registry
+DEFAULT_BASE_URLS
+register_provider()
+stream_completion()
+get_effective_base_url()
}
LLMStreamService --> StreamContext : "创建"
LLMStreamService --> StreamResult : "返回"
```

**图表来源**
- [llm_stream.py](file://backend/services/llm_stream.py#L11-L35)
- [llm_stream.py](file://backend/services/llm_stream.py#L47-L53)

### 计费系统

系统实现了灵活的多维度计费系统：

```mermaid
flowchart TD
INPUT[生成请求] --> CALC[计算费用]
CALC --> DIM[维度映射]
DIM --> INPUT_COST[输入费用]
DIM --> TEXT_COST[文本输出费用]
DIM --> IMAGE_COST[图像输出费用]
DIM --> SEARCH_COST[搜索费用]
INPUT_COST --> TOTAL[TOTAL]
TEXT_COST --> TOTAL
IMAGE_COST --> TOTAL
SEARCH_COST --> TOTAL
TOTAL --> CHECK[余额检查]
CHECK --> SUFFICIENT{余额充足?}
SUFFICIENT --> |是| DEDUCT[扣除积分]
SUFFICIENT --> |否| ERROR[余额不足错误]
DEDUCT --> RECORD[记录交易]
RECORD --> SUCCESS[完成]
ERROR --> FAIL[失败]
```

**图表来源**
- [billing.py](file://backend/services/billing.py#L230-L269)

**章节来源**
- [llm_stream.py](file://backend/services/llm_stream.py#L1-L551)
- [billing.py](file://backend/services/billing.py#L1-L270)

## 安全机制

### 认证授权

系统实现了多层次的安全机制：

1. **JWT令牌管理** - 使用RSA算法进行令牌签名
2. **用户/管理员双角色** - 独立的用户和管理员认证体系
3. **权限控制** - 基于角色的访问控制(RBAC)
4. **密码加密** - 使用bcrypt进行密码哈希

### 安全流程

```mermaid
flowchart TD
LOGIN[用户登录] --> VERIFY[验证凭据]
VERIFY --> HASH[密码哈希验证]
HASH --> VALID{验证通过?}
VALID --> |是| TOKEN[生成JWT令牌]
VALID --> |否| ERROR[认证失败]
TOKEN --> STORE[存储令牌]
STORE --> ACCESS[访问受保护资源]
ACCESS --> CHECK[检查权限]
CHECK --> ROLE{用户角色}
ROLE --> |用户| USER_ACCESS[用户权限]
ROLE --> |管理员| ADMIN_ACCESS[管理员权限]
USER_ACCESS --> SUCCESS[访问成功]
ADMIN_ACCESS --> SUCCESS
```

**图表来源**
- [auth.py](file://backend/auth.py#L30-L74)
- [auth.py](file://backend/auth.py#L119-L156)

**章节来源**
- [auth.py](file://backend/auth.py#L1-L229)

## 部署配置

### 环境配置

系统使用Pydantic Settings进行配置管理：

| 配置项 | 默认值 | 用途 |
|--------|--------|------|
| DATABASE_URL | sqlite:///infinite_theater.db | 数据库连接字符串 |
| JWT_SECRET_KEY | change-me-in-production | JWT密钥 |
| ACCESS_TOKEN_EXPIRE_MINUTES | 30 | 访问令牌过期时间 |
| REFRESH_TOKEN_EXPIRE_DAYS | 7 | 刷新令牌过期时间 |
| OPENAI_API_KEY | 空 | OpenAI API密钥 |
| CLAUDE_API_KEY | 空 | Claude API密钥 |
| GEMINI_API_KEY | 空 | Gemini API密钥 |

### 依赖管理

系统使用pip进行依赖管理，主要依赖包括：

- **Web框架**: FastAPI 0.129.0+, Uvicorn 0.41.0+
- **数据库**: SQLAlchemy 2.0.46+, asyncpg 0.31.0+, aiosqlite 0.19.0
- **AI服务**: agentscope 1.0.16+, openai 2.21.0+, google-genai 1.65.0
- **认证**: bcrypt 4.0.0+, python-jose 3.3.0
- **前端**: Next.js 14+, React 18+

**章节来源**
- [config.py](file://backend/config.py#L1-L40)
- [requirements.txt](file://backend/requirements.txt#L1-L24)

## 总结

提示词模板管理系统是一个功能完整、架构清晰的AI内容生成平台。系统的主要特点包括：

1. **模块化设计** - 采用分层架构，职责分离明确
2. **多模态支持** - 支持文本、图像等多种AI模型
3. **灵活的模板系统** - 提供多种预定义模板类型
4. **完善的权限管理** - 用户和管理员双角色体系
5. **智能计费系统** - 基于使用量的精确计费
6. **前后端分离** - 现代化的技术栈组合

该系统为剧场开发者和内容创作者提供了一个强大而易用的AI辅助工具，能够显著提高内容创作效率和质量。通过合理的架构设计和安全机制，系统能够在保证性能的同时确保数据安全和用户体验。