...
- 前端（Next.js）：游戏客户端，包含 Pixi.js 画布组件用于基础图形渲染。
...
subgraph "前端"
F_client["GameCanvas.tsx<br/>Pixi.js 画布组件"]
end
subgraph "后台管理"
...
- 数据模型与服务
  ...
  - GameService：封装世界初始化、章节生成与一致性检查流程。
- 路由与配置
...
## 架构总览
系统采用分层架构：API 层（FastAPI）、服务层（GameService）、数据层（SQLAlchemy 异步 ORM）、智能体层（AgentScope）。WebSocket 用于实时推送，后台管理通过 REST API 管理 LLM 与智能体配置。
...
## 依赖关系分析
- 组件耦合
  - GameService 依赖 NarrativeEngine 与数据库；NarrativeEngine 依赖 LLMProvider 与 AgentScope。
...
```mermaid
graph LR
Service["GameService"] --> Engine["NarrativeEngine"]
