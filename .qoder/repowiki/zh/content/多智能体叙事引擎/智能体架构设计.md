...
该系统以 FastAPI 作为后端入口，结合 SQLAlchemy 异步 ORM 与 Alembic 迁移工具，构建可动态切换 LLM 提供商的叙事引擎，支撑“无限剧情”游戏的动态世界生成与交互。

## 项目结构
后端采用分层与按功能模块划分的组织方式：
- 配置层：读取环境变量与默认参数，统一管理数据库与外部服务地址
- 数据访问层：定义 ORM 模型、异步引擎与会话工厂
- 业务服务层：封装游戏世界初始化、章节生成等业务逻辑
...
## 核心组件
- DialogAgent：继承自 AgentScope 的 AgentBase，负责对话与记忆管理，遵循“system/user/assistant”的角色映射规则，将历史消息序列化为模型输入。
...
- GameService：业务服务层，负责玩家创建、世界初始化与章节生成，调用 NarrativeEngine 完成故事生成。
...
- 章节生成：GameService 调用 NarrativeEngine 的流水线生成章节内容，并将结果持久化到 StoryChapter 表
...
章节来源
- [backend/agents.py](file://backend/agents.py#L66-L125)
...
## 结论
本项目以 AgentScope 为核心，结合 FastAPI、SQLAlchemy 与 Alembic，构建了可动态配置、可扩展的智能体叙事引擎。通过 LLMProvider 的集中管理与热重载机制，实现了对多种 LLM 提供商的无缝切换；通过 DialogAgent 的消息上下文与多智能体协作，完成了从世界观到章节内容的自动化生成。建议在生产环境中进一步完善缓存策略、上下文截断与可观测性指标，以提升稳定性与性能。
...
- 连接测试（POST /api/admin/llm-providers/test-connection）
  ...
  - 示例路径：[backend/routers/llm_config.py](file://backend/routers/llm_config.py#L20-L111)
