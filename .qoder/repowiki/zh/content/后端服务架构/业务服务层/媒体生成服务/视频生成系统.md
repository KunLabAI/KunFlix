...
## 简介
本系统提供统一的AI视频生成服务，支持多家供应商（xAI、MiniMax、Gemini Veo、**Ark Seedance**），通过适配器模式屏蔽供应商差异，向上提供一致的提交与轮询接口。系统包含完整的任务生命周期管理：任务提交、状态轮询、结果获取、计费与本地媒体落盘、以及与前端的集成。新增的视频编辑和视频扩展功能进一步丰富了视频生成能力。

**更新** 新增的Ark Seedance Provider适配器支持火山方舟的Seedance系列视频生成模型，包括2.0、1.5、1.0等多个版本，提供丰富的视频生成能力。

## 项目结构
- **服务层**
  - 视频生成统一入口：[video_generation.py](file://backend/services/video_generation.py)
  - 供应商适配器：[base.py](file://backend/services/video_providers/base.py)、[xai_provider.py](file://backend/services/video_providers/xai_provider.py)、[minimax_provider.py](file://backend/services/video_providers/minimax_provider.py)、[gemini_provider.py](file://backend/services/video_providers/gemini_provider.py)、**[ark_provider.py](file://backend/services/video_providers/ark_provider.py)**
...
AP["ark_provider.py<br/>Ark Seedance适配器"]
end
subgraph "路由层"
...
- [models.py:403-491](file://backend/models.py#L403-L491)
...
- **适配器基类与注册**
  ...
  - 注册表 PROVIDER_REGISTRY 将字符串键映射到适配器类，支持动态扩展（**新增ark供应商**）
- **增强的数据模型**
...
- **扩展的模型能力系统**
  ...
  - **新增Ark Seedance系列模型能力配置**
- **管理员功能**
  - 提供视频模型能力查询接口
  - 支持视频生成任务的管理与监控

**章节来源**
- [video_generation.py:85-180](file://backend/services/video_generation.py#L85-L180)
- [base.py:56-121](file://backend/services/video_providers/base.py#L56-L121)
- [models.py:403-491](file://backend/models.py#L403-L491)
- [model_capabilities.py:9-477](file://backend/services/video_providers/model_capabilities.py#L9-L477)
...
participant ADP as "适配器(xAI/MiniMax/Gemini/Ark)"
...
- [ark_provider.py:161-203](file://backend/services/video_providers/ark_provider.py#L161-L203)
...
- **infer_provider_type(model)**
  - 根据模型名特征推断供应商类型（优先级：Gemini Veo > MiniMax > xAI > **Ark**）
...
**图表来源**
- [video_generation.py:85-121](file://backend/services/video_generation.py#L85-L121)
- [video_generation.py:134-180](file://backend/services/video_generation.py#L134-L180)
...
- **具体适配器实现**
  ...
  - ****Ark Seedance 适配器**（新增）**
    ...
    - **支持模型版本**：2.0（fast）、1.5 Pro、1.0 Pro（fast）、1.0 Lite（T2V、I2V）
    - **支持模式**：text_to_video、image_to_video、reference_images、video_extension
    - **状态映射**：queued/pending/running->pending/processing，succeeded->completed，failed/expired->failed
    - **高级功能**：支持首尾帧、参考图片、有声视频（2.0系列）、快速版模型
    - **增强错误处理**：支持多种错误格式提取（error字典、字符串、error_message、message字段）
...
- [ark_provider.py:55-300](file://backend/services/video_providers/ark_provider.py#L55-L300)
...
- ****Ark Seedance 模型**（新增）**
  - **doubao-seedance-2-0-260128**：2.0标准版，支持多模态参考、有声视频
  - **doubao-seedance-2-0-fast-260128**：2.0快速版，同等能力
  - **doubao-seedance-1-5-pro-251215**：1.5专业版，支持首尾帧、有声视频
  - **doubao-seedance-1-0-pro-250801**：1.0专业版，支持首尾帧
  - **doubao-seedance-1-0-pro-fast-250801**：1.0快速版，首帧+T2V
  - **doubao-seedance-1-0-lite-t2v**：1.0精简版，纯文本生成
  - **doubao-seedance-1-0-lite-i2v**：1.0精简版，I2V+参考图

### 能力配置结构
每个模型都定义了详细的配置信息：
- **基本能力**：支持的模式、时长范围、分辨率
- **高级功能**：首尾帧支持、参考图片支持、视频扩展支持、有声视频支持
...
- [ark_provider.py:1-300](file://backend/services/video_providers/ark_provider.py#L1-L300)
- [models.py:403-491](file://backend/models.py#L403-L491)
- [schemas.py:636-682](file://backend/schemas.py#L636-L682)
- [admin_tools.py:206-213](file://backend/routers/admin_tools.py#L206-L213)
- [7459f2d26782_add_video_tasks_and_video_agent_fields.py:21-84](file://backend/migrations/versions/7459f2d26782_add_video_tasks_and_video_agent_fields.py#L21-L84)
...
- ****Ark Seedance 错误处理**（新增）**
  - **检查错误格式**：支持 error字典、字符串、error_message、message等多种格式
  - **默认错误消息**：当状态为failed但无错误信息时，设置"Video generation failed (unknown reason)"
...
- [ark_provider.py:265-289](file://backend/services/video_providers/ark_provider.py#L265-L289)

## 结论
本系统通过适配器模式实现了多供应商视频生成的统一接入，具备良好的扩展性与稳定性。新增的视频编辑、视频扩展功能和完整的数据库Schema支持，使得系统能够处理更复杂的视频生成场景。**新增的Ark Seedance Provider适配器进一步扩展了系统支持的模型生态，包括多个版本的Seedance模型，提供丰富的视频生成能力。**统一入口函数与路由层配合，提供了从任务提交到结果获取的完整闭环；计费与媒体落盘保障了业务闭环与用户体验。建议在生产环境中结合监控与告警完善可观测性，并持续扩展更多供应商适配器。
...
### **Ark Seedance 模型使用示例**（新增）
- **支持的模型名称**
  - doubao-seedance-2-0-260128：2.0标准版，支持多模态参考、有声视频
  - doubao-seedance-2-0-fast-260128：2.0快速版
  - doubao-seedance-1-5-pro-251215：1.5专业版，支持首尾帧、有声视频
  - doubao-seedance-1-0-pro-250801：1.0专业版，支持首尾帧
  - doubao-seedance-1-0-pro-fast-250801：1.0快速版，首帧+T2V
  - doubao-seedance-1-0-lite-t2v：1.0精简版，纯文本生成
...
- **请求参数**
  - 支持 reference_images（最多9张，2.0系列）
  - 支持 extension_video_url（2.0系列）
  - 支持 last_frame_image（支持首尾帧的模型）
  - 支持 generate_audio（2.0系列）

**章节来源**
- [ark_provider.py:94-159](file://backend/services/video_providers/ark_provider.py#L94-L159)
- [model_capabilities.py:351-457](file://backend/services/video_providers/model_capabilities.py#L351-L457)