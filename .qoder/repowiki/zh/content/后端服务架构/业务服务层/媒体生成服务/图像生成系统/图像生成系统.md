...
## 更新摘要
**所做更改**
- 新增完整的Gemini Nano Banana图像生成功能，支持文本到图像、图像编辑和多轮编辑
- 扩展图像生成工具，新增Gemini供应商支持，实现统一配置适配
- 增强批量图像生成服务，支持Gemini的并行调用和配置管理
- 新增Gemini配置迁移支持，包含thinking_level和image_config字段
- 扩展前端配置类型，支持Gemini的思考模式和搜索功能
- 新增完整的多语言示例文档，涵盖Python、JavaScript、Go、Java、REST API
...
## 简介
本项目是一个基于FastAPI的图像生成系统，现已重构为支持统一配置系统的多供应商图像生成服务。系统采用统一的UnifiedImageGenConfig配置格式，支持动态供应商能力检测，提供统一的工具接口、批量处理能力、质量控制与成本计算功能。系统现已新增完整的Gemini Nano Banana图像生成功能，支持文本到图像、图像编辑、多轮图像编辑等高级功能，涵盖多种编程语言的完整示例。

**更新** 新增Gemini Nano Banana图像生成功能，支持文本到图像、图像编辑、多轮编辑，扩展了统一配置框架以支持全局工具级别配置管理，新增完整的多语言示例文档。
...
- **Gemini图像生成功能**：新增完整的Gemini Nano Banana图像生成功能，支持文本到图像、图像编辑、多轮编辑
...
participant Service as "批量图像生成服务<br/>batch_image_gen.py"
participant Gemini as "Gemini批量生成<br/>batch_image_gen.py"
participant Media as "媒体工具<br/>media_utils.py"
participant Billing as "计费服务<br/>billing.py"
Client->>Router : 提交批量图像生成请求
Router->>Service : batch_generate_images(ctx)
Service->>Gemini : _batch_generate_gemini(ctx)
Gemini->>Gemini : 并行生成图片
Gemini-->>Service : 返回批量结果
...
### Nano Banana图像生成模型
- **功能概述**：Gemini Nano Banana是Gemini原生图像生成功能的统称，支持文本到图像、图像编辑、多轮编辑等高级功能
- **关键特性**：
...
## 结论
该图像生成系统通过重构统一配置系统和引入动态供应商能力检测，实现了更加灵活和可扩展的多供应商统一接入。系统现已扩展支持图像编辑功能，与图像生成共享全局配置，进一步增强了系统的实用性。新增的Gemini Nano Banana图像生成功能提供了完整的文本到图像、图像编辑、多轮编辑能力，支持多种编程语言的示例实现。统一配置适配器与计费服务进一步提升了系统的可维护性与可控性。新的架构支持更好的用户体验，通过前端动态能力检测提供智能配置界面。新增的ImageGenConfigDialog组件提供了直观的配置界面，支持动态供应商选择和实时能力展示。工具管理器的引入实现了统一的工具协调和缓存机制。建议在生产环境中合理设置并发参数、监控供应商限流与内容审核策略，并定期清理本地媒体文件以控制存储空间。
