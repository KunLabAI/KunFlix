...
**更新** 新架构引入了全新的ToolManager架构，包括ToolProvider协议、ToolContext上下文系统，以及统一的工具管理器实现。系统现在支持动态能力检测和统一配置接口，为图像生成工具提供了更加灵活的供应商管理机制。
...
**更新** 新架构中的ToolManager提供了更强大的工具定义缓存和重建机制，支持动态能力检测和按需更新。
...
**更新** 新架构中的ToolContext提供了延迟解析的缓存机制，包括技能目录和图像供应商类型的动态解析。
...
**更新** 新架构引入了动态工具定义构建和重建机制，支持根据上下文条件动态启用或禁用工具。
...
**更新** 新架构中的ImageGenProvider引入了动态能力检测机制，支持根据图像供应商类型动态调整工具定义参数，包括宽高比枚举和批量生成限制。
...
**更新** 新架构引入了动态能力检测，根据图像供应商类型调整工具定义参数，包括宽高比枚举和批量生成限制。
...
**更新** 新架构中的管理员路由现在支持图像供应商能力检测，提供每个供应商支持的参数选项。
...
**更新** 新架构中的日志系统提供了更精细的工具名称推断机制，支持不同工具类型的分类记录。
...
**更新** 新架构中的依赖关系更加清晰，ToolManager作为中央协调器，ToolContext提供统一的上下文传递，ToolProvider协议确保了良好的抽象性。
...
**更新** 新架构引入了工具定义缓存和按需重建机制，显著提升了工具定义构建的性能。
...
**更新** 新架构引入的ToolManager、ToolProvider协议和ToolContext系统，为Infinite Game项目提供了更加灵活和强大的工具管理能力。系统现在支持动态能力检测、统一配置接口和高效的缓存机制，能够满足复杂的创作工作流和多样化的工具需求。通过持续的优化和扩展，系统能够适应不断增长的功能需求和技术挑战。