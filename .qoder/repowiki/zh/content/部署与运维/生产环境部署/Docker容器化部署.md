...
## 简介
本文件为 KunFlix 提供完整的 Docker 容器化部署方案，覆盖 Python 后端与 Next.js 前端的分离构建、多阶段镜像优化、服务编排、网络与卷挂载、环境变量与健康检查、日志管理以及容器安全最佳实践。目标是帮助运维与开发团队以可重复、可扩展的方式部署该系统。
...
## 结论
通过分离构建与多阶段优化，结合合理的环境变量、健康检查与日志策略，KunFlix 可实现稳定、可扩展的容器化部署。建议在生产环境中启用 PostgreSQL、外部 Redis、严格的密钥管理与非 root 用户运行，并配合持久化卷与网络隔离以满足安全与可靠性要求。
