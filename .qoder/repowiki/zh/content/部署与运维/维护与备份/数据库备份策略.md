...
## 引言
本文件面向KunFlix项目的数据库备份与恢复需求，基于现有代码库中的数据库配置、迁移机制与启动流程，制定一套可落地的备份策略。重点涵盖：
...
## 项目结构
KunFlix后端采用异步SQLAlchemy引擎与Alembic迁移框架，数据库默认使用SQLite（本地开发友好），生产环境可通过配置切换至PostgreSQL。迁移脚本位于migrations目录，启动时可按需执行迁移。
