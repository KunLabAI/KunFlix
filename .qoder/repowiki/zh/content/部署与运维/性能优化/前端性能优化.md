...
## 简介
本文件面向 Infinite Game 前端（Next.js）应用，系统性梳理并给出性能优化策略，覆盖以下方面：
...
## 结论
通过合理的构建配置、组件级 memo 化、虚拟滚动、图片懒加载与节流/防抖策略，Infinite Game 前端可在保证交互流畅的同时，有效降低首屏与滚动成本。建议持续引入 Web Vitals 监控与性能预算管理，形成闭环优化机制。
