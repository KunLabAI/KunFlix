...
## 更新摘要
**变更内容**
- 新增上下文压缩相关数据库迁移，包括ChatSession、AdminDebugSession、Agent模型的字段变更
- 添加compressed_summary和compressed_before_id字段到chat_sessions表
- 添加compressed_summary和compressed_before_id字段到admin_debug_sessions表  
- 添加compaction_config字段到agents表
- 实现智能体级别的上下文压缩配置管理
- 支持会话级别的上下文压缩与摘要持久化
- 提供管理员调试会话的独立上下文压缩功能
...
**更新** 新增上下文压缩功能，通过compressed_summary和compressed_before_id字段实现对话历史的智能压缩与摘要持久化，支持会话级别的上下文管理与性能优化。
...
- 服务层：billing.py 实现计费与积分原子化扣减/退款；theater.py 实现剧场画布的批量同步；context_compaction.py 实现上下文压缩功能。
...
R3["路由层<br/>routers/admin_debug.py"]
end
subgraph "服务层"
S1["服务层<br/>services/theater.py"]
S2["服务层<br/>services/billing.py"]
S3["服务层<br/>services/context_compaction.py"]
end
subgraph "数据层"
...
V5["上下文压缩迁移<br/>versions/349404aa5c3c_add_context_compaction_to_chat_sessions.py"]
V6["管理员调试压缩迁移<br/>versions/4c1a8970b23d_add_compaction_to_admin_debug_sessions.py"]
V7["智能体压缩配置迁移<br/>versions/909762287e70_add_compaction_config_to_agent.py"]
V8["管理员调试会话迁移<br/>versions/4d66cc052bfb_add_admin_debug_sessions.py"]
end
R1 --> S1
R2 --> S2
R3 --> S3
...
- [backend/routers/admin_debug.py:1-593](file://backend/routers/admin_debug.py#L1-L593)
...
- [backend/models.py:1-502](file://backend/models.py#L1-L502)
...
- [backend/migrations/versions/4d66cc052bfb_add_admin_debug_sessions.py:1-68](file://backend/migrations/versions/4d66cc052bfb_add_admin_debug_sessions.py#L1-L68)
...
- 智能体（agents）
  ...
  - **新增**：上下文压缩配置（compaction_config），支持智能体级别的压缩参数设置
...
- 聊天会话（chat_sessions）
  ...
  - **新增**：compressed_summary 字段，存储旧消息的LLM摘要
  - **新增**：compressed_before_id 字段，标识此ID之前（含）的消息已被摘要覆盖
...
- 管理员调试会话（admin_debug_sessions）
  - 主键：字符串 UUID
  - 外键：agent_id → agents.id；admin_id → admins.id
  - **新增**：compressed_summary 字段，存储旧消息的LLM摘要
  - **新增**：compressed_before_id 字段，标识此ID之前（含）的消息已被摘要覆盖

- 管理员调试消息（admin_debug_messages）
  - 主键：字符串 UUID
  - 外键：session_id → admin_debug_sessions.id（带索引）

**章节来源**
- [backend/models.py:10-502](file://backend/models.py#L10-L502)
...
- 历史迁移示例
  ...
  - **新增** 上下文压缩迁移：为 chat_sessions 表添加 compressed_summary 和 compressed_before_id 字段
  - **新增** 管理员调试压缩迁移：为 admin_debug_sessions 表添加 compressed_summary 和 compressed_before_id 字段
  - **新增** 智能体压缩配置迁移：为 agents 表添加 compaction_config JSON 字段
  - **新增** 管理员调试会话迁移：创建 admin_debug_sessions 和 admin_debug_messages 表
...
### 上下文压缩功能
**新增** 智能体级别的上下文压缩功能，通过compressed_summary和compressed_before_id字段实现对话历史的智能压缩与摘要持久化：

- 字段定义：
  - compressed_summary：Text类型，存储旧消息的LLM摘要
  - compressed_before_id：String类型，标识此ID之前（含）的消息已被摘要覆盖
- 数据来源：从聊天结果对象中提取input_tokens和output_tokens字段
- 更新机制：每次聊天完成后累加到现有值，确保数据完整性
- 应用场景：会话级别的token使用统计、计费审计、性能监控
- 配置管理：通过compaction_config字段存储智能体级别的压缩参数
...
participant Service as "服务层"
participant DB as "数据库"
participant Session as "ChatSession/AdminDebugSession"
Router->>Service : 处理聊天请求
Service->>DB : 执行LLM调用
DB-->>Service : 返回聊天结果
Service->>Session : 累加token使用量
Service->>DB : 更新compressed_summary和compressed_before_id字段
DB-->>Service : 更新成功
Service-->>Router : 返回响应
```
...
- [backend/models.py:181-196](file://backend/models.py#L181-L196)
- [backend/models.py:443-458](file://backend/models.py#L443-L458)
...
### 管理员调试会话功能
**新增** 独立的管理员调试会话功能，支持管理员对智能体进行调试和测试：

- 会话隔离：管理员调试会话与普通用户会话完全隔离
- 独立存储：admin_debug_sessions和admin_debug_messages表独立管理
- 权限控制：每个会话绑定特定管理员和智能体
- 上下文压缩：支持与普通会话相同的上下文压缩功能
- 调试工具：提供专门的API接口进行会话管理和消息查询

**章节来源**
- [backend/models.py:443-470](file://backend/models.py#L443-L470)
- [backend/routers/admin_debug.py:1-593](file://backend/routers/admin_debug.py#L1-L593)
...
- 索引与查询
  ...
  - **新增** 为compressed_summary和compressed_before_id字段建立索引，支持上下文压缩的快速查询
- 事务与锁
  - 计费采用原子更新与交易记录，避免竞态条件
  - 画布同步使用集合运算与批量 DML，减少锁竞争
  - **新增** token使用统计采用原子累加操作，确保并发安全性
  - **新增** 上下文压缩采用原子更新操作，确保摘要数据的一致性
- 缓存策略
...
- 迁移相关
  ...
  - **新增** 上下文压缩迁移问题：确认compressed_summary和compressed_before_id字段已正确添加到chat_sessions和admin_debug_sessions表
  - **新增** 智能体压缩配置迁移问题：确认compaction_config字段已正确添加到agents表
- 计费异常
...
- 数据一致性
  ...
  - **新增** 上下文压缩异常：检查context_compaction服务中的摘要生成和存储逻辑
- 运维工具
...
- **新增** 上下文压缩功能故障排查
  - 摘要生成失败：检查LLM提供商配置和API密钥
  - 压缩配置无效：确认智能体的compaction_config字段配置正确
  - 会话数据不一致：检查compressed_before_id的递增逻辑
...
**更新** 新增的SQLite特定优化配置显著提升了数据库的并发性能，通过WAL模式、busy_timeout和synchronous PRAGMA设置有效减少了'database is locked'错误，为高并发场景提供了更好的稳定性。新增的聊天会话token使用跟踪功能增强了系统的可观测性，为计费审计和性能监控提供了可靠的数据支持。新增的上下文压缩功能通过compressed_summary和compressed_before_id字段实现了对话历史的智能压缩，显著提升了大上下文场景下的性能表现。新增的管理员调试会话功能提供了独立的调试环境，支持更灵活的开发和测试工作流。建议在生产环境中进一步完善权限控制、审计日志与备份恢复方案，持续优化查询与事务设计。
...
- **新增** 上下文压缩配置
  - compressed_summary字段：Text类型，存储旧消息的LLM摘要
  - compressed_before_id字段：String类型，标识被压缩消息的边界
  - compaction_config字段：JSON类型，存储智能体级别的压缩参数
  - 默认配置：enabled=True, compact_ratio=0.75, reserve_ratio=0.15
  - 工具结果截断：支持对tool角色消息的内容进行智能截断
  - 摘要生成：使用LLM生成简洁的对话摘要，支持多轮对话的长期记忆
...
- [backend/models.py:181-196](file://backend/models.py#L181-L196)
- [backend/models.py:264-266](file://backend/models.py#L264-L266)
- [backend/services/context_compaction.py:25-35](file://backend/services/context_compaction.py#L25-L35)
- [backend/services/context_compaction.py:97-115](file://backend/services/context_compaction.py#L97-L115)
- [backend/services/context_compaction.py:167-203](file://backend/services/context_compaction.py#L167-L203)