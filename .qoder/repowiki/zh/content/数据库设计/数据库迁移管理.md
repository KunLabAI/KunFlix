...
- [o2p3q4r5s6t7_rebuild_assets_table.py](file://backend/migrations/versions/o2p3q4r5s6t7_rebuild_assets_table.py)
- [p4q5r6s7t8u9_add_tool_executions_table.py](file://backend/migrations/versions/p4q5r6s7t8u9_add_tool_executions_table.py)
</cite>

## 更新摘要
**变更内容**
- 新增 ToolExecution 模型和对应的数据库迁移
- 更新数据库模式设计文档以反映新的工具执行日志功能
- 添加工具执行表的详细设计说明和索引策略
- 更新迁移历史和版本控制策略
...
H["ToolExecution 模型<br/>工具执行日志"] --> D
I["p4q5r6s7t8u9 迁移<br/>新增工具执行表"] --> B
```
...
**更新** 新增 ToolExecution 模型，用于记录工具执行的详细信息，包括工具名称、提供者、执行状态、错误信息等。
...
- **新增** 工具执行表迁移（p4q5r6s7t8u9）：新增ToolExecution表，记录工具调用的完整生命周期信息
...
### 新增工具执行表设计
**更新** 新增ToolExecution表用于记录工具调用的详细信息，支持工具执行监控、调试和审计需求。

#### 表结构设计
- 主键：id (String(36))
- 工具标识：tool_name (String(100)), provider_name (String(50))
- 关联信息：agent_id (String(36)), session_id (String(36)), user_id (String(36))
- 执行上下文：is_admin (Boolean), theater_id (String(36))
- 参数与结果：arguments (JSON), result_summary (Text), error_message (Text)
- 执行状态：status (String(20)), duration_ms (Integer)
- 时间戳：created_at (DateTime)

#### 索引策略
- 主键索引：id
- 查询优化索引：tool_name, provider_name, agent_id, session_id, user_id, status, created_at
- 外键约束：agent_id -> agents.id
...
- **新增** 模型依赖
  - ToolExecution模型依赖agents表的外键关联
  - 迁移文件依赖SQLAlchemy的op接口进行DDL操作
...
- **新增** 工具执行表性能：为高频查询字段建立索引，支持JSON字段的查询优化
...
- **新增** 工具执行表相关问题
  - 现象：ToolExecution表创建失败或索引创建异常
  - 处理：检查agents表是否存在，确认外键约束正确；验证JSON字段权限
...
## 结论
Infinite Game的数据库迁移体系以Alembic为核心，结合异步SQLAlchemy与批量DDL渲染，实现了对SQLite与PostgreSQL的兼容支持，并通过管理器封装了常见的迁移操作。项目在关键迁移中体现了对数据完整性、外键一致性与临时表清理的重视。

**更新** 最新版本新增了ToolExecution表，为工具执行提供了完整的日志记录能力，支持工具调用的监控、调试和审计需求。这一功能增强了系统的可观测性和可维护性。

建议在生产环境中遵循严格的版本控制、回滚策略与备份机制，确保迁移过程可控、可追溯、可恢复。
...
- **新增** 工具执行表监控
  - 定期检查ToolExecution表的数据量和索引使用情况
  - 设置适当的归档策略，避免表过大影响性能

### 工具执行表使用指南
**新增** ToolExecution表的使用场景和最佳实践：

- **监控与审计**
  - 记录所有工具调用的详细信息，便于问题排查
  - 跟踪工具执行成功率和性能指标
- **调试支持**
  - 提供完整的参数和结果记录，支持问题复现
  - 区分管理员和普通用户的工具调用
- **性能分析**
  - 通过duration_ms字段分析工具执行性能
  - 识别慢查询和性能瓶颈
- **数据清理**
  - 定期清理过期的执行记录，控制表大小
  - 建议保留30-90天的历史数据
