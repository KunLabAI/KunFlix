...
## 项目结构
后端采用 FastAPI + SQLAlchemy 异步 ORM 架构，围绕“叙事引擎”与“游戏服务”两大核心模块组织业务逻辑；数据库通过 Alembic 进行迁移管理；前端采用 Next.js（不在本指南重点范围内）。调试相关的入口包括：
...
- 游戏服务
  - GameService 将世界构建、初始章节生成与后续章节预生成串联起来，调用叙事引擎并持久化结果
- 数据层
...
participant Service as "GameService<br/>services.py"
...
- 断点位置
  ...
  - GameService.init_world/generate_chapter
- 跟踪要点
