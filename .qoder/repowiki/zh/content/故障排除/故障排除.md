...
## 简介
本指南面向系统管理员与开发者，聚焦于“无限叙事游戏”项目的故障排除与运维。内容覆盖网络连接、数据库连接、WebSocket 断开、智能体运行异常、API 调用错误、前端渲染问题、系统监控与告警、备份恢复与回滚、跨平台兼容与依赖冲突等。文档以实际源码为依据，提供症状、原因分析、定位步骤与修复建议，并辅以可视化图示帮助快速定位问题。
