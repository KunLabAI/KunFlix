...
L["layout.tsx<br/>根布局"]
P["page.tsx<br/>首页"]
C["GameCanvas.tsx<br/>Pixi.js 画布"]
...
- 首页页面：条件渲染登录表单与游戏区域；动态加载 Pixi.js 画布；通过自定义 Hook 管理 WebSocket。
- 游戏画布组件：客户端侧初始化 Pixi.js Application，动态挂载 Canvas 并清理资源。
...
## 架构总览
前端通过 Next.js 提供 SSR/CSR 混合渲染，页面按需加载组件；游戏画布采用动态导入以避免 SSR 环境缺失 DOM；WebSocket 用于实时通信；后端提供 REST 与 WebSocket 接口。

```mermaid
sequenceDiagram
participant U as "用户"
participant P as "首页(page.tsx)"
participant C as "画布(GameCanvas)"
