...
## 更新摘要
**所做更改**
- 新增用户认证系统，将 Player 模型替换为 User 模型，引入完整的认证和授权机制
- 新增用户角色管理，支持普通用户和管理员双角色体系
- 新增用户表结构，包含邮箱、昵称、密码哈希、社交登录标识等认证字段
- 新增用户状态管理，包括激活状态、资金冻结状态、订阅状态等
- 新增用户操作统计，包括输入输出 token 统计、字符统计、登录信息等
- 更新前端界面，将玩家管理页面重命名为用户管理页面
- 新增用户认证路由和相关服务，支持注册、登录、令牌刷新等功能
- 新增用户认证中间件，支持 JWT 令牌验证和用户权限管理
...
## 简介
本文件面向数据库模型设计，围绕 SQLAlchemy 异步 ORM 的模型定义、字段类型选择、关系映射展开；重点阐释 User 与 StoryChapter 模型的字段、约束与索引设计；并系统讲解数据库迁移机制、Alembic 版本管理与数据模型演进策略；最后给出数据验证规则、业务规则实现与查询优化技巧，并提供模型扩展指南与常见数据操作模式。

**更新** 本次更新反映了应用的重大功能变更，包括新增用户认证系统、玩家到用户的术语更新、双角色管理体系、完整的用户状态管理以及相关的前端界面重构。

## 项目结构
后端以 FastAPI + SQLAlchemy 异步 ORM 架构为核心，数据库层通过 async/await 实现高并发读写；迁移体系基于 Alembic，提供脚本化版本管理与批量渲染能力；模型文件集中定义数据结构，路由层负责业务校验与数据流编排。新增的用户认证系统通过专门的认证路由、服务层和前端界面实现完整的用户管理功能，包括注册、登录、令牌管理和权限控制。
...
M_UUID["UUID迁移<br/>versions/a3b8c9d0e1f2_convert_ids_to_uuid.py"]
M_USER["用户认证迁移<br/>versions/b5d7e2f8a1c3_player_to_user_auth.py"]
...
FRONTEND["前端界面<br/>admin/src/app/admin/*"]
end
CFG --> DB
DB --> MODELS
AUTH --> MODELS
...
- [backend/routers/auth.py:1-136](file://backend/routers/auth.py#L1-L136)
- [backend/admin/src/app/admin/players/page.tsx:1-450](file://backend/admin/src/app/admin/players/page.tsx#L1-L450)
...
- **新增** 用户认证系统：完整的用户注册、登录、令牌管理和权限控制机制。
- **新增** 双角色管理体系：支持普通用户和管理员两种角色，分别对应不同的数据访问权限。
...
**更新** 新增了完整的用户认证系统和双角色管理体系，将原有的 Player 模型替换为 User 模型，并重构了相关的前端界面。
...
## 架构总览
异步 ORM 架构围绕"配置 → 引擎 → 会话 → 模型 → 迁移"展开，路由层与服务层通过异步会话与模型交互，实现数据持久化与业务编排。新增的用户认证系统通过专门的认证路由模块提供完整的用户管理功能，支持注册、登录、令牌刷新和权限验证。前端界面通过专门的用户管理页面实现用户信息展示、积分管理和订阅管理功能。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant AuthRouter as "认证路由<br/>routers/auth.py"
participant AuthService as "认证服务<br/>auth.py"
participant Session as "异步会话<br/>database.py"
participant UserModel as "用户模型<br/>models.py"
participant AdminRouter as "管理路由<br/>routers/admin.py"
participant Frontend as "用户管理界面<br/>admin/src/app/admin/players/page.tsx"
participant Alembic as "迁移系统<br/>migrations/*"
Client->>Frontend : "用户管理界面"
Frontend->>AuthRouter : "用户认证请求"
AuthRouter->>AuthService : "JWT令牌验证"
AuthService->>Session : "获取异步会话"
Session-->>AuthService : "AsyncSession"
AuthService->>UserModel : "用户信息查询"
UserModel-->>AuthService : "用户对象"
AuthService-->>AuthRouter : "令牌验证结果"
AuthRouter-->>Frontend : "认证响应"
Frontend-->>Client : "用户管理页面"
Note over Alembic,UserModel : "模型变更通过 Alembic 生成并应用迁移"
```
...
- [backend/auth.py:1-229](file://backend/auth.py#L1-L229)
- [backend/database.py:1-31](file://backend/database.py#L1-L31)
- [backend/models.py:35-79](file://backend/models.py#L35-L79)
- [backend/admin/src/app/admin/players/page.tsx:1-450](file://backend/admin/src/app/admin/players/page.tsx#L1-L450)
- [backend/migrations/env.py:1-105](file://backend/migrations/env.py#L1-L105)
...
- 字段与约束
  ...
  - current_chapter：整数，默认 1，当前章节进度
  - personality_profile/inventory/relationships：JSON，默认空结构，存储用户状态
- 索引设计
  - 主键索引（隐含）
  - id 索引（显式）
  - email 唯一索引
...
**更新** 将原有的 Player 模型完全替换为 User 模型，新增了完整的认证字段、社交登录支持和订阅管理功能。
...
**章节来源**
- [backend/models.py:81-101](file://backend/models.py#L81-L101)
...
**章节来源**
- [backend/models.py:167-220](file://backend/models.py#L167-L220)
...
**章节来源**
- [backend/models.py:244-265](file://backend/models.py#L244-L265)
...
**章节来源**
- [backend/models.py:267-291](file://backend/models.py#L267-L291)
...
**章节来源**
- [backend/models.py:330-350](file://backend/models.py#L330-L350)
...
- **新增** User 与 Asset：一对多关系，通过 user_id 外键关联
- **新增** User 与 ChatSession：一对多关系，通过 user_id 外键关联
...
**图表来源**
- [backend/models.py:1-383](file://backend/models.py#L1-L383)
- [backend/migrations/versions/b5d7e2f8a1c3_player_to_user_auth.py:94-109](file://backend/migrations/versions/b5d7e2f8a1c3_player_to_user_auth.py#L94-L109)
...
- 字符串与文本
  ...
  - **新增** String(255)/String(100)：用于邮箱、昵称等固定长度字段，提高索引效率
- JSON
...
**更新** 新增了密码哈希字段、社交登录标识字段和角色管理字段，以及UUID主键的统一使用。
...
**更新** 新增了用户认证系统的业务规则，包括密码加密、JWT令牌验证、角色权限控制和视频任务系统的业务规则。
...
- 组件耦合
  ...
  - **新增** auth.py 依赖 models.User，实现用户认证功能
  - **新增** routers.auth.py 依赖 auth.py 与 models.User，实现认证路由
  ...
  - **新增** admin 用户管理页面依赖 routers.auth 与 schemas，实现用户管理界面
- 外部依赖
  - SQLAlchemy 异步引擎与会话
  - Alembic 迁移框架
  - FastAPI 路由与依赖注入
...
DB --> MODELS["models.py"]
AUTH["auth.py"] --> MODELS
AUTH_ROUTER["routers/auth.py"] --> AUTH
AUTH_ROUTER --> MODELS
...
FRONTEND_USERS["admin/src/app/admin/players/page.tsx"] --> ROUTERS_AUTH
FRONTEND_PROMPT["admin/src/app/admin/prompt-templates/*"] --> ROUTERS_P
...
- [backend/admin/src/app/admin/players/page.tsx:1-450](file://backend/admin/src/app/admin/players/page.tsx#L1-L450)
- [backend/admin/src/app/admin/prompt-templates/page.tsx:1-268](file://backend/admin/src/app/admin/prompt-templates/page.tsx#L1-L268)
- [backend/admin/src/app/admin/prompt-templates/PromptTemplateDialog.tsx:1-416](file://backend/admin/src/app/admin/prompt-templates/PromptTemplateDialog.tsx#L1-L416)
- [backend/services/orchestrator.py:1-889](file://backend/services/orchestrator.py#L1-L889)
...
## 结论
本项目采用 SQLAlchemy 异步 ORM 与 Alembic 迁移体系，构建了可演进、可扩展的数据层。通过本次更新，系统新增了完整的用户认证系统，将原有的 Player 模型完全替换为 User 模型，引入了完整的认证和授权机制，包括邮箱验证、密码哈希、JWT令牌管理、双角色权限控制等。同时，系统还新增了完整的信用点计费系统、多智能体协作能力、Gemini配置支持、订阅计划管理、管理员信用点管理、提示词模板系统以及全新的视频生成和计费功能，形成了从用户管理到智能体协作再到模板化和视频生成的完整数据库架构。

User 与 StoryChapter 模型通过合理的字段类型、约束与索引设计，支撑动态叙事与一致性校验；新增的 CreditTransaction、TaskExecution、SubTask、SubscriptionPlan、Admin、PromptTemplate、**User** 和 **VideoTask** 模型完善了计费、协作、管理、认证、模板化、智能体类型区分和视频生成功能；迁移脚本与管理脚本提供了可控的版本演进路径；路由与服务层实现了数据验证与业务编排。

用户认证系统通过 User 模型支持邮箱密码登录、第三方社交登录、JWT令牌管理、角色权限控制等功能，为后续的功能扩展奠定了坚实的基础。视频任务系统通过 VideoTask 模型支持异步视频生成、状态追踪和计费功能，包括 text_to_video、image_to_video、edit 三种模式，支持时长、质量、宽高比等参数配置。计费系统使用映射表驱动的方式，避免复杂的 if-else 分支，支持输入图片计费、输出时长计费等多种计费维度。前端界面通过专门的用户管理界面实现完整的用户信息展示、积分管理和订阅管理体验。

建议在后续迭代中持续完善索引策略、JSON 查询与批量操作，特别是针对用户认证系统和视频任务系统的异步处理和计费计算，以进一步提升性能与可维护性。
