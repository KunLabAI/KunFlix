...
- 添加AdminDebugSession和AdminDebugMessage模型，支持独立的管理员调试功能
- 新增4d66cc052bfb版本号的数据库迁移，创建admin_debug_sessions和admin_debug_messages表
- 更新路由层，添加/admin/debug前缀的管理员调试API端点
- 新增管理员调试会话的Schema定义，支持独立的数据验证和序列化
...
**更新** 本次更新反映了应用的重大功能变更，新增了管理员调试会话和消息的数据模型，确保与用户数据完全隔离，提供独立的管理员调试功能，支持智能体测试、问题排查和功能验证。

## 项目结构
后端以 FastAPI + SQLAlchemy 异步 ORM 架构为核心，数据库层通过 async/await 实现高并发读写；迁移体系基于 Alembic，提供脚本化版本管理与批量渲染能力；模型文件集中定义数据结构，路由层负责业务校验与数据流编排。新增的管理员调试功能通过专门的admin_debug路由、服务层和Schema定义实现完整的调试会话管理，包括会话创建、消息记录、流式响应和计费统计。智能体画布控制权限追踪功能通过canvas_tools服务实现，支持基于节点类型的权限验证和节点创建追踪。
...
ADMIN_DEBUG["管理员调试路由<br/>routers/admin_debug.py"]
...
DB --> MODELS
AUTH --> MODELS
ADMIN_DEBUG --> ADMIN_DEBUG_MODELS
ADMIN_DEBUG --> ADMIN_DEBUG_SCHEMAS
ADMIN_DEBUG --> MODELS
...
- 模型集合：包含 User、Theater、TheaterNode、TheaterEdge、Agent、CreditTransaction、TaskExecution、SubTask、SubscriptionPlan、Admin、PromptTemplate、VideoTask、**新增** AdminDebugSession、**新增** AdminDebugMessage 等。
...
- **新增** 智能体画布控制权限追踪功能：通过agents表的target_node_types字段和theater_nodes表的created_by_agent_id字段，实现了对画布节点操作的精细化权限控制。
...
**更新** 新增了管理员调试会话和消息的数据模型，确保与用户数据完全隔离，提供独立的管理员调试功能。
...
## 架构总览
异步 ORM 架构围绕"配置 → 引擎 → 会话 → 模型 → 迁移"展开，路由层与服务层通过异步会话与模型交互，实现数据持久化与业务编排。新增的管理员调试系统通过专门的admin_debug路由模块提供独立的调试功能，支持会话创建、消息记录、流式响应和计费统计。智能体画布控制权限追踪功能通过canvas_tools服务实现，支持基于节点类型的权限验证和节点创建者追踪。前端界面通过专门的剧院页面实现可视化的画布编辑体验，包括节点拖拽、边连接、属性编辑等功能。

```mermaid
sequenceDiagram
participant Admin as "管理员"
participant AdminDebugRouter as "管理员调试路由<br/>routers/admin_debug.py"
participant AdminDebugModels as "调试模型<br/>models.py"
participant Session as "异步会话<br/>database.py"
participant AgentModel as "智能体模型<br/>models.py"
participant BillingService as "计费服务<br/>services/billing.py"
Admin->>AdminDebugRouter : "创建调试会话"
AdminDebugRouter->>AdminDebugModels : "AdminDebugSession"
AdminDebugRouter->>Session : "保存会话"
Session-->>AdminDebugRouter : "会话ID"
Admin->>AdminDebugRouter : "发送调试消息"
AdminDebugRouter->>AgentModel : "验证智能体"
AdminDebugRouter->>BillingService : "计算计费"
AdminDebugRouter->>AdminDebugModels : "AdminDebugMessage"
AdminDebugRouter-->>Admin : "流式响应"
Note over AdminDebugRouter,AdminDebugModels : "调试数据与用户数据完全隔离"
```
