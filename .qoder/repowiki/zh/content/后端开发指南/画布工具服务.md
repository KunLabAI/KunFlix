...
- 新增节点类型描述系统，提供详细的节点类型说明和字段描述
- 实现动态文本节点尺寸估算功能，根据内容长度自动调整节点大小
- 添加遗留节点类型迁移支持，兼容旧版节点类型命名规范
- 增强节点类型验证和描述功能，提升智能代理对画布操作的理解能力
...
**更新** 新增智能代理权限验证、实时同步(SSE)、Canvas节点类型管理、节点类型描述系统、动态文本节点尺寸估算和遗留节点类型迁移支持，增强了系统的安全性和实时协作能力。
...
- **节点类型描述系统**: 提供详细的节点类型说明和字段描述
- **动态尺寸估算**: 根据内容长度自动计算文本节点尺寸
- **遗留类型迁移**: 兼容旧版节点类型命名规范

主要节点类型支持：
- `text`: 文本节点，包含标题、描述、内容和标签
- `image`: 图像节点，包含名称、描述和图片 URL
- `video`: 视频节点，包含名称、描述和视频 URL
- `storyboard`: 故事板节点，包含镜头编号、描述、持续时间和透视配置

**更新** 新增智能代理权限验证机制，通过 `target_node_types` 字段限制代理可操作的节点类型，确保不同代理只能访问其授权范围内的画布操作。同时新增节点类型描述系统，提供详细的节点类型说明和字段描述，增强智能代理对画布操作的理解能力。

**章节来源**
- [canvas_tools.py:25-32](file://backend/services/canvas_tools.py#L25-L32)
- [canvas_tools.py:34-83](file://backend/services/canvas_tools.py#L34-L83)
- [canvas_tools.py:98-107](file://backend/services/canvas_tools.py#L98-L107)
...
**更新** 新增实时同步功能，通过 SSE 推送画布更新事件，实现多用户实时协作。同时增强节点尺寸管理功能，支持动态调整节点尺寸以适应内容变化。
...
+_EXECUTORS dict
+NODE_TYPE_INFO dict
+NODE_TYPE_SCHEMA dict
+NODE_TYPE_MIGRATION dict
+_migrate_node_type(node_type) str
+_estimate_text_node_size(data) tuple
+_build_node_type_description() str
}
...
THEATER_NODES ||--|| TEXT_NODE : "text"
THEATER_NODES ||--|| IMAGE_NODE : "image"
THEATER_NODES ||--|| VIDEO_NODE : "video"
THEATER_NODES ||--|| STORYBOARD_NODE : "storyboard"
```

**更新** 新增智能代理权限验证，通过 `target_node_types` 字段限制代理可操作的节点类型，确保不同代理只能访问其授权范围内的画布操作。同时新增节点类型描述系统，提供详细的节点类型说明和字段描述，增强智能代理对画布操作的理解能力。

**图表来源**
- [models.py:93-130](file://backend/models.py#L93-L130)
- [canvas_tools.py:27-32](file://backend/services/canvas_tools.py#L27-L32)
- [canvas_tools.py:38-83](file://backend/services/canvas_tools.py#L38-L83)
- [models.py:241-242](file://backend/models.py#L241-L242)
...
- [useCanvasStore.ts:185-461](file://frontend/src/store/useCanvasStore.ts#L185-L461)
- [AIAssistantPanel.tsx:199-242](file://frontend/src/components/canvas/AIAssistantPanel.tsx#L199-L242)
- [ScriptEditor.tsx:117-174](file://frontend/src/components/canvas/ScriptEditor.tsx#L117-174)
...
CreateNode --> LegacyMigration["遗留类型迁移"]
LegacyMigration --> SizeEstimation["动态尺寸估算"]
SizeEstimation --> AutoPosition["计算自动位置"]
...
**更新** 新增权限验证流程，在节点操作前检查代理的目标节点类型权限，确保操作的安全性。同时新增遗留类型迁移和动态尺寸估算流程，提升系统的兼容性和用户体验。
...
CanvasTools --> NodeTypeInfo
CanvasTools --> SizeEstimator
CanvasTools --> Migration
AssistantPanel --> SSE
PermissionValidator --> CanvasTools
NodeTypeInfo --> CanvasTools
SizeEstimator --> CanvasTools
Migration --> CanvasTools
```

**更新** 新增Tiptap编辑器、Markdown解析、节点类型信息、尺寸估算器和类型迁移等依赖关系，以及权限验证模块的依赖关系。
...
**更新** 新增动态尺寸估算、节点类型描述和遗留类型迁移的性能优化策略。
...
- **节点类型描述系统**: 提供详细的节点类型说明和字段描述
- **动态尺寸估算**: 根据内容长度自动调整节点大小
- **遗留类型迁移**: 兼容旧版节点类型命名规范
- **增强的验证功能**: 提升智能代理对画布操作的理解能力
