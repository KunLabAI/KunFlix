...
## 更新摘要
**所做更改**
- 新增完整的 Gemini Veo Provider 集成，支持 Veo 3.1 和 Veo 2.0 系列模型
- 扩展多提供商视频模型系统，现支持 xAI、MiniMax 和 Gemini 三大提供商
- 增强视频生成服务和媒体工具功能，完善统一适配器架构
...
- 完善 Gemini Veo 3.1 的原生音频支持和首尾帧功能
- 增强模型推断机制，支持更精确的提供商识别
...
BE5[视频提供商适配器<br/>xAI + MiniMax + Gemini]
BE6[模型能力配置<br/>动态参数验证]
end
subgraph "外部服务"
ES1[xAI 视频生成 API]
...
+SUPPORTED_MODELS = ["veo-3.1-generate-preview","veo-3.1-fast-generate-preview","veo-2.0-generate-001"]
+STATUS_MAP = {"false" : "processing","true" : "completed"}
+RESOLUTION_MAP = {"720p" : "720p","1080p" : "1080p","4k" : "4k"}
+ASPECT_RATIO_MAP = {"16 : 9" : "16 : 9","9 : 16" : "9 : 16"}
+submit(ctx) VideoResult
...
- [services/video_providers/gemini_provider.py:31-265](file://backend/services/video_providers/gemini_provider.py#L31-265)
- [services/video_generation.py:50-160](file://backend/services/video_generation.py#L50-L160)
- [services/video_providers/model_capabilities.py:9-223](file://backend/services/video_providers/model_capabilities.py#L9-L223)
...
Adapter-->>Service : VideoResult
Service->>Adapter : (MiniMax/Gemini) get_video_url/api_key(file_id/video_uri)
...
**图表来源**
- [services/video_generation.py:80-123](file://backend/services/video_generation.py#L80-L123)
...
- [services/video_providers/gemini_provider.py:178-219](file://backend/services/video_providers/gemini_provider.py#L178-219)

#### 支持的提供商对比

| 特性 | xAI | MiniMax | Gemini |
|------|-----|---------|--------|
| 支持模型 | grok-imagine-video | Hailuo系列、T2V系列、I2V系列、S2V系列 | Veo 3.1系列、Veo 2.0系列 |
| 生成模式 | 文字生成、图片生成、视频编辑 | 文字生成、图片生成、主题参考生成 | 文字生成、图片生成、视频扩展 |
| 时长支持 | 1-15秒 | 6秒、10秒 | 4秒、6秒、8秒、10秒 |
| 画质支持 | 480p、720p | 480p、512p、720p、768p、1080p | 720p、1080p、4K |
| 首帧图片 | 支持 | 部分模型支持 | 部分模型支持 |
| 尾帧图片 | 不支持 | MiniMax-Hailuo-02支持 | Veo 3.1系列支持 |
| 提示词优化 | 不支持 | 支持 | 不支持 |
| 快速预处理 | 不支持 | 部分模型支持 | 不支持 |
| 原生音频 | 不支持 | 不支持 | Veo 3.1系列支持 |
| 下载方式 | 直接返回 | 需要额外API获取 | 需要API key下载 |
...
- [services/video_providers/gemini_provider.py:34-38](file://backend/services/video_providers/gemini_provider.py#L34-L38)
- [services/video_providers/model_capabilities.py:23-204](file://backend/services/video_providers/model_capabilities.py#L23-L204)
...
| Gemini-Veo 3.1 | 文字生成、图片生成、视频扩展 | 4秒、6秒、8秒 | 720p、1080p、4K | 首尾帧支持、原生音频 |
| Gemini-Veo 2.0 | 文字生成、图片生成 | 5秒、6秒、8秒 | 720p | 首帧支持、无声 |

**章节来源**
- [services/video_providers/model_capabilities.py:23-204](file://backend/services/video_providers/model_capabilities.py#L23-L204)
- [backend/admin/src/types/video.ts:5-54](file://backend/admin/src/types/video.ts#L5-L54)
...
**图表来源**
- [services/video_generation.py:50-76](file://backend/services/video_generation.py#L50-L76)
- [services/video_providers/base.py:49-114](file://backend/services/video_providers/base.py#L49-L114)
...
| gemini | GeminiVeoAdapter | Veo 3.1系列、Veo 2.0系列 | 原生音频、首尾帧支持、API key下载 |

**章节来源**
- [services/video_generation.py:44-51](file://backend/services/video_generation.py#L44-L51)
...
Service->>Factory : get_provider_adapter(provider_type)
Factory-->>Service : 返回适配器实例
Service->>Adapter : poll_with_key(api_key, task_id/operation_name)
Adapter-->>Service : VideoResult
Service->>Adapter : (MiniMax/Gemini) get_video_url/get_video_url(api_key, file_id/video_uri)
Adapter-->>Service : 下载URL
Service-->>Client : VideoResult
```

**图表来源**
- [services/video_generation.py:84-123](file://backend/services/video_generation.py#L84-L123)
...
CheckPatterns --> |包含'veo-'| Gemini["推断为Gemini"]
CheckPatterns --> |包含'hailuo'或'minimax'或't2v-01'或'i2v-01'或's2v-01'| Minimax["推断为MiniMax"]
CheckPatterns --> |其他| XAI["推断为xAI"]
Gemini --> Result[返回'gemini']
Minimax --> Result[返回'minimax']
XAI --> Result[返回'xai']
```

**图表来源**
- [services/video_generation.py:129-160](file://backend/services/video_generation.py#L129-L160)

**章节来源**
- [services/video_generation.py:84-160](file://backend/services/video_generation.py#L84-L160)
...
**章节来源**
- [backend/admin/src/app/admin/videos/page.tsx:1-270](file://backend/admin/src/app/admin/videos/page.tsx#L1-L270)
...
**章节来源**
- [backend/admin/src/app/admin/videos/VideoPreviewModal.tsx:1-122](file://backend/admin/src/app/admin/videos/VideoPreviewModal.tsx#L1-L122)
...
系统新增了完整的视频任务删除功能，支持已完成和失败任务的清理：

```mermaid
flowchart TD
Delete[删除任务] --> CheckStatus{检查任务状态}
CheckStatus --> |completed/failed| AllowDelete[允许删除]
CheckStatus --> |pending/processing| DenyDelete[拒绝删除]
AllowDelete --> Confirm[删除确认对话框]
Confirm --> DeleteAPI[调用删除API]
DeleteAPI --> DeleteLocal[删除本地文件]
DeleteLocal --> DeleteChatMsg[删除关联聊天消息]
DeleteChatMsg --> DeleteDB[删除数据库记录]
DeleteDB --> Success[删除成功]
DenyDelete --> Error[状态不允许删除]
```

**图表来源**
- [routers/videos.py:266-297](file://backend/routers/videos.py#L266-L297)
- [backend/admin/src/app/admin/videos/page.tsx:78-95](file://backend/admin/src/app/admin/videos/page.tsx#L78-L95)

#### 删除限制

- **可删除状态**：仅已完成(completed)和失败(failed)任务可删除
- **本地文件清理**：删除任务时同时清理对应的本地视频文件
- **关联数据清理**：删除任务时清理相关的聊天消息和数据库记录
- **权限控制**：仅管理员和任务创建者可执行删除操作

**章节来源**
- [routers/videos.py:262-297](file://backend/routers/videos.py#L262-L297)
- [backend/admin/src/app/admin/videos/page.tsx:44](file://backend/admin/src/app/admin/videos/page.tsx#L44)
...
2. **多提供商支持**：支持xAI、MiniMax和Gemini三提供商，提供更好的灵活性
...
12. **完整的生命周期管理**：支持任务创建、状态轮询、结果下载、计费扣减和删除清理
