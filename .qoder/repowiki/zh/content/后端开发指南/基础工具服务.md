...
- [shell.py](file://CoPaw-main/src/copaw/agents/tools/shell.py)
- [dangerous_shell_commands.yaml](file://CoPaw-main/src/copaw/security/tool_guard/rules/dangerous_shell_commands.yaml)
- [engine.py](file://CoPaw-main/src/copaw/security/tool_guard/engine.py)
- [config.py](file://CoPaw-main/src/copaw/config/config.py)
- [__init__.py](file://CoPaw-main/src/copaw/agents/tools/__init__.py)
- [react_agent.py](file://CoPaw-main/src/copaw/agents/react_agent.py)
</cite>
...
无限游戏项目是一个基于FastAPI的AI创意平台，提供了完整的视频生成、智能体协作和内容创作功能。该项目的核心是"基础工具服务"，这些服务为整个系统提供了基础设施支持，包括数据库管理、认证授权、计费系统、媒体处理、视频生成和**shell命令执行**等关键功能。

该项目采用现代化的异步架构设计，支持多种AI模型提供商，具备完善的权限管理和计费系统，能够处理复杂的多智能体协作场景。**新增的shell命令执行系统**为智能体提供了强大的系统操作能力，同时内置了完善的安全防护机制。
...
**新增功能**：系统现在提供完整的shell命令执行能力，支持安全的命令执行、超时机制和输出截断。

```mermaid
flowchart TD
Start([Shell命令执行开始]) --> Validate["参数验证<br/>检查命令格式"]
Validate --> Sanitize["命令安全检查<br/>危险命令检测"]
Sanitize --> Safe{"命令安全?"}
Safe --> |否| Block["阻止执行<br/>返回安全警告"]
Safe --> |是| Execute["执行命令<br/>设置超时机制"]
Execute --> Platform{"操作系统?"}
Platform --> |Windows| WinExec["Windows专用执行<br/>进程树管理"]
Platform --> |Unix/Linux| UnixExec["Unix/Linux执行<br/>异步子进程"]
WinExec --> Capture["捕获输出<br/>标准输出和错误输出"]
UnixExec --> Capture
Capture --> Timeout{"超时检查"}
Timeout --> |超时| Terminate["终止进程<br/>清理资源"]
Timeout --> |正常| Truncate["输出截断<br/>限制最大长度"]
Terminate --> Format["格式化输出<br/>包含退出码"]
Truncate --> Format
Format --> Return["返回执行结果"]
Block --> Return
```

**图表来源**
- [shell.py:125-277](file://CoPaw-main/src/copaw/agents/tools/shell.py#L125-L277)
- [dangerous_shell_commands.yaml:12-120](file://CoPaw-main/src/copaw/security/tool_guard/rules/dangerous_shell_commands.yaml#L12-L120)

**新增功能特性**：
- **安全防护**：内置危险命令检测规则，包括删除、移动、格式化、反向连接等高危操作
- **超时机制**：默认60秒超时，防止长时间运行的恶意命令
- **跨平台支持**：Windows使用进程树管理，Unix/Linux使用异步子进程
- **输出截断**：防止大量输出导致的内存问题
- **OpenAI格式**：完全符合OpenAI函数调用规范

**章节来源**
- [shell.py:1-288](file://CoPaw-main/src/copaw/agents/tools/shell.py#L1-L288)
- [dangerous_shell_commands.yaml:1-120](file://CoPaw-main/src/copaw/security/tool_guard/rules/dangerous_shell_commands.yaml#L1-L120)
- [engine.py:1-218](file://CoPaw-main/src/copaw/security/tool_guard/engine.py#L1-L218)
...
**新增功能**：
- **超时控制**：默认60秒超时，防止长时间运行的命令
- **进程管理**：Windows使用进程树管理，确保子进程被正确清理
- **输出截断**：防止大量输出导致内存溢出
- **编码处理**：智能解码支持多种字符编码
...
- [shell.py:173-239](file://CoPaw-main/src/copaw/agents/tools/shell.py#L173-L239)
- [dangerous_shell_commands.yaml:12-120](file://CoPaw-main/src/copaw/security/tool_guard/rules/dangerous_shell_commands.yaml#L12-L120)
...
- **异步架构**：高效的并发处理能力
- **安全机制**：多层次的安全防护措施，特别是新增的shell命令安全系统
- **监控完善**：全面的性能和错误监控
- **扩展性强**：支持新功能和服务的快速集成

**新增功能的价值**：
- **Shell命令执行系统**：为智能体提供强大的系统操作能力
- **安全防护机制**：内置危险命令检测和超时控制
- **跨平台兼容**：支持Windows和Unix/Linux系统
- **OpenAI兼容**：完全符合OpenAI函数调用规范
