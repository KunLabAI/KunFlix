...
这是一个基于 FastAPI 的多提供商视频生成系统，支持集成多个视频生成 AI 服务提供商，包括 xAI (Grok Video) 和 MiniMax (Hailuo) 系列模型。系统采用适配器模式设计，提供了统一的接口来管理不同提供商的视频生成服务。

该系统的核心特性包括：
- 多提供商支持：统一管理 xAI 和 MiniMax 视频生成服务
- 模型能力配置：详细的模型参数和能力映射
- 统一 API 接口：简化前端集成和使用
...
Adapter-->>Service : VideoResult
Service->>Adapter : get_video_url(file_id) (MiniMax)
...
**图表来源**
- [backend/services/video_generation.py:80-120](file://backend/services/video_generation.py#L80-L120)
...
| 模型类型 | 支持模式 | 时长限制 | 分辨率 | 首帧支持 | 尾帧支持 | 提示词优化 |
|---------|---------|---------|-------|---------|---------|-----------|
| MiniMax-Hailuo-2.3 | 文本生成, 图片生成 | 6, 10秒 | 768P, 1080P | 是 | 否 | 是 |
| MiniMax-Hailuo-02 | 文本生成, 图片生成 | 6, 10秒 | 512P, 768P, 1080P | 是 | 是 | 是 |
| T2V-01 | 文本生成 | 6秒 | 720P | 否 | 否 | 是 |
| I2V-01 | 图片生成 | 6秒 | 720P | 是 | 否 | 是 |
| S2V-01 | 主题参考 | 6秒 | 720P | 否 | 否 | 是 |
| grok-imagine-video | 文本生成, 图片生成, 编辑 | 1-15秒 | 480P, 720P | 是 | 否 | 否 |

**章节来源**
- [backend/services/video_providers/model_capabilities.py:22-180](file://backend/services/video_providers/model_capabilities.py#L22-L180)
