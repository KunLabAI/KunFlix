...
- [llm_stream.py:1-800](file://backend/services/llm_stream.py#L1-L800)
...
## 结论
该体系通过“适配器基类 + 注册表分发 + 能力配置中心”的架构，实现了对多家 LLM/多模态供应商的一致接入与扩展。统一的流式处理、参数适配与错误处理策略显著降低了集成复杂度；通过能力表与状态映射，进一步提升了跨供应商的稳定性与可维护性。建议在新增供应商时遵循现有模式，确保最小改动即可上线。
