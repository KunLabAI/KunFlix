...
- 聊天与智能体
  ...
  - agents：智能体配置，含供应商绑定、工具、思考模式、定价与上下文压缩配置。
- 计费与审计
...
- 迁移脚本：按功能演进拆分，如初始表、UUID 迁移、积分计费、用户/管理员拆分。
...
## 结论
本数据库设计围绕“统一 UUID 主键、JSON 承载动态配置、严格的外键与索引、完善的迁移体系与审计流水”展开，满足多智能体协作、画布创作、视频生成与计费审计等核心业务需求。通过异步连接池、SQLite WAL 优化与批量查询策略，兼顾性能与可靠性。建议在生产环境中强化敏感字段加密、引入 Redis 缓存与更细粒度的监控告警。
