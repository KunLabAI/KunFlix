...
import { useCanvasStore, type AudioNodeData } from '@/store/useCanvasStore';
import { useResourceStore } from '@/store/useResourceStore';
import {
  MAX_AUDIO_SIZE_BYTES,
  VALID_AUDIO_TYPES,
} from '@/components/canvas/AudioNode/constants';

/**
 * 封装音频节点上传：XHR + 进度 + 错误；同时暴露资产库选择。
 */
export function useAudioNodeUpload(id: string) {
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 校验类型与大小
    const invalidType = !VALID_AUDIO_TYPES.includes(file.type);
    if (invalidType) {
      setUploadError(t('canvas.node.upload.audioFormatError', '不支持的音频格式'));
      return;
    }
    if (file.size > MAX_AUDIO_SIZE_BYTES) {
      setUploadError(t('canvas.node.upload.audioSizeError', '音频文件过大'));
      return;
    }
    ...
    try {
      ...
      updateNodeData(id, { uploading: false } as Partial<AudioNodeData>);
    } finally {
