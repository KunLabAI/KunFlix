...
import type { MusicModelFlat } from '@/hooks/useMusicGeneration';
import type { FlatMusicModelItem } from './types';

interface Props {
  selectedModelKey: string;
  selectedModel: MusicModelFlat | null;
  selectedProviderType: string;
  flatModels: FlatMusicModelItem[];
  modelsCount: number;
...
/** 音乐模型选择器（与 VideoGeneratePanel/ModelSelector 保持一致的 UI） */
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {modelsLoading ? '...' : (selectedModel?.display_name || t('canvas.node.audio.selectModel', '选择模型'))}
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 w-64 max-h-72 overflow-y-auto rounded-lg border border-border/50 bg-popover shadow-lg z-50 animate-in fade-in zoom-in-95 duration-100 custom-scrollbar">
          {flatModels.map(({ key, model: m, providerType }) => {
          ...
          {modelsCount === 0 && !modelsLoading && (
            <div className="p-3 text-[10px] text-muted-foreground text-center">
              {t('canvas.node.audio.noMusicProviders', '尚未配置音乐模型')}
            </div>
          )}
        </div>
      )}
